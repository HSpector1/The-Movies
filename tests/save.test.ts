...
import type { SaveFileV3 } from '../src/core/save.js'
...
// makeSave is the D-11 default (V3), so a new-game save is a SaveFileV3.
function wellFormedSave(): SaveFileV3 {
...
describe('§17 — loud rejection of an unknown saveVersion', () => {
  it('throws on an unknown saveVersion (e.g. 4)', () => {
    // Source: §17 "loud rejection of unknown versions". Versions 1, 2 and 3 are all
    // valid now (D-9 SaveFileV2 + D-11 SaveFileV3); any other version is rejected
    // loudly. V3 acceptance is covered by the round-trip tests above.
    const save = wellFormedSave()
    const bad = { ...save, saveVersion: 4 } as unknown as SaveFileV3
...
describe('M14 — loud rejection when envelope seed ≠ state.seed', () => {
  it('throws when the envelope seed diverges from state.seed', () => {
    ...
    const save = wellFormedSave()
    const bad: SaveFileV3 = { ...save, seed: 'a-different-seed' }
    expect(() => loadSave(bad)).toThrow()
  })
})
...
describe('M14 — loud rejection when broadcastCache ≠ state.broadcastItems', () => {
  it('throws when broadcastCache diverges from state.broadcastItems', () => {
    ...
    const bad: SaveFileV3 = { ...save, broadcastCache: [divergentItem] }
    expect(() => loadSave(bad)).toThrow()
  })
  ...
  it('throws when broadcastCache differs from state.broadcastItems by length', () => {
    ...
    const save = wellFormedSave()
    const bad: SaveFileV3 = { ...save, broadcastCache: [] }
    expect(() => loadSave(bad)).toThrow()
  })
})
