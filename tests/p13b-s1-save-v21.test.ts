...
function roundTripsByteIdentical(state: GameState): string {
  ...
  const restored = migrateToLive(importSave(direct)).state
...
describe('P13B-S1 V20 to V21 migration (test 8)', () => {
  ...
  it('continues the migrated active-280 project lawfully for one more funded week and re-saves byte-identically', () => {
    // Lifted all the way to the live boundary (V34, P14C.2a sweep — was V26 at
    // authoring, then V32, then V33): this result flows into
    // `roundTripsByteIdentical`, which calls `makeSave` directly and requires
    // the live-shaped leaves `migrateToV21` alone never adds.
    const migrated = migrateToLive(importSave(load('./fixtures/p13b/legacy-v20-research-active-280.json.gz')))
  ...
  it('keeps the migrated paused-expired seat retained but ineligible, refuses resumeResearch, then accepts it after rehiring the same id', () => {
    const migrated = migrateToLive(importSave(load('./fixtures/p13b/legacy-v20-research-paused-expired-468.json.gz'))).state
...
describe('P13B-S1 campaign isolation (test 10)', () => {
  it('produces independent migrated copies from the same fixture; advancing one never touches the other', () => {
    ...
    // Lifted to the live boundary (V34, P14C.2a sweep — was V26 at authoring, then V32, then V33) —
    // `b.state` round-trips through `makeSave` below, which requires the
    // live-shaped leaves.
    const a = migrateToLive(importSave(json))
    const b = migrateToLive(importSave(json))
