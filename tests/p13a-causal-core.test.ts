...
import { exportCurrentState, importSave, migrateToV23, validateSaveV23, makeSave } from '../src/core/save.js'
...
describe('P13A causal Core through the real weekly engine',()=>{
  ...
  it.each([1,3,2499,5600,9999])('retains exact paid work and reloads a lawful $%s weekly budget', budgetPerWeek => {
    ...
    const restored = migrateToV23(importSave(exportCurrentState(mid))).state
  ...
  it('one Scientist completes 64 units in 43 funded weeks, then P09 completes the exact chain at315',()=>{
    ...
    const restored=migrateToV23(importSave(exportCurrentState(state))).state
  ...
  it('saturation caps both output and actual spend, while pause/cancel/resume retain work',()=>{
    ...
    const roundtrip=migrateToV23(importSave(exportCurrentState(state))).state
