...
import { applyActions, exportSave, importSave, makeSave, stableStringify, tick, validateSaveV21 } from '../../src/core/index.js'
import type { GameState, SaveFileV21 } from '../../src/core/index.js'
import {
  auditionSlate,
  availableConceptId,
  availableWriterId,
...
function legalSave(state: GameState): SaveFileV21 {
...
function proveRefusal(
  legal: SaveFileV21,
  forge: (save: SaveFileV21) => { restore: (save: SaveFileV21) => void; slotKey: string },
  label: string,
): void {
  const legalJson = exportSave(legal)
  expect(exportSave(importSave(legalJson) as SaveFileV21), `${label}: legal twin round-trips`).toBe(
    legalJson,
  )
  ...
  expect(validateSaveV21(forged as SaveFileV21)).toBe(forged)
}
