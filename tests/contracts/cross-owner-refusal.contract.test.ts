...
import { applyActions, exportSave, importSave, makeSave, stableStringify, tick, validateSaveV29 } from '../../src/core/index.js'
import type { GameState, SaveFileV29 } from '../../src/core/index.js'
import {
  auditionSlate,
  availableConceptId,
  availableWriterId,
...
function legalSave(state: GameState): SaveFileV29 {
...
function proveRefusal(
  legal: SaveFileV29,
  forge: (save: SaveFileV29) => { restore: (save: SaveFileV29) => void; slotKey: string },
  label: string,
): void {
  const legalJson = exportSave(legal)
  expect(exportSave(importSave(legalJson) as SaveFileV29), `${label}: legal twin round-trips`).toBe(
    legalJson,
  )
  ...
  expect(validateSaveV29(forged as SaveFileV29)).toBe(forged)
}
