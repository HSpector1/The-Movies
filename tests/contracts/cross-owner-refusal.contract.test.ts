...
import { applyActions, exportSave, importSave, makeSave, stableStringify, tick, validateSaveV26 } from '../../src/core/index.js'
import type { GameState, SaveFileV26 } from '../../src/core/index.js'
import {
  auditionSlate,
  availableConceptId,
  availableWriterId,
...
function legalSave(state: GameState): SaveFileV26 {
...
function proveRefusal(
  legal: SaveFileV26,
  forge: (save: SaveFileV26) => { restore: (save: SaveFileV26) => void; slotKey: string },
  label: string,
): void {
  const legalJson = exportSave(legal)
  expect(exportSave(importSave(legalJson) as SaveFileV26), `${label}: legal twin round-trips`).toBe(
    legalJson,
  )
  ...
  expect(validateSaveV26(forged as SaveFileV26)).toBe(forged)
}
