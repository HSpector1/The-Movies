...
describe('D-17A/D — validateSave still guards the version boundary loudly', () => {
  ...
  // bump on.
  it('rejects unknown V34 loudly (stale title corrected post-C.1; the boundary moves on every save bump)', () => {
    expect(() => validateSave({ ...v6, saveVersion: 34 })).toThrow(/unknown saveVersion 34/)
  })
