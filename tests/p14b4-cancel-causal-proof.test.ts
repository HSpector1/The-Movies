...
//
// AUTHORED AGAINST THE LAW, NOT THE OUTPUT. The current coupling (promises.ts :753) breaks a promise
// whenever `reclassifyPromise(...).classification === 'IMPOSSIBLE'`. Every RED case therefore asserts,
// BEFORE the cancel, that the current service refuses with the exact NON-target-specific bottleneck the
// case isolates (a joint reservation, the classless fresh-offer refusal, an unsupported family, a
// class-blind physical bound, or the ORIGINAL count instead of the remaining count). A case that later
// turns GREEN because its premise stopped holding fails at the premise line, not at the expectation.
//
...
//
// SCHEDULE LAW TRANSCRIBED (promises.ts :352-367 `expectedFirstTakeWeek`; the one physical clock this
// source owns and the bound the plan's "hard physical/lawful impossibility" can be computed from here):
...
//   does not move the physical bound, only the existing-path count. A picture at remainingTicks 5
//   expects its take at week + 1, so cancelling it moves the bound from week + 1 to week + 5 (case 6c).
//
// CAUSE STRINGS pinned below are DESIGN-OWNED: the cancel cause is the current :755 literal; if the
// corrected proof renames it, change `CANCEL_CAUSE` here (one line) and record the new string.
...
// ── design-owned strings (promises.ts :690-755) ──────────────────────────────
...
// current NON-target-specific refusal bottlenecks the RED premises isolate (promises.ts :213-217, :392-394, :420-421)
...
// ── schedule law, transcribed (see header) ───────────────────────────────────
...
/** N_max once the person's only running picture is gone: fresh events expected strictly before `due`. */
function nMaxAfterCancel(from: number, due: number): number {
  let n = 0
...
let tookCache: Took | undefined
/** actors[0] LEAD on picture A, walked to its REAL first take (assign director, schedule, tick 5 -> 4). */
function took(): Took {
  if (tookCache === undefined) {
    const r = roster()
    const five = atFive(r, { lead: r.actors[0]!, antagonist: r.actors[1]!, support: r.actors[2]! })
    ...
    const take = takes[0]!
    expect(take).toMatchObject({ studioId: player(state), week: state.market.tick, cast: { lead: r.actors[0] } })
    tookCache = { state, productionId: five.productionId, greenlightWeek: five.greenlightWeek, take }
  }
  return structuredClone(tookCache)
}
...
describe('629-T case 1: a joint reservation conflict is not causal proof (plan :249-256; 26 §2 "a joint reservation conflict ... is not target-specific physical impossibility")', () => {
  it('one person, two bound OPEN count-1 promises, seated LEAD on one greenlit picture, one event still reachable after the cancel: NEITHER is BROKEN at the cancel; the due-week owner decides', () => {
    ...
    expect(nMaxAfterCancel(w, due)).toBe(1)
...
describe('629-T case 2: a bound legacy P2 is never routed through fresh-offer refusal (26 §2 last sentence; plan :257 "legacy generic-cast compatibility")', () => {
  it('legacy count-only P2 (no kind), person seated SUPPORT (generic cast qualifies any seat), wide window: NOT broken at the cancel', () => {
    ...
    expect(nMaxAfterCancel(w, due)).toBe(2)
  ...
  it('legacy count-only P2, person seated SUPPORT, window physically too tight after the cancel (due = week + 2): BROKEN at the cancel — target-specific physical impossibility applies to legacy roots too', () => {
    ...
    const w = g.week
    const due = w + 2
    expect(nMaxAfterCancel(w, due)).toBe(0)
...
describe('629-T case 6: target-specific physical impossibility still breaks at the cancel (plan :252-254)', () => {
  ...
  it('differential shape: the picture at remainingTicks 5 COULD have served (take expected week + 1 < due = week + 3) and nothing else can after the cancel — BROKEN at the cancel', () => {
    ...
    expect(nMaxAfterCancel(w, due)).toBe(0) // after: a fresh greenlight first-takes at w+5 >= due
