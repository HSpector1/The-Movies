...
describe('P09 R1 — the founding regime is persisted, exact, and immutable', () => {
  ...
  it('V18 round-trips the regime and refuses a forged or missing one', () => {
    ...
    expect(() => validateSaveV34({ ...raw, state: { ...raw.state, foundingRegime: 'sandbox' } })).toThrow(/not a known founding regime/)
    const { foundingRegime: _r, ...missing } = raw.state
    expect(() => validateSaveV34({ ...raw, state: missing })).toThrow(/foundingRegime is missing/)
    // Laundering: an endowed property cannot claim the bare-lot regime, nor the reverse.
    expect(() => validateSaveV34({ ...raw, state: { ...raw.state, foundingRegime: 'bare-lot' } })).toThrow(/cannot carry founding structures/)
    const bare = JSON.parse(exportSave(makeSave(bareLot('p09-r1-forge-b')))) as { state: Record<string, unknown> }
    expect(() => validateSaveV34({ ...bare, state: { ...bare.state, foundingRegime: 'endowed' } })).toThrow(/must carry its founding structures/)
  })
