...
import { exportCurrentState, importSave, makeSave, migrateToV32 } from '../src/core/save.js'
...
describe('P13A exact historical technology membership under local indexing', () => {
  ...
  it('does not reuse membership across independent round-tripped copies with the same world and entity IDs', () => {
    const original = migrateToV32(importSave(sourceJson)).state
    const branch = migrateToV32(importSave(sourceJson)).state
