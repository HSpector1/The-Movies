...
import { exportCurrentState, importSave, makeSave, migrateToLive } from '../src/core/save.js'
...
describe('P13A exact historical technology membership under local indexing', () => {
  ...
  it('does not reuse membership across independent round-tripped copies with the same world and entity IDs', () => {
    const original = migrateToLive(importSave(sourceJson)).state
    const branch = migrateToLive(importSave(sourceJson)).state
