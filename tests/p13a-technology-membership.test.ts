...
import { exportCurrentState, importSave, makeSave, migrateToV27 } from '../src/core/save.js'
...
describe('P13A exact historical technology membership under local indexing', () => {
  ...
  it('does not reuse membership across independent round-tripped copies with the same world and entity IDs', () => {
    const original = migrateToV27(importSave(sourceJson)).state
    const branch = migrateToV27(importSave(sourceJson)).state
