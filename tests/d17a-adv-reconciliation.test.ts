...
// explicit commitment. This drive commits every ready picture before each tick so the
// hostile schedule reaches its releases on the original timeline — committing at the ready
// week adds NO week (P06A W1), so nothing about the asserted timing below changes.
...
describe('D-17A/B — odd dollars: whole-dollar determinism and a governed tie-break', () => {
  ...
  it('is identical across two independent calls and across a save → validate → migrate → load trip', () => {
    ...
    // Property State V13 (C1-M1a): the live load-to-play entry is migrateToV16.
    const reloaded = migrateToV16(validateSave(importSave(exportSave(makeSave(s)))))
    ...
    const hBack = migrateToV16(validateSave(importSave(exportSave(makeSave(h)))))
