...
import { exportSave, importSave, makeSave, migrateToLive, validateSaveV29, validateSaveV36 } from '../src/core/save.js'
...
function oldBound() {
  if (oldBoundCache === undefined) {
    ...
    const state: GameState = structuredClone(migrateToLive(saved).state) // the governed lift: the empty relationship and lifecycle roots only
...
describe('P14B4 genuine submit/settle/take routes, no synthetic commitments', () => {
  it.each(CLASSES)('PLAYER: actual offered %s P2 settles and the real class-compatible take satisfies it', (seatClass) => {
    ...
    const reloaded = validateSaveV36(importSave(exportSave(makeSave(after.state))))
