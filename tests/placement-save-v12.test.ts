...
describe('Placement Core V12 — the frozen envelope', () => {
  ...
  it('rejects overlapping placements and clearance-ring violations in a forged save', () => {
    ...
    expect(validateSaveV17(valid)).toBe(valid)
    ...
    expect(() => validateSaveV17(overlapped)).toThrow(/overlaps placed facility 1/)
    ...
    expect(() => validateSaveV17(tooClose)).toThrow(/violates its clearance ring/)
  })
  ...
  it('rejects a forged operating charge that disagrees with the operational facilities', () => {
    ...
    expect(() => validateSaveV17(doubled)).toThrow(
    ...
    expect(() => validateSaveV17(early)).toThrow(
...
describe('Placement Core V12 — historical boundary guards (law 19)', () => {
  ...
  it('refuses to downgrade a V13 save through migrateToV12 or any earlier boundary', () => {
    ...
    expect(() => migrateToV12(v13)).toThrow(
      /migrateToV12: cannot downgrade SaveFileV17 or discard property, set, queue, screenplay, and studio-history state/,
    )
    expect(() => migrateToV11(v13)).toThrow(
      /cannot downgrade SaveFileV17 or discard placement and property state/,
    )
    expect(() => migrateToV10(v13)).toThrow(
      /migrateToV10: cannot downgrade SaveFileV17/,
    )
  })
...
describe('Placement Core V12 — the V11 → V12 migration', () => {
  ...
  it('gives a managed VACANT world the empty managed root and a free parcel', () => {
    ...
    const started = applyActions(convertV16ToV17(convertV15ToV16(convertV14ToV15(convertV13ToV14(convertV12ToV13(v12))))).state, [{ kind: 'startDevelopmentCastingAnnex' }])
  ...
  it('moves a BUILDING legacy Annex onto the legacy parcel with the same clock', () => {
    ...
    const completed = advance(
      convertV16ToV17(convertV15ToV16(convertV14ToV15(convertV13ToV14(convertV12ToV13(v12))))).state,
  ...
  it('moves a COMPLETED legacy Annex without disturbing its facility or reservations', () => {
    ...
    const next = tick(convertV16ToV17(convertV15ToV16(convertV14ToV15(convertV13ToV14(convertV12ToV13(v12))))).state)
