...
describe('Placement Core V12 — the frozen envelope', () => {
  ...
  it('rejects overlapping placements and clearance-ring violations in a forged save', () => {
    ...
    expect(validateSaveV32(valid)).toBe(valid)
    ...
    expect(() => validateSaveV32(overlapped)).toThrow(/overlaps placed facility 1/)
    ...
    expect(() => validateSaveV32(tooClose)).toThrow(/violates its clearance ring/)
  })
  ...
  it('rejects a forged operating charge that disagrees with the operational facilities', () => {
    ...
    expect(() => validateSaveV32(doubled)).toThrow(
    ...
    expect(() => validateSaveV32(early)).toThrow(
