// Installed from independently frozen194 (195 qualified KEEP); original inert provenance below retained unchanged.
...
import { buildScenario, greenlightA, projectById, SEED } from './_p04a2WriterCreditFixtures.js'
...
describe('P14B4 started replay — real background and explicit command supplement', () => {
  it('releases original due-writing holds BEFORE sweep while new-start A keeps a DIFFERENT slot through later 8→7', () => {
    ...
    // New combined premise: real initialization +132's actual deadlock route.
    // It is guarded, not claimed previously executed or repaired if incompatible.
    const state = applyActions(initializeHollywood(scenario.deadlock, 'fresh'), [greenlightA(scenario)])
    supported(state)
    assert.ok(state.hollywood)
