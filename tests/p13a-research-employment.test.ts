...
import { exportSave, importSave, makeSave, migrateToV30 } from '../src/core/save.js'
...
describe('P13A research employment continuity', () => {
  it('releasing the actual Scientist pauses work immediately and survives save/reload without losing verified work', () => {
    ...
    const restored = migrateToV30(importSave(exportSave(makeSave(released)))).state
    ...
    expect(makeSave(migrateToV30(importSave(exportSave(makeSave(rehired)))).state)).toEqual(makeSave(rehired))
  }, 30_000)
})
