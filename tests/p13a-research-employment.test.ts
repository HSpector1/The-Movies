...
describe('P13A research employment continuity', () => {
  it('releasing the actual Scientist pauses work immediately and survives save/reload without losing verified work', () => {
    ...
    const restored = migrateToV22(importSave(exportSave(makeSave(released)))).state
    ...
    expect(makeSave(migrateToV22(importSave(exportSave(makeSave(rehired)))).state)).toEqual(makeSave(rehired))
  }, 30_000)
})
