...
import { exportSave, importSave, makeSave, migrateToLive, validateSaveV35 } from '../src/core/save.js'
...
describe('P13B-S2 access identity: completion grant must key on (studioId, technologyId)', () => {
  ...
  it('6. save/reload and later ticks: exactly two rows, no duplicates, no erasure; validateSaveV35 accepts (stale title said V32 before this sweep too)', () => {
    const { afterSecond, firstId, secondId } = soundThenLight
    const json = exportSave(makeSave(afterSecond))
    const imported = importSave(json)
    const migrated = migrateToLive(imported)
...
describe('P13B-S2 access identity: forward validator invariant "completed research without its access grant"', () => {
   ...
   * `validateSaveV35`, exercising the full save-file boundary, not just the
   * in-memory validator). */
