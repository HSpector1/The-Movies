...
describe("frozen save builders project exact historical roots", () => {
  it("makeSaveV1 through makeSaveV7 omit every later and unknown live-state root", () => {
    const live = {
      ...generateWorld("frozen-builder-projection"),
      futureV10State: { mustNotLeak: true },
    };
    ...
    for (const { version, save, roots } of cases) {
      ...
      expect("futureV10State" in save.state).toBe(false);
    }
    ...
    expect("futureV10State" in migrated.state).toBe(false);
