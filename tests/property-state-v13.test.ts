...
function withoutTalentProvenanceAndAge(state: GameState): Record<string, unknown> {
  ...
  return {
    ...
    // P14C.4: this spread carries `cohorts` (Save V35) through unchanged, same
    // as `.records`. Both call sites below only run this world to week 34
    // (< the week-52 cohort floor), so `cohorts` is `[]` on both sides at
    // every point this comparison runs; the comparison still means what it
    // says. Re-check this note if either call site is ever extended past 52.
...
describe('C1-M1a (b) — nothing assumes eight structures or a small placement count', () => {
  it('carries twelve-plus placed facilities through invariants, completion, opex, and save', () => {
    ...
    expect(reloaded.saveVersion).toBe(37)
  ...
  it('carries far more than twelve when the property has room for them', () => {
    ...
    const json = exportSave(makeSave(operational))
    expect(exportSave(makeSave(migrateToCurrentControl(importSave(json)).state))).toBe(json)
  })
})
...
describe('C1-M1a (d) — SaveFileV13', () => {
  it('writes V13 by default and round-trips every lifecycle state byte-identically', () => {
    ...
    for (const state of states) {
      ...
      expect(exportSave(migrateToCurrentControl(importSave(json)))).toBe(json)
    }
  })

  it('validates the original property root without dropping unknown future fields', () => {
    const withFuture = {
      ...managedStudio('c1-m1a-save-projection'),
      futureV14: { mustNotLeak: true },
    }
    expect(() => makeSave(withFuture)).toThrow(/unknown field "futureV14"/)
    const save = makeSave(managedStudio('c1-m1a-save-projection'))
  ...
  it('rejects a malformed property root structurally, before any domain rule', () => {
    ...
    for (const [, mutate, expected] of cases) {
      ...
      mutate(bad)
      expect(() => validateSaveV37(bad)).toThrow(expected)
    }
  })
  ...
  it('refuses a placement standing inside an authored structure', () => {
    ...
    expect(() => validateSaveV37(forged)).toThrow(
  ...
  it('rejects unknown V38 beyond the current V37 reader boundary (stale numbers corrected post-C.2b)', () => {
    ...
    expect(() => validateSave({ ...live, saveVersion: 38 })).toThrow(
