...
import { exportCurrentState, importSave, migrateToV25 } from '../src/core/save.js'
...
describe('P13A shared commercial adoption and exact rival cash consequence', () => {
  ...
  it('replays the actual weekly commercial purchase, reconciles its debit and creates one dated operational receipt', () => {
    ...
    const restored = migrateToV25(importSave(exportCurrentState(released))).state
    ...
    expect(exportCurrentState(migrateToV25(importSave(exportCurrentState(operational))).state)).toBe(exportCurrentState(operational))
  }, 30_000)
})
