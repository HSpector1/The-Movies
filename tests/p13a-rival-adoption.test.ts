...
import { exportCurrentState, importSave, migrateToV31 } from '../src/core/save.js'
...
describe('P13A shared commercial adoption and exact rival cash consequence', () => {
  ...
  it('replays the actual weekly commercial purchase, reconciles its debit and creates one dated operational receipt', () => {
    ...
    const restored = migrateToV31(importSave(exportCurrentState(released))).state
    ...
    expect(exportCurrentState(migrateToV31(importSave(exportCurrentState(operational))).state)).toBe(exportCurrentState(operational))
  }, 120_000)
})
