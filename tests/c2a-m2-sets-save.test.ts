...
describe('C2a-M2 — sets across the save boundary', () => {
  ...
  it('refuses, at the V16 boundary, every state the sets authority forbids', () => {
    ...
    expect(() => validateSaveV29(envelope)).not.toThrow()
  ...
  it('greenlights, binds a set, and shoots — while the picture it migrated with never does', () => {
    ...
    const liveMigrated: GameState = {
      ...
      foundingRegime: 'endowed',
          ...
          // P14A.1 (Save V28): a migrated world holds no contested expiry.
    ...
    expect(() => validateSaveV29(JSON.parse(exportSave(makeSave(played))))).not.toThrow()
  })
