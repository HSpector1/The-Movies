...
import {importSave,migrateToV30,exportSave,makeSaveV18,generateWorld} from '../src/core/index.js'
...
function state(store:Store){return migrateToV30(importSave(working(store).currentSaveJson)).state}
...
describe('R05 independent named campaign transactions',()=>{
  ...
  it('imports real old current/saved checkpoints as independently recoverable records without losing original bytes',async()=>{
    ...
    for(const record of imported.records) {
      const cp=JSON.parse(record.checkpointJson),s=migrateToV30(importSave(cp.currentSaveJson)).state
