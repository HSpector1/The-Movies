...
describe('group 1: PROJECTION_VERSION 46 / schema / view market / converted law', () => {
  ...
  it('LIVE_SAVE_VERSION is 34 (stale title corrected post-C.2a) under projection 46 — no persisted fact of A.2’s own, the converted law is unchanged', () => {
...
describe('group 9: save/load', () => {
  ...
  it('the three genuine V28 T0 fixtures load byte-stable and the market page reads them as T0 pins', () => {
    ...
    if (reSavedOpen.accepted) expect((JSON.parse(reSavedOpen.saveJson) as { saveVersion: number }).saveVersion).toBe(34)
    // NOT YET EXISTING: marketPage — this test's RED cause.
    ...
    if (reSavedSettled.accepted) expect((JSON.parse(reSavedSettled.saveJson) as { saveVersion: number }).saveVersion).toBe(34)
    ...
    if (reSavedLegacy.accepted) expect((JSON.parse(reSavedLegacy.saveJson) as { saveVersion: number }).saveVersion).toBe(34)
  ...
  it('a V28 save round-trips through the bridge save/load path, and the market page reads identically on both sides', () => {
    ...
    expect(parsed.saveVersion).toBe(34)
