...
describe('group 9: save/load', () => {
  ...
  it('the three genuine V28 T0 fixtures load byte-stable and the market page reads them as T0 pins', () => {
    ...
    if (reSavedOpen.accepted) expect((JSON.parse(reSavedOpen.saveJson) as { saveVersion: number }).saveVersion).toBe(30)
    // NOT YET EXISTING: marketPage — this test's RED cause.
    ...
    if (reSavedSettled.accepted) expect((JSON.parse(reSavedSettled.saveJson) as { saveVersion: number }).saveVersion).toBe(30)
    ...
    if (reSavedLegacy.accepted) expect((JSON.parse(reSavedLegacy.saveJson) as { saveVersion: number }).saveVersion).toBe(30)
