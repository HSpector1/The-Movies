...
describe('group 1: PROJECTION_VERSION 46 / schema / view market / converted law', () => {
  ...
  it('LIVE_SAVE_VERSION is 33 (stale title corrected post-C.1) under projection 46 — no persisted fact of A.2’s own, the converted law is unchanged', () => {
...
describe('group 9: save/load', () => {
  ...
  it('the three genuine V28 T0 fixtures load byte-stable and the market page reads them as T0 pins', () => {
    ...
    if (reSavedOpen.accepted) expect((JSON.parse(reSavedOpen.saveJson) as { saveVersion: number }).saveVersion).toBe(33)
    // NOT YET EXISTING: marketPage — this test's RED cause.
    ...
    if (reSavedSettled.accepted) expect((JSON.parse(reSavedSettled.saveJson) as { saveVersion: number }).saveVersion).toBe(33)
    ...
    if (reSavedLegacy.accepted) expect((JSON.parse(reSavedLegacy.saveJson) as { saveVersion: number }).saveVersion).toBe(33)
