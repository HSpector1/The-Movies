...
import { generateWorld,tick,makeSave,exportSave,importSave,migrateToLive } from '../src/core/index.js'
...
describe('R05 shared-law rival lifecycle',()=>{
  ...
  it('commissions, staffs, funds, operates, releases and settles real films without player RNG draws',()=>{
    ...
    const reloaded=migrateToLive(importSave(exportSave(makeSave(state)))).state
