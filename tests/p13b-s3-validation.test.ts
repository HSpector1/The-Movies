...
describe('P13B-S3 validator refusals for physicalPlans (test 8)', () => {
  ...
  it('accepts the unmutated hand-authored baseline directly through validateSaveV30', () => {
    const json = exportSave(makeSave(base))
    expect(() => validateSaveV30(JSON.parse(json))).not.toThrow()
  })
