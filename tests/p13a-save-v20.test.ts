...
import { convertV19ToV20, exportSave, importSave, makeSave, makeSaveV18, migrateToV19, migrateToV20, migrateToV32, validateSaveV19, validateSaveV20 } from '../src/core/save.js'
...
describe('P13A governed V19 to V20 persistence', () => {
  ...
  it('round-trips a migrated V20 world exactly and creates independent state objects', () => {
    ...
    // The live writer emits V32 only (was V26 at authoring); a genuine V20 state reaches it through the governed lifts.
    const lifted = migrateToV32(a)
  ...
  it('validates the source before JSON detachment can erase an invalid leaf', () => {
    const save = migrateToV32(accepted())
