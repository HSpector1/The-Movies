...
import { exportSave, importSave, makeSave, migrateToV25, migrateToV30, validateSaveV19, validateSaveV30 } from '../src/core/save.js'
...
describe('P13A dated public technology history', () => {
  ...
  it('round-trips both boundaries exactly and resumes without duplicating the permanent events', () => {
    for (const week of [259, 260, 415, 416]) {
      ...
      const restored = migrateToV30(importSave(json)).state
  ...
  it('never backfills after migration, skips dates not actually reached, and leaves the frozen V19 vocabulary closed', () => {
    const migrated = migrateToV25(accepted())
    expect(milestones(migrated.state as unknown as GameState)).toEqual([]) /* P14A.1: this envelope is deliberately pinned at its own frozen version; the live-typed reader never touches the V28 root. */
  ...
  it('rejects invented dates, unknown facts, wrong significance or subjects, duplicates, and pre-recording history', () => {
    ...
    for (const change of [
      ...
      expect(() => validateSaveV30(forged)).toThrow(/technology milestone/)
    }
    ...
    expect(() => validateSaveV30(duplicated)).toThrow(/duplicate technology milestone/)
    const preRecorded = structuredClone(save)
    preRecorded.state.technology.recordingStartedWeek = 260
    expect(() => validateSaveV30(preRecorded)).toThrow(/invented technology history/)
  })
})
