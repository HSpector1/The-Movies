...
describe('Current-game Unity adoption bridge', () => {
  it('pins protocol v4/projection v32 and fingerprints named projections and exact intent fields', () => {
    expect(PROTOCOL_VERSION).toBe(4)
    expect(SNAPSHOT_VERSION).toBe(38)
    ...
    expect(BRIDGE_CONTRACT.$defs.StudioProjectionBundle.required).toEqual([
      ...
      'journeyNotices',
      'lot',
      'operationsEvents', // R3-N7-SIM-01: the read-only operating ledger joins the bundle
      'people',
      'productions',
      'release', // P06A W2: the closed Release projection joins the bundle
      'releaseResults',
      'talent', // P10A W0: the player-safe Talent projection joins the bundle
    ])
