// ── R3-N4-TEST-25 (item 4) — the r3n1-stale-schedule-take-02 command-owner
// refusal proof, run on the NEW projection-32 sibling fixture ─────────────
//
// Item 4's rule is "TS refusal tests only where an ENGINE refusal exists for
// the N4 routes (none invented)". The two shapes below are the only engine
// refusals the N4 film-journey routes actually have on this fixture, and they
// are already proved at projection 30 and 31; this file re-proves them on the
// projection-32 fixture the DATA-07 item minted, so a fixture regenerated for
// a schema bump cannot silently lose the refusal that guards its own route.
// NO new refusal is asserted, because the engine publishes none.
//
...
// a fixture minted under the projection-30 contract. This file proves
// the identical two shapes against `fixtures/r3n1-dense-02p32`
// (see `evidence/Playability-Interaction-01/entry/generate-r3n1-dense-04.ts` /
...
function loadFixtureState(): GameState {
  const path = resolve(
    'evidence/Playability-Interaction-01/fixtures/r3n1-dense-02p32/generated-r3n1-dense-02p32.checkpoint.json',
  )
  const raw = readFileSync(path, 'utf8')
  const loaded = loadCampaignLibrary(raw, limits)
  if (loaded.changed) {
    throw new Error(
      'r3n1-dense-02p32 fixture: unexpected migration/change on load — this fixture was minted at the running schema',
    )
  }
  return loaded.session.gameState
}

describe('R3-N4-TEST-25 — stale production-operation route on r3n1-dense-02p32: SERVER REFUSAL by the bridge', () => {
  it('SERVER REFUSAL — a stale expectedStateRevision on the resolveProductionBlocker intent is rejected (STALE_REVISION / state-stale), with no mutation', () => {
    ...
    const session = new BridgeSession(state, 'r3n1-stale-take-02p32-session-01')
    ...
    const advanced = submit(session, intentB!, 'r3n1-advance-unrelated-screenplay-p32')
    ...
    const stale = submit(session, intentA!, 'r3n1-stale-attempt-p32', staleRevision)
  ...
  it('SERVER REFUSAL — the OLD intentId at the CURRENT revision is INTENT_NOT_AVAILABLE once the state has moved on; the fresh resolveProductionBlocker intent for the same production is then accepted exactly once', () => {
    const state = loadFixtureState()
    const session = new BridgeSession(state, 'r3n1-stale-take-02p32-session-02')
    const snapshot = session.snapshot()
    const intentA = snapshot.availableIntents.find(
      (intent) => intent.kind === 'resolveProductionBlocker' && intent.productionId === 'prod-0015',
    )
    ...
    // family).
    const advanced = submit(session, intentB!, 'r3n1-settle-unrelated-screenplay-p32')
    expect(advanced.accepted).toBe(true)
    if (!advanced.accepted) throw new Error(advanced.message)

    const oldIntent = submit(session, intentA!, 'r3n1-old-intent-current-revision-p32')
    ...
    const acceptedOnce = submit(session, freshA!, 'r3n1-fresh-intent-accepted-once-p32', revisionForFresh)
    ...
    const repeat = submit(session, freshA!, 'r3n1-fresh-intent-accepted-once-p32', revisionForFresh)
