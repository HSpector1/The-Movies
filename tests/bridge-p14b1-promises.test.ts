...
describe('group 1: PROJECTION_VERSION / LIVE_SAVE_VERSION', () => {
  it('LIVE_SAVE_VERSION is 34 (stale title corrected post-C.2a) and the live promise surface carries projection 47 after the P14B.4 cutover (record 600 / 616)', () => {
...
describe('group 6: save/load', () => {
  it('a genuine V28 fixture (open-case-45) converts to V29 with empty firstTakes/promises, and the bridge reads it with no promise fact anywhere', () => {
    ...
    if (reSaved.accepted) expect((JSON.parse(reSaved.saveJson) as { saveVersion: number }).saveVersion).toBe(34)
  })
