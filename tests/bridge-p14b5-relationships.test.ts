...
// The FROZEN SIDE (R-VERSION class, re-expressed by the 700-T2 sweep after P14B.6, re-swept by P14B.8): projection 50
// (49 outgoing), `SCHEMA_ID` = sha256:e2d354dc… (60af24c5… outgoing), 38 prior ids, `LIVE_SAVE_VERSION` 32, the
...
describe('family 11 — projection 48 THIN (RED by value): the enum, the registry, the seven-member order, the D5 sentence on the wire', () => {
  ...
  it.each(['currentSaveJson', 'savedSaveJson'] as const)('the genuine projection-47 checkpoint takes the governed prior path and %s lands on the live save through migrateToV32', (slot) => {
    ...
    // 776-S9 (P14C.2a bridge sweep): this "expected" is the LIVE lift, not the
    // frozen V33 step — migrateToV33 STAYS above only as an existence guard.
    const expected = migrateToLive(importSave(prior[slot]))
    ...
    // 776-S9 (P14C.2a bridge sweep): the live member is V34 now, two steps further.
    if (actual.saveVersion !== LIVE_SAVE_VERSION) throw new Error('slot did not reach the live version')
