...
describe('P06A W1 — save law', () => {
  it('imports a pre-P06 Release Ready save as UNCOMMITTED (holds until committed)', () => {
    ...
    const live = migrateToV22(v15)
    expect(live.saveVersion).toBe(22)
  ...
  it('round-trips a committed V16 save byte-exactly and refuses downgrades', () => {
    ...
    const reimported = migrateToV22(importSave(exportSave(save)))
    ...
    expect(() => migrateToV15(save)).toThrow(/cannot downgrade SaveFileV22/)
  })

  it('validateSaveV22 rejects forged authority at the save boundary', () => {
    ...
    expect(() => validateSaveV22(orphan)).toThrow(/foreign identity|orphan/)
    ...
    expect(() => validateSaveV22(extraKey)).toThrow(/unknown field .surprise./)
  })
})
