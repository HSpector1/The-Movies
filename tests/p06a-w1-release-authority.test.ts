...
describe('P06A W1 — save law', () => {
  it('imports a pre-P06 Release Ready save as UNCOMMITTED (holds until committed)', () => {
    ...
    const live = migrateToV21(v15)
    expect(live.saveVersion).toBe(21)
  ...
  it('round-trips a committed V16 save byte-exactly and refuses downgrades', () => {
    ...
    const reimported = migrateToV21(importSave(exportSave(save)))
    ...
    expect(() => migrateToV15(save)).toThrow(/cannot downgrade SaveFileV21/)
  })

  it('validateSaveV21 rejects forged authority at the save boundary', () => {
    ...
    expect(() => validateSaveV21(orphan)).toThrow(/foreign identity|orphan/)
    ...
    expect(() => validateSaveV21(extraKey)).toThrow(/unknown field .surprise./)
  })
})
