...
import { importSave, migrateToV29 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV29(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
