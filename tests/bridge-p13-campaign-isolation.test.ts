...
import { importSave, migrateToV33 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV33(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
