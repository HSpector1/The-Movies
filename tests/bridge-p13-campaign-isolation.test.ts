...
import { importSave, migrateToV32 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV32(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
