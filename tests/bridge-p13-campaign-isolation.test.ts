...
import { importSave, migrateToV28 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV28(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
