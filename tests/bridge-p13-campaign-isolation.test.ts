...
import { importSave, migrateToV25 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV25(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
