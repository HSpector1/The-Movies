...
import { importSave, migrateToV22 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV22(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
