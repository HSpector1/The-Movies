...
import { importSave, migrateToV24 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV24(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
