...
import { importSave, migrateToV30 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV30(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
