...
import { importSave, migrateToV23 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV23(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
