...
import { importSave, migrateToV31 } from '../src/core/save.js'
...
function state(store: MemoryStore) {
  return migrateToV31(importSave(JSON.parse(library(store).workingCheckpointJson).currentSaveJson)).state
}
