...
describe('D-11 — determinism & live saves', () => {
  it('new games save at the live version and round-trip byte-identically', () => {
    ...
    expect(save.saveVersion).toBe(31) // P13B-S8: new games save as V27.
  ...
  it('no duplicate payroll after reload (reload ≡ continuous run)', () => {
    ...
    if (reloaded.saveVersion !== 31) throw new Error('expected V28')
    const split = advanceWeeks(migrateToV31(reloaded).state, 3)
