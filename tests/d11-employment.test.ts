...
describe('D-11 — determinism & live saves', () => {
  it('new games save at the live version and round-trip byte-identically', () => {
    ...
    expect(save.saveVersion).toBe(22) // P04A SaveFileV15: new games save as V15.
  ...
  it('no duplicate payroll after reload (reload ≡ continuous run)', () => {
    ...
    if (reloaded.saveVersion !== 22) throw new Error('expected V22')
    const split = advanceWeeks(migrateToV22(reloaded).state, 3)
