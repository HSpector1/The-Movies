...
export function reconcilePlayerPolicyCash(
  ...
  if (
    ...
    throw new Error(
      'roster-wall player policy: invalid SaveFileV14 cash-ledger checkpoint length',
    )
  }
...
function exactSave(state: GameState): {
  ...
  if (imported.saveVersion !== 18) {
    throw new Error('roster-wall player policy: entry did not import as SaveFileV18')
  }
...
function exactPlayerPolicyEntryState(result: RosterWallPlayerPolicyResult): GameState {
  ...
  if (imported.saveVersion !== 18) {
    throw new Error('roster-wall player policy evidence: entry is not SaveFileV18')
  }
