...
// exact-fidelity check now hashes a genuine SaveFileV18 round trip, so this is
...
function harvestSave(state: GameState): {
  ...
  if (imported.saveVersion !== 18) {
    throw new Error('roster-wall observatory: Week-196 import did not return SaveFileV18')
  }
  const validated = validateSaveV18(imported)
  const importedReexport = exportSave(validated)
  if (importedReexport !== entrySaveBytes) {
    throw new Error('roster-wall observatory: imported SaveFileV18 re-export changed bytes')
  }
  ...
  if (remadeReexport !== entrySaveBytes) {
    throw new Error('roster-wall observatory: re-made SaveFileV18 changed entry bytes')
  }
  const entryStateHash = stateHash(validated.state)
  if (entryStateHash !== stateHashBefore) {
    throw new Error('roster-wall observatory: SaveFileV18 replay changed entry state')
  }
  return {
    entrySave: validated,
    entrySaveBytes,
 ...
 * the exact validated/replayed Week-196 SaveFileV18 entry.
 */
