...
describe('D-17A fix-pass — a stored V5 autosave migrates on load', () => {
  it('an ENGAGED V5 session restores with the flag true and the converted banner', () => {
    ...
    // …and re-saving it writes a V13 envelope that loads back UNCONVERTED.
    saveActiveSession(loaded.state)
    expect(JSON.parse(localStorage.getItem(ACTIVE_SESSION_KEY)!).saveVersion).toBe(13) // C1-M1a: new games save as SaveFileV13.
  ...
  it('a NEVER-ENGAGED headless V5 session restores with the flag false', () => {
    ...
    // Round-trip: saved as V13, reloaded unconverted, still not engaged.
    saveActiveSession(loaded.state)
    expect(JSON.parse(localStorage.getItem(ACTIVE_SESSION_KEY)!).saveVersion).toBe(13) // C1-M1a: new games save as SaveFileV13.
    const again = loadActiveSession()
    expect(again.ok).toBe(true)
...
describe('Script Projects V1 — a stored V8 autosave migrates on load', () => {
  it('preserves operations, seeds legacy screenplay and casting state, then re-saves as V13', () => {
    ...
    expect(parsedV11.saveVersion).toBe(13)
