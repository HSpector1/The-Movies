...
type CurrentEnvelopeSave = SaveFileV31
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== 31) {
    fail(path, `must be a current V31 save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, 'must preserve the canonical V31 save bytes exactly')
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: SaveFileV31['state'] } {
  let migrated: SaveFileV31
  try {
    migrated = migrateToV31(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: SaveFileV31['state']): boolean =>
