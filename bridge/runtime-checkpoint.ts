...
type CurrentEnvelopeSave = SaveFileV30
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== 30) {
    fail(path, `must be a current V30 save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, 'must preserve the canonical V30 save bytes exactly')
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: SaveFileV30['state'] } {
  let migrated: SaveFileV30
  try {
    migrated = migrateToV30(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: SaveFileV30['state']): boolean =>
