...
export const SUPPORTED_PRIOR_PROTOCOL_4_SCHEMA_IDS: ReadonlyMap<string, string> = new Map<string, string>([
  ...
  ['sha256:204a71924bd8c2e8ae9af47591226894b3e42f62457da3cc20ed6b106ede611a', 'projection-v24'],
...
type CurrentEnvelopeSave = LiveSaveFile
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== LIVE_SAVE_VERSION) {
    fail(path, `must be a current V${LIVE_SAVE_VERSION} save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, `must preserve the canonical V${LIVE_SAVE_VERSION} save bytes exactly`)
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: LiveSaveFile['state'] } {
  let migrated: LiveSaveFile
  try {
    migrated = migrateToLive(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: LiveSaveFile['state']): boolean =>
