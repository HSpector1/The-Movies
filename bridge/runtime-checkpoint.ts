...
type CurrentEnvelopeSave = SaveFileV26
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== 26) {
    fail(path, `must be a current V26 save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, 'must preserve the canonical V26 save bytes exactly')
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: SaveFileV26['state'] } {
  let migrated: SaveFileV26
  try {
    migrated = migrateToV26(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: SaveFileV26['state']): boolean =>
