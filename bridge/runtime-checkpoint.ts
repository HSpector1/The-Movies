...
type CurrentEnvelopeSave = SaveFileV27
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== 27) {
    fail(path, `must be a current V27 save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, 'must preserve the canonical V27 save bytes exactly')
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: SaveFileV27['state'] } {
  let migrated: SaveFileV27
  try {
    migrated = migrateToV27(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: SaveFileV27['state']): boolean =>
