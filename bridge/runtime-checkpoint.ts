...
type CurrentEnvelopeSave = SaveFileV25
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== 25) {
    fail(path, `must be a current V25 save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, 'must preserve the canonical V25 save bytes exactly')
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: SaveFileV25['state'] } {
  let migrated: SaveFileV25
  try {
    migrated = migrateToV25(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: SaveFileV25['state']): boolean =>
