...
type CurrentEnvelopeSave = SaveFileV24
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== 24) {
    fail(path, `must be a current V24 save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, 'must preserve the canonical V24 save bytes exactly')
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: SaveFileV24['state'] } {
  let migrated: SaveFileV24
  try {
    migrated = migrateToV24(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: SaveFileV24['state']): boolean =>
