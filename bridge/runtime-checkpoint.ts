...
type CurrentEnvelopeSave = SaveFileV28
...
function validateCanonicalCurrentSave(
  ...
  if (imported.saveVersion !== 28) {
    fail(path, `must be a current V28 save, received V${String(imported.saveVersion)}`)
  }
  if (exportSave(imported) !== saveJson) {
    fail(path, 'must preserve the canonical V28 save bytes exactly')
  }
...
function importPriorSaveViaCanonicalChain(
  json: string,
  path: string,
): { json: string; state: SaveFileV28['state'] } {
  let migrated: SaveFileV28
  try {
    migrated = migrateToV28(importSave(json))
  } catch (error) {
...
function migratePriorProtocol4Checkpoint(
  ...
  const nativeR05Draft = (original: string, state: SaveFileV28['state']): boolean =>
