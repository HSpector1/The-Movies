...
Choose the smallest coherent change that most increases the amount of meaningful play available on
the persistent Studio Lot while preserving Engine authority, determinism, the premium Hollywood
quality bar, and accessible deep-management fallbacks.

The Owner's critical experience test is now the controlling product filter:

```text
select production in world
→ inspect its stage
→ see and click a blockage
→ assign or redirect a named person
→ watch travel
→ see authoritative work resume
...
## Closed — World-First Annex Construction Interaction V1

Contract `6cab9c9` and implementation `7a370fd` close the fixed parcel's full same-lot choice. Exact
Vacant facts, the existing App-owned start action, immediate Building repaint, weekly progress,
...
timing remain open. This world-interaction priority authorizes no macroeconomic reclassification.
