...
**AMENDMENT — D-17A, Owner ruling R3 (`docs/D-16-OWNER-RULINGS.md`, 2026-08-12). `releaseTalent` is a RECORDED EXCEPTION to the gate above and stays UNGATED.** The D-16 audit found that `applyReleaseTalent` debits the termination cost without consulting `canAfford`, and asked whether that was an oversight. It is not: the Owner ruled it **intended behavior**, and the governing language above is amended here to say so explicitly rather than leave the code contradicting the contract.

Termination is the one immediate debit that **removes a future obligation** — it buys down the remaining guaranteed salary and takes the talent off weekly payroll permanently. Gating it would trap a studio in exactly the situation the exception exists for: a studio too poor to afford the termination cost would be **unable to stop paying the salary that is making it poor**, with no loans and no bankruptcy to resolve the deadlock (§12). So `releaseTalent` may **intentionally drive current cash below zero**; the studio then recovers under §12's normal rules. Everything else in the voluntary-commitment list (signing, renewal, freelancer engagement, greenlight, marketing) remains gated exactly as written above — those all **add** obligation or exposure.

Regression coverage: `tests/d17-engagement-persistence.test.ts` ("T11 / R3 — releaseTalent is intentionally ungated"), which releases a contract from a studio with cash just above zero, asserts the action succeeds, asserts cash goes negative, and asserts the `termination` ledger entry is written for the full cost. That test is the guard against a future "consistency" cleanup silently gating this action.
