...
**Correction record (second Future Ops disposition, 2026-09-12, reviewed commit `81a4aedf`):** two bounded corrections were applied in place — (1) **estate work semantics** (§1.4, §5.3–5.6): frozen participants at greenlight do not prove completed work; the four states terminated employment / completed contribution / remaining production obligation / availability for new work are now distinguished per person and per production against the accepted filming and release gates (`productionPhases.ts`, `operations.ts`, `releaseAuthority.ts`, `hollywoodTick.ts` at `592e926`); "8 ticks + 6 run weeks = 14 weeks" is withdrawn as a proven maximum — an estate duration cap is now an explicit proposed policy; (2) **the cross-package firing reference** (§5.3, D21): the 50 % early-release charge is accepted-P12 runtime history (`HIRING_TERMINATION_FRACTION 0.5`), while the Owner-selected P14 direction is `weekly × min(remaining weeks, 26)` (P14 preparation at `8ef5246aec115cc32d01d9fb8c916e3538342dca`, rulings §3.4.1 items 1–2 and companion §3.2) — not yet implemented, and not relabelled here as shipped. Model E's public-gross proxy is now labelled explicitly as *not* net worth (§3.4). The report's corresponding lines are listed in §8.
...
- it **completes only work that needs no further person-work** (pictures past their authoritative take, and runs) **and that it can fund** (the support test in §5.4 below); anything still owing a take is suspended, never worked for free;
- it **ends under an explicit policy, not by constants**: `PRODUCTION_TICKS 8` and `THEATRICAL_WEEKS 6` `[CODE]` are the countdown when nothing holds, but the accepted gates can hold a picture indefinitely — phase entry waits for stage/set/post capacity (`operations.ts` weekly sweep, longest-waiting-first), the first Shooting week advances only after the authoritative take is scheduled and no scenery blocker stands (`operations.ts:1452-1457`), and Release Ready holds until an explicit release commitment (`releaseAuthority.ts:5-11`; `operations.ts:1467-1477`). The correction in §5.4 says which pictures an estate may finish at all; the **proposed estate duration cap** (§5.4, `[RECOMMENDATION; PROVISIONAL]` 26 weeks from the terminal event) is what bounds the estate — it is proposed policy, not a maximum proven by those two constants;
...
| D21 | 11 | Voluntary bankruptcy declaration | **E** | New optional proposal, not a prerequisite; if ever added it must not discharge guarantees more cheaply than the studio-initiated early-release charge in force (accepted 50 % rule today; P14's `weekly × min(remaining, 26)` once implemented). | — |
...
- **Model E — two ranks, one table:** beside the creative Power Rank, a separately ordered **Financial Strength rank (public proxy)** (1…N, with its own movement arrow) computed from a published, capped formula over **public** inputs only — e.g., trailing-three-year gross of disclosed releases (already public industry facts at 592e926) and the public band — never from private cash or debt. **A public-gross proxy is not net worth:** it measures public commercial scale, must be named as a proxy in the table, and must never be labelled or read as net worth, book value or studio value (those remain the own-studio exact figures and the public band of §3.5). The two ranks are never combined into a third number; Standing and Legacy are untouched. What it buys: a real comparison ("#2 creatively, #5 financially") the band cannot express. What it costs: a second ordering that reads as a wealth leaderboard at small cohorts (three rows disclose by subtraction), and a public-proxy formula that can disagree with a studio's true balance sheet (a hot leveraged studio ranks high on public grosses while privately Strained — which is why the band must still be shown beside it).
...
- **Ordinary studio-initiated early release** (solvent employer) — two references, kept distinct: (a) **accepted-P12 runtime history** `[CODE]`: `terminationCost = round(0.5 × guaranteedComp)` (`HIRING_TERMINATION_FRACTION 0.5`, `employment.ts`/`tuning.ts`), debited as a `termination` ledger row, the person a free agent that week; refused while the person holds an active screenplay task; no active-production-seat guard (`actions.ts` `applyReleaseTalent`); (b) **Owner-selected P14 direction** (`8ef5246a`, rulings §3.4.1 items 1–2; companion §3.2): `charge = weekly × min(remaining weeks, 26)`, base salary only, signing bonus excluded, already-earned unpaid obligations still payable — **not yet implemented**; existing saves keep their recorded amounts through a `legacyTerminations` migration receipt (companion §3.4). The old 50 % figure is "not law" (rulings §3.4.1 item 1) and is not described here as the selected P14 rule; the P14 rule is not described as shipped. Either way this is a solvent-employer action, paid in cash and recorded as paid — separate from bankruptcy settlement below.
...
- If voluntary bankruptcy (D21, class E) is ever added, it must not discharge a guarantee more cheaply than the studio-initiated early-release charge in force (today's 50 % rule; the P14 `weekly × min(remaining, 26)` rule once implemented), or failing becomes the cheap way to shed contracts.

### 5.4 Estate productions: what can actually be completed `[RECOMMENDATION]` (corrected in the second disposition)

**Facts at 592e926 `[CODE]`.** A rival's negative and marketing budgets are debited in full at greenlight (`hollywoodTick.ts:169`); the player's greenlight likewise debits negative + marketing + film salaries up front (`actions.ts:16`). Participants are *locked* at greenlight (`hollywoodTick.ts:165`; the player's `cast`/`directorId`/`writerId`/`craftIds`) — that fixes **who is credited and whose attributes the reception reads**, not that their work is done. The countdown maps to phases: 8 development · 7 pre-production · 6 rehearsal · 5–4 shooting · 3–2 post-production · 1 release-ready (`productionPhases.ts:36-45`), each phase holding an exact capability multiset — development-casting, soundstage, soundstage + set-scenery, post, none (`productionPhases.ts:52-60`). Three gates can hold a picture for an unbounded time: (i) **phase entry waits for capacity** (the weekly fixed-point sweep, longest-waiting-first; `operations.ts` §3.3); (ii) **the first Shooting week (tick 5 → 4) advances only after the locked director has been assigned, any scenery-load-in blocker has cleared and the authoritative take has been scheduled** (`operations.ts:1452-1457`; `assignShootingDirector` accepts only the locked `directorId`); (iii) **Release Ready (tick 1) holds until an explicit release commitment** (`releaseAuthority.ts:5-11`; `operations.ts:1467-1477`). Rivals operate all three automatically inside their weekly loop (`operateStage`, `hollywoodTick.ts:69-83`; auto-commit at `hollywoodTick.ts:225-227`); the player does them by action. A theatrical run pays for `THEATRICAL_WEEKS 6` and costs nothing further. Career growth for the locked participants is recorded at release (`hollywoodTick.ts` `growth`), not at greenlight.

**Per-person states at the settlement week — four different things:**

| State | Meaning | Rule at settlement |
|---|---|---|
| **Terminated employment** | the contract ends (`expiry` if due, `employerClosed` otherwise); wages stop; the person is a free agent for employment purposes | always, for every open contract at the closed studio |
| **Completed contribution** | the person's authoritative work for a conserved picture is already recorded: a writer's once the screenplay is locked at greenlight; a director's, cast member's or craft's once the authoritative take is `completed` (picture at tick ≤ 4) | nothing further is owed by or to the person for that picture beyond the wage claim of §5.3; the credit is fixed |
| **Remaining production obligation** | the take has **not** been completed (picture at ticks 8–5): the locked director/cast/craft still owe the take | the estate has no employment relationship and cannot compel or pay for that work — scheduling the take without them would bypass gate (ii), paying them would be invented compensation; therefore the picture is **suspended** (below), and the obligation lapses if the picture is cancelled |
| **Availability for conflicting new work** | from the settlement week every released person may be hired under the ordinary law; a suspended picture's locked participant list is **not** a claim on their time | no duplicate bookings: an acquirer may resume a suspended picture only if it holds every locked participant's employment at resume time (retention priority before release, §5.6, or a later lawful hire); a participant who has taken conflicting work makes resumption impossible → cancellation |

**Per-picture disposition at settlement:**

| Picture state | People's work remaining | Facilities the estate must still hold | Estate rule |
|---|---|---|---|
| Theatrical run in progress | none | none | **always completes**; receipts to the estate; no cost |
| Release Ready (tick 1) | none | none | the estate **commits the release itself** (the same auto-commit a rival performs) → release → run |
| Post-production (ticks 3–2) | none (take completed) | post capability — the estate's own facility, not yet sold in the handoff | completes **if the support test passes**; may wait for post capacity if several conserved pictures share one slot |
| Shooting after the take (tick 4) | none | soundstage + set until the phase exits | completes if the support test passes |
| Shooting before the take (5), rehearsal (6), pre-production (7), development (8) | **yes** — the authoritative take | soundstage / set / development-casting | **suspended** at settlement (`productionSuspended` receipt); offered in the disposal handoff; resumable only by an acquirer that holds the locked participants' employment and the capacity; otherwise **cancelled** at the handoff deadline (`projectCancelled`: the sunk budget is lost, no result is fabricated, no credits are minted, the concept persists as a cancelled fact) |
| Screenplay in development (no `productionId`) | the writer's | — | cancelled |

**Support test (funded completion, PROVISIONAL):** for each completable picture (tick ≤ 4 or a run), the estate computes weeks-to-release from the workflow's actual remaining transitions under the estate's own freed capacity (no other work competes), never from the constant 8, and the residual cost = those weeks × (`OVERHEAD_BASE` + the estate's facility/capacity Opex). Estate cash after wage priority must cover the residual cost of every picture it keeps; it keeps pictures in `productionId` order (deterministic) until funds run out and suspends the rest. No free work: the estate never operates a facility it cannot pay Opex for, never schedules a take, never hires, never invents compensation.

**Proposed estate duration cap `[RECOMMENDATION; PROVISIONAL policy — not a proven maximum]`:** the estate closes **26 weeks** after the terminal event; any conserved picture not released by then is cancelled, and a run already open completes regardless (it costs nothing and pays the estate). With nothing holding, a tick-4 picture releases in 4 weeks and its run ends 6 weeks later; capacity waits can extend that, which is exactly why the bound must be policy. The disposal handoff window (§5.6) sits inside the cap.

### 5.5 Talent release: employment vs residual project commitment `[RECOMMENDATION]`

At the settlement week a released person has (a) **no employment** (`employerClosed`/`expiry`; enters `freeAgents` under the ordinary hiring law, P14 rules when P14 exists); (b) possibly a **residual project commitment**, which is one of two different things: for a picture whose take is completed (tick ≤ 4) it is a **completed contribution** — a credit fact, not a job; it does not block hiring, it does not pay (their pay was the wage claim of §5.3), and it resolves at release; for a picture whose take is *not* completed (ticks 8–5) it is a **remaining production obligation** the estate cannot call on — it does not block hiring either, it is not a claim on the person's time, and it lapses when the suspended picture is cancelled or is re-engaged only through an acquirer that lawfully holds the person's employment (§5.4). `PersonId`, exact history, outstanding work and normal P14 availability rules are preserved; no `PersonId` is ever re-minted (`hollywoodValidation.ts` `[CODE]`).
...
**Genuinely unresolved settlement choices (labelled):** none that block P15 preparation. One is P16's: whether a *suspended* picture may be transferred to an acquirer who resumes it (well-defined only if the acquirer holds every locked participant's employment and the required capacity at resume time — §5.4; whether P16 wants it is P16's).
...
|---|---|---|---|---|
| D2 | How financial strength is shown beside the creative Power Rank | Model D (band beside the rank, never ordered) · Model E (a separately ordered Financial Strength rank from a public-gross proxy beside it — a proxy of public commercial scale, **not** net worth, and never labelled as such) | Model D; Model E if the Owner wants a comparable financial ordering | Owner (P15A.2) |
...
5. The support test's residual-cost definition (which Opex an estate owes while holding post or stage capacity for a rival whose capacity is abstract) and the 26-week cap are paper policy, not verified against a running build (no builds or runtime tests were permitted); the gate facts they rest on are cited to source lines at `592e926`.
