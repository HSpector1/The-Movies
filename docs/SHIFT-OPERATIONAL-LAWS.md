...
25. Structural pins: always name the fixture; compare across independent fresh windows over
    ...
    Week 0: 231/14/8,806,568/6 — re-pinned at C1-M6 for the visual warmth pass: +57 authored
    backlot dressing objects, +260,848 decoded bytes of world art, draws 4→6 from
    texture-unit batch overflow, actors unchanged; the 173 and 174 figures are pre-warmth
    history). The frozen 30/13/11,096,896/1 in
...
29. `INITIAL_PROPERTY` (`lot.ts:272`) is HISTORICAL migration authority, not a fresh-game
...
(`SaveFileV13` at `save.ts:267`; `makeSave` → `makeSaveV13` at `save.ts:4388`; highest
migration `convertV12ToV13` at `save.ts:5076`); browser session key =
