...
## FINAL FORECAST-CAUSALITY & PLAYER-LEARNING CLOSURE (2026-07-28)

Owner directive "D-12 FINAL FORECAST CAUSALITY AND PLAYER-LEARNING CLOSURE". Two commits; no frozen constant changed; no new RNG; critic never a box-office multiplier; no non-goal entered; SaveFileV4 schema unchanged (Phase 4 persistence deferred — see below).

**Commit 1 — forecast causality (engine).** The engaged forecast's `expectedTotal` was box office at a SINGLE noisy appeal sample (`center + one gaussian offset`, sigma largest for low-confidence films); convex box office (opening ∝ appeal^1.8, legs ∝ WAS^1.4) inflated it into an optimistic outlier (the owner's Letters ~$91.5M for a ~$11M-center film). Fix: `offset = engaged ? 0 : rawOffset` → the engaged central estimate is the deterministic CENTER; uncertainty stays in the band + realized RNG. Draw still taken (stream unchanged); non-engaged M0A byte-identical. audienceAlignedWeakExecution (200 seeds): forecast expected −$1.11M (was ~+$42M), downside −$6.34M, forecast opening/legs ≈ realized. See the study's "FORECAST CAUSALITY FIX" section.

**Commit 2 — player learning (UI / read-models).** All engine-derived; React does no engine math.
- **P3 Team Direction Preview** (`teamDirectionPreview`) — pre-greenlight, on Talent + Review steps. Uses the SAME deterministic contributor vectors + ROLE_WEIGHT-weighted centroid (incl. shape) + expressiveStrength as the autopsy's Delivered Talent Alignment, so the full-team band EQUALS the band the autopsy later shows (regression-tested). Shows expected direction (Weak/Mixed/Strong), most compatible / most opposed pair, per-axis conflicts, and a confidence qualifier; no future RNG, never presents realized performance as fact.
- **P5 Same-week standing honesty** — the standing delta is the studio-wide weekly movement (sequential clamped D-6 updates, not per-film additive). When >1 film releases the same week, the autopsy AND the weekly-releases screen now label it "Studio standing change for Week N — includes …", list the co-releases, and keep each film's own drivers separate. No film implies it caused the whole shared delta.
- **P6 Commercial result visibility** — the "In theaters" panel shows direct commitment, projected full-run Contribution, and projected ROI (repay-its-cost visible without an autopsy). "Recent releases" gains a compact multi-axis scorecard (Critic / Audience / Gross / Studio Rev / Contribution / ROI / Result) — profit, critics and audiences kept as SEPARATE truths (no blended score).
- **P7 Commercial strengths** — a profitable film always names an engine-derived commercial reason (low break-even, audience positioning, cost discipline) instead of "Nothing stood out"; never claims creative merit it didn't have.
- **P8 Create-talent UX** — age defaults to 18 (validated [18,70] at the engine boundary, unchanged); "Current standing" split into four labeled facts (Primary ability / Experience status / Market percentile / Career tier). No founding-generation change.

Independent adversarial review: 4 real findings, ALL fixed (P5 weekly-releases screen was un-disclaimed; P5 autopsy-from-release inconsistency; P3 band diverged from the autopsy — now faithful; P6 Studio-Rev "—" beside a real Contribution — now one scorecard basis). Contract audit CONFORMS (0 violations). Full suite 863 pass; root+ui typecheck clean; M0A corpus + replay + migration byte-identical; 9 Playwright pass; production build clean. **Not merged; Gate D not begun.**

**Phase 4 (actual contributor performance in the autopsy) — DEFERRED, not implemented.** It requires adding per-contributor actual-performance to the release/autopsy snapshot with a SaveFileV4-compatible optional/defaulted field + a "Unavailable for legacy release" fallback + an autopsy table column with variance labels. That is a save-schema change that should not be rushed alongside the read-model work; it is called out here as the one remaining phase for a scoped follow-up.

---

## EVENT-STOP & TEAM-GUIDANCE CLOSURE (2026-07-29)

Owner directive "D-12 EVENT-STOP AND TEAM-DIRECTION USABILITY CLOSURE". Two commits; UI/adapter only — `src/core` untouched (no rebalancing; no change to forecast/reception/box-office/standing formulas, cash, salaries, overhead, share, Marketing, Production Budget, legs, RNG); no SaveFileV4 schema change; per-contributor Actual-Performance snapshot stays DEFERRED.

**Commit 1 — event stop.** Root cause: `advanceToNextEvent` had no run-ending stop branch (a prior "beta P2" decision treated run completion as routine), so with runs in theaters and no productions it ran to the first cash<0 crossing (owner: Week 22→131; reproduced Week 9→156). Fix: detect run endings by diffing active-before→not-active-after each tick (status 'completed' OR removed) and stop with `runCompleted`, reporting every completed run's title; the tick still applies the week's payment/payroll/overhead and completes the run in canonical order (final payment + payroll/overhead applied exactly once, no empty overshoot weeks). Two runs ending the same tick → one stop, both titles. release/contract/renewal/cash<0 still stop. Engine-derived `stopMessage` shown verbatim by WeeklySummary (React never infers the reason). Safety guard `SIM_CAP` 520 weeks + `guardHit`. After the fix the reproduction stops Week 9→14 (`runCompleted`, cash +$21.49M). 8 regression tests + corrected the old beta-P2 test.

**Commit 2 — team guidance + age control.**
- **Team Direction actionable (P2):** exposed the 0–100 score behind the band (P2.1); "Change" delta vs the previous complete team, per-panel-instance, never cross-film (P2.2); `teamDirectionGuidance` evaluates every hypothetical single-role substitution against the SAME engine source as the preview/autopsy — pure, no state mutation, no RNG — surfacing the best available candidate per role, the single best swap, and whether Mixed/Strong is reachable with the current roster (P2.3/2.4/2.6); renamed "Assessment confidence" with a tooltip clarifying it is certainty of the assessment, not team quality (P2.5). The band now derives from the displayed integer score so score and band never contradict at the 40/70 boundaries (adversarial-review fix; consistency scanned across 120 teams). 8 guidance tests.
- **Create-talent age (P3):** the numeric spinner is now an accessible integer `<select>` (18–70, default 18, keyboard nav + type-to-select), preserved across steps and shown on Review; engine boundary `[18,70]` unchanged. 3 tests.

Adversarial review: 3 findings — MEDIUM band/score boundary inconsistency FIXED; two LOW (an inert alert CSS class FIXED; a header/message week-numbering that both read correctly in their own frame — the message uses the owner's resulting-week convention, left as-is). Contract audit CONFORMS (0 violations). Full suite 882 pass; root+ui typecheck clean; M0A corpus + replay + migration byte-identical; 9 Playwright pass; production build clean. Not merged; Gate D not begun.
