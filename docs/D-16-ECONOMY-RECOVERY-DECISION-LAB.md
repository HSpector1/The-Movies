...
2. **The cause is a feedback stock with no equilibrium, not the D-13 lottery.** `audienceAwareness` is the only standing channel that touches money; a competent film's gross (~0.25×bMV) sits far below the awareness-neutral gross (0.522×bMV), so awareness ratchets down ~1.5 points per release from a starting value (40) that sits exactly on the separatrix (~40–45). It never rises for any player policy in 400 full-series runs, the oracle decays too (40 → 14.6), and no positive equilibrium exists anywhere in the system. Turning the D-13 discovery draw **off** makes distress *worse* in all five policies tested — D-13 did not cause the ruin; it made it illegible.
3. **Player agency exists and is the cheapest thing to fix.** The oracle's $62.6M edge over good play decomposes (paired) into ~$31.5M of *menu breadth* and **$0.00M of hidden information** — the forecast is literally true. A one-sentence human rule ("only greenlight films whose forecast center is positive") beats default play on 99% of seeds for +$12.16M. But the UI shows that center **with the wrong sign** on exactly the packages a struggling studio can afford (fixed costs excluded: a green "+$358K Profit" upside that is a −$190K studio loss), shows two contradictory runway numbers (186wk vs 72wk on the same save), and labels 62.4% of runs "healthy" at week 104 while median awareness is 12.5 of 40.
...
5. **Two genuine engine defects bracket everything.** (a) The **engagement cliff**: `economyEngaged ≡ contracts.length > 0`, so firing the whole roster ($925,614 from Week-86, no solvency check) reverts the studio to the legacy 100%-of-gross economy — the same film swings from −$2.29M to +$4.50M, and the labelled exploit policy wins 61.2% of all-arms corpus seeds at a $142M median, beating the oracle. (b) `releaseTalent` carries no solvency gate, contradicting D-12.11's own enumeration.
...
| # | Gate | Metric (proposed) | Verdict (CURRENT) | Decisive evidence |
|---|---|---|---|---|
| G1 | Multiple viable strategies | No sane player policy wins a majority pairwise vs the field; ≥3 policies each best in ≥10% of worlds | **FAIL** | P5 beats all 13 active policies pairwise (min 56%); P1/P3/P8/P12 best in 0/1000 worlds at every horizon; doNothing tops argmax at 40.2% (A9) |
| G2 | Capital matters | Payroll/budget choices produce two-sided outcome pressure | **PARTIAL** | Payroll kills (17.7% of star-roster runs die with profitable films, A5) but the gradient is one-way: less roster is always better; overhead spread across policies $0.46M (A12) |
| G3 | Better decisions matter | Policy explains more end-cash variance (asinh) than world+draw | **FAIL** | Policy 9–25% vs world 47–70% (asinh, both designs); luck alone flips end-cash sign in 28/50 worlds; the decisive world variable (bMV, r=0.73) is invisible at decision time (A9, A18) |
| G4 | Uncertainty without dominating | Draw share of per-film contribution variance <40% for supported films; no two-atom outcomes | **FAIL** (low-support) / PASS (supported) | Draw share 43–69% for low-support tiers vs 0.9–3.7% supported; only 24.4% of full-shortfall outcomes are interior — a 0.2×/1.8× coin flip (A4) |
| G5 | Cheap films have a purpose | Cheap tier is the best choice in an identifiable, player-visible state class | **FAIL** | P1: 98% distress, −$1.05M median film; below awareness 20 its best case is loss-minimization; **not fixable balance-only** (A12 oracle tier study) |
| G6 | Premium films have a purpose | Premium is the truly-best tier in ≥10% of decision states | **FAIL** | 0% of 406 oracle decision states under CURRENT and under every awareness arm; needs bMV $148M+ against an $80M world cap; 1.25× loses to 1.0× on 89% of paired seeds with p90Δ +$0.04M — no upside tail (A9, A12) |
| G7 | Stars create tradeoffs | Fame purchase has two-sided studio-level EV somewhere | **PARTIAL/FAIL** | The tradeoff is well-formed at film level (insurance: discovery immunity 9%→52.5%, p10 +$0.70M, upside sold) but priced at 2.2× its payout — matched-budget fame edge $0.00M median (A5, A12) |
| G8 | Distress recoverable sometimes | Durable recovery (healthy at +103wk) reachable with player information from non-insolvent distress | **FAIL** (player) / PARTIAL (oracle) | Player arms: 5/0/0% from constrained/bareMinOnly/noProduction (124,200 continuations); oracle: 7.5–10.8% — unfindable, not impossible; insolvency absorbing for both, 0/4,700 arms (A7, A8, A18) |
| G9 | Waiting is not magical | Producing beats abstention for a competent studio at every horizon | **FAIL** | doNothing: +$9.73M median, 0% distress, 100% end-positive, beats 12/13 policies pairwise; waiting is also the best single action from every distress class (A8, A9) |
| G10 | Recovery is not free | Any recovery path costs time/consequence | **PASS** (today, vacuously: the only path is the exploit) | Median weeks-to-transient-recovery 6, and 27% of "recovered" runs still die; preserve this property in every future mechanic (A12) |
| G11 | Snowballing bounded | Success does not make future failure impossible; upside not purely world-determined | **FAIL** | Max 59× start, runaway runs never end negative, drawdown ≤1.4%; runaway is a world property (585/1000 worlds never run away under any policy); the current brake on snowballing is the death spiral itself (A7, A10) |
| G12 | Failure is legible | The player can see decline coming at decision time | **FAIL** (prospective) / PASS (retrospective) | State ladder lags: median 92 weeks from peak cash to first non-healthy label; 62.4% labelled healthy at wk104 at median awareness 12.5; break-even understated 27%; insolvency alert fires exactly once, ever (A16, A19) |

**Overall: the current economy fails 9 of 12 proposed gates.** The three that pass do so partially, vacuously, or only retrospectively.
...
- **The oracle gap is a menu gap.** Paired decomposition: search breadth $31.5M, hidden information $0.00M. Adding a single shape option to the scan menu is worth +$24.6M; a legal player-information rule with the wider menu beats the oracle on 63% of seeds (A9; A18-corrected magnitudes).
...
**Is recovery credible under current rules?** No. Bare-minimum film: P(profit) = 0 exactly (0/200 engine runs; best of 250 luck arms −$107K *with* a maximum draw); waiting: −$1.95M by week 208 (least bad); every legal composite ends negative except the labelled exploit (+$124.4M at 122wk). **Another bare-minimum film is technically legal and strategically irrational** — the truthful answer the recap already gestures at, now proven.
...
- **Recommended Tier-1 pair:** `AWARENESS_REACH_NEUTRAL 0.58→0.45` + `DISC_SUPPORT_EXP 1.5→2.5`. Confirmed at 1000×208 paired: default cadence −$2.18M → **+$4.32M** (98% paired win; every producing policy improves, P5 +$46M); draw share of per-film variance 0.39 → 0.07; recovery|distress 78% → 97%; terminal decline 63% → 46%; doNothing argmax share 43% → 30%. **Named cost:** runaway roughly doubles (P3 6%→17%, P5 39%→54%) and the trimmed discovery tail costs P3 ~$3.2M of median vs the awareness fix alone.
...
4. `experiment.ts` TUNING allowlist needs the 18 verified `DISC_*`/`MARKETING_*` keys; `luck.ts` "independent arms" wording should be softened to match its measured (sound) behavior; `summary.md`'s pooled per-film table over-weights high-cadence runs (sign-flips P2/P3) — the per-run metric is the correct default.
