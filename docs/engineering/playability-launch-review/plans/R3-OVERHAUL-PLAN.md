...
| **N7 3A contextual help + 4B attention/retrievable history** | new Unity help/tooltip owner + per-screen copy; attention cues from `BridgePeopleAttentionSnapshot`/`StudioHistorySnapshot`; outcomes without a retrievable source become named data dependencies (never an invented journal) | N4–N6; TS read-model audit (contract-auditor) | EditMode help-availability per screen; rendered essential routes with help only, no tutorial; "badge clears, history persists" test; TS tests for any new read-only projection | 8 / 2 | no |
...
- C4 1280×720/200 % pictures-list shortfall → accept the ≈ 10 px scroll cost, measure natively; never raise the minimum viewport or disable
  200 %.
- C5 drag routes → only the three routes with existing lawful commands; person→facility and person→picture published as named dependencies.
