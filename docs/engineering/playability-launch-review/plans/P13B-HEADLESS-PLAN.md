...
- [x] **S7-T2 Engine increment (sim-core, landed `18ab734`, 2026-09-17 ≈23:05):** catalogue `publicWindow`/`replacementLabel` data (no validator or test pinned the entry's key set), pure `src/core/technologyDisclosure.ts` (43 lines, no `GameState`); S7 19/20, S2/S5/S6/membership 174/174, typechecks clean, no save change (evidence `08-engine-*`). The one failure is test-side: the independence file's week-900 vacuity guard — MEASURED: `considerRivalSoundPurchase` admits at most one rival adoption and the base world's own tick re-mints byte-for-byte the purchase the fixture applied at 416, so the pair converges at 417 (JSON-identical at 900) while every forecast/announcement equality holds. **Adjudicated (coordinator):** the test-author re-authors case 5 with a rival-only difference the engine preserves past ticks (validator-accepted), or else asserts the measured convergence and renames the case a regression guard — the forecast assertions never weaken (evidence `09`). Sim-core's deliberate omission recorded: `validateTechnologyCatalogue` does not validate `publicWindow` (integers, `from <= to`, `announceWeek <= from`, `to >= commercialWeek`) — OPEN for a later sweep, not required by the plan.
