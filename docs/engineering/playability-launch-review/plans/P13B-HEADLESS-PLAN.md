...
- [~] **S7-T3 Bridge projection 40** (test-author dispatched ≈23:10 on `18ab734`; sim-core brief staged): test 5 RED-first (forecast rows, replacement labels on purchase/adopt/adoptions rows for both technologies, the derived announcement row with `studioId: null`, the generalised `wait-*`/`purchase-*` rows), then sim-core; pins 39 → 40; schema id recorded; backlog notes the two binding changes.
