...
- [x] **R07 tests 1–5 RED (test-author, 2026-09-17 ≈10:20; `evidence/p13b-r07-20260917/01`–`05`):** `tests/p13b-r07-{recipes,timeline,gate,controls,save-v25}.test.ts` (9 + 6 + 6 + 8 + 8 cases) all fail at module resolution of `src/core/productionSetup.ts`. Measured fixture timelines (probes): conventional ballroom on a founded studio rehearsal 11 / admission 12 → 16; lighting ballroom on the two-Lab world rehearsal 806 / admission 807 → 809 (lighting operational 795); ordinary interior on an endowed house Set rehearsal 3 / admission 4 → 5; `set-grand-ballroom` builds in 8 weeks; the plan's 820 → 824/822/821 is pinned as the arithmetic law through `setupForecast(admittedWeek, requiredUnits)`. Interpretations adopted as delegated decisions: module surface `SETUP_RECIPES`, `setupForecast`, `validateProductionSetup(state): string[]`; the action dispatches through `applyActions` and refuses by throwing; a simpler recipe on a ballroom-bound production is refused as a wrong-Set-type case (the two recipes have disjoint Set-type gates). Premise limits recorded, not proxied: no Set size class exists (CAT-011 OPEN — wrong-type only); the second-sweep-visit rule is engine-internal (one-unit-per-week and same-week-repeat-action are tested); the different-binding restart is tested on a constructed before/after pair (no pure rebind action exists); mid-setup save/reload is tested as codec round-trip until the producer exists; Save As isolation at core level (bridge-level in T3).
- [x] **R07-T1/T2 Engine increment (sim-core, landed `90e8307`, 2026-09-17 ≈11:20; closed ≈13:15 with the five R07 files 37/37 after the controls case moved to the Hollywood-mode fixture, evidence `08`):** `src/core/productionSetup.ts` (`SETUP_RECIPES` frozen, `setupRecipeById`, `setupForecast = admittedWeek + requiredUnits`, `deriveSetupProvenance`, `createProductionSetupRouteResolver`, `applySetProductionSetupRecipe` refusing by throwing, `validateProductionSetup`), `ProductionWorkflow.setup` + `planRevision`, four setup `StudioEvent` arms (tier W, windowed), the hold branch in `advanceManagedProductions` (`advanceSetupWeek`: the visit that would move 6 → 5 stamps `admittedWeek = currentTick + 1`, fixes route/provenance/requiredUnits, credits nothing and defers `enterPhase`; each later visit credits one unit guarded by `lastCreditedWeek`; credited = required stamps `completedWeek` and falls through to Shooting), binding-change rollover with `planRevision` bump and `setupRebound` in `retargetUnfilmedProduction`, `tick.ts` passes the route resolver; **Save V25** = `GameStateV24` with two widened leaves (validated then stripped to the frozen V24 shape, both operations roots incl. rival businesses — a real defect caught by `p06a-w1-release-authority` and fixed; a non-null rival `setup` refused, S8 owns symmetry), 12 downgrade guards, sentinel 26. Probe evidence: conventional 11 → admission 12 → credits 13–16 → entry 16; lighting 806 → 807 → 809 with a real adoptionId/equipmentAssetId; same-week re-selection a no-op; forged lighting route refused by the validator; mid-setup save/reload value-identical. `bridge/operations-events.ts` filters the four setup rows until T3 publishes them (recorded). R07 15/37 before adjudication. **Adjudicated (coordinator):** the 17-case selection-timing premise (selecting after the gate-week tick would hold a recipe-less picture) contradicts the plan — tests select during rehearsal; the mid-setup fixture sits on a rehearsing workflow; three solvency stagings; the gate's adoption count scoped to the player; the active-retrofit fixture is unreachable on a stage a picture stands on (P09 engagement) — reverse order or a constructed not-yet-operational adoption. **Amendments + sweep landed (`689dc42`, ≈12:55; evidence `06`, `07-sweep-*`):** the five R07 files 36/37 (selection during rehearsal; mid-setup record on a rehearsing workflow; measured solvency $5,700,000 / $5,450,000; player-scoped adoption count; the active-retrofit law pinned at `deriveSetupProvenance` because no engine-consistent state can hold a rehearsing picture on a stage whose fit-out is mid-deployment — `operationalWeek` self-heals from placements and the occupancy invariant refuses the double claim); sweep 24→25 over 68 files incl. leaf strips in `_v14Contract.ts` (`projectToV13State` AND `forgedV13From`), `p06a-w1-release-authority`, `save.test.ts`, `v14-boundary-guards`, and a FIFTH literal class the four greps miss — bare `saveVersion !== 24` / `.toBe(24)` checks (16 files); P13A 62/65 (+3 inherited), S2–S5 217/217, named bridge 52/52, all edited files 911/920 (8 campaign-library budgets + the one R07 case); typecheck root/bridge clean after the coordinator lifted the one `ui/` test literal. Remaining R07 case: controls "changed pre-Shooting sound choice mid-hold" — `setProductionTechnology` requires Hollywood mode while the fixture is the non-Hollywood `operationsStudio`; adjudicated as a fixture amendment (use the Hollywood-mode two-Lab lighting fixture), test-author dispatched. The stale `converted` comparison is pinned by the T3 bridge test and fixed in T3 against `LIVE_SAVE_VERSION`.
- [x] **R07-T3 Bridge projection 38:** test 6 `tests/bridge-p13b-r07-setup.test.ts` RED 11 failed / 2 passed at projection 37 (2026-09-17 ≈11:50; `evidence/p13b-r07-20260917/10`): version pin, `setup` snapshot per production (hold walked 11 → 12 → 16), `setup-recipe-*` rows with `productionSetupAction` and S4 disclosure, commit/revision/stale refusal, Shooting entry on the three measured fixtures, the four setup history rows on `operationsEvents`, legacy `setup: null`; the two passes are structural guards. Named wire premises adopted as delegated decisions: `setupRecipeActions` on the production row; `recipeLabel` = recipe name; `weeksRemaining` = forecast − current week during a hold; weeks driven by core `tick()` + fresh sessions (no `advanceWeek` intent on established-studio fixtures). Extra pin: `converted` on load — `load()` accepts only current saves, so `bridge/session.ts:123`'s `!== 23` reads true on EVERY real load today (an S5 consumer miss, now pinned; fix against a live-version constant in T3). **Bridge landed `06f05cf` (sim-core, ≈13:40):** projection 38, schema `sha256:baecd21e5fbfd4f13bd08b7af132d4b05fe24a40f1b38cd19d2782d9e2d0851e` (`urn:project-studio:bridge:protocol-4:projection-38`), union-fixture identity `f811b325…`; `bridge/productionSetup.ts` (new: `productionSetupSnapshot`, `setupRecipeActionSpecs` by engine dry run, `withProductionSetupRows`), `StudioProductionSetup` + `StudioSetupRecipeAction` DTOs, `setup`/`setupRecipeActions` on `StudioProductionOperationsSnapshot` (declared optional-nullable because raw `studioLotSnapshot()` rows also pass this closed schema; every SERVED row carries both), the four setup kinds on the operations-events wire with name-resolved sentences, `productionSetupAction` intent kind, `LIVE_SAVE_VERSION` exported from `save.ts` and used for `converted` in `bridge/session.ts` and the ui adapter; generated DTOs + both contract checks verified; RED file 13/13, retained bridge 61/61, schema/generator/lock 130/130, sixteen 37→38 pin bumps 88/88, typechecks clean (`evidence/p13b-r07-20260917/12-t3-*`); the stale `ui/src/saves.test.tsx` pin (21) lifted to 25 by the coordinator (15/15, `11`). Delegated readings recorded: a refused recipe row carries the engine's single thrown sentence in both `refusal` and `rejections`; an already-selected plan is published disabled (office byte-neutral precedent); `weeksRemaining` during a hold = forecast − current week, unclamped. OPEN (product/ops, not decided): `executeLoad` routes through `validateCanonicalCurrentSave`, so its "loaded and migrated" branch is unreachable via `load()` — the flag discriminates genuinely on `fromSaveJson` and the ui import banner.
- [x] **R07-T4 Matched pass on `06f05cf` (2026-09-17 11:41→13:00 UTC; `evidence/p13b-r07-20260917/20-RUN`, `20-test-core-06f05cf`, `21-test-bridge-06f05cf`, attribution `26`):** core 245/259 files · 2925/2966 tests, bridge 60/62 · 608/620, both contract checks verified; failing set = inherited 22 + 12 (campaign-library, r3n1, campaign-isolation, scientist-foundation digests, scenery) + ONE S5-R07-caused class of 19 (the V25 workflow leaves reaching frozen envelopes and historical hashes), fixed after the pass started without touching R07 law: `projectStateV14` now projects workflows positively to the V14 frozen shape (`4fab9cb`, evidence `22`), `liftV18Control` lifts the leaves (`4fab9cb`, `23` 16/16), `historicalHashState` strips them under a guard (`602134c`, `25` roster-wall 77/77); every R07 and S5 file passed (13 core files 108 cases, both bridge files) → **S5-R07 LOGIC VERIFIED · UNITY NOT VERIFIED** (S5-R07 only; not P13B completion, not Owner acceptance; A12/K4/CAT-011 OPEN; S6 must exercise cancellation/restoration against this consumer; S8 must bind rival symmetry). Backlog entry `007524c`/`c80adff`.
...
- [x] **S6 tests 1–7 RED (test-author, 2026-09-17 ≈15:50; `evidence/p13b-s6-20260917/01`–`07`):** `tests/p13b-s6-{receipts,refund-once,equipment,restoration,ordering,finance,save-v26}.test.ts` (6 + 3 + 3 + 4 + 3 + 1 + 11 cases) all fail at module resolution of `src/core/installationCancellation.ts`; harness `src/harness/p13b/s6-fixtures.ts`. Measured: calendar year boundary 303 → 1925 / 312 → 1926; sound stage capex row −675,000 at 303; cancel at 313 (10 of 12 weeks) refunds 175,000. Interpretations adopted as delegated decisions: `cancellationQuote` shape modelled on `adoptionQuote`; `constructionRefund` amount positive (demolition-refund convention); rounding toward the studio on the refund side (unit-tested on the pure function since every authored component divides evenly); `restoration-office` figure is a CANDIDATE authored by sim-core (office conversions author one component; the tests pin only offline/return behaviour). Two engine conflicts pinned ahead by reading: `occupancy.ts` must not claim the body of a `cancelled` placement; `equipmentPlan`'s unheld predicate must key on `holderAdoptionId` only. Methodological finding: an imported-but-unused binding is elided by esbuild, so a missing MODULE can still load — RED-first imports must be called (memory updated).
- [x] **S6-T1/T2 Engine increment (sim-core, landed `055a907`, 2026-09-17 ≈16:50):** `src/core/installationCancellation.ts` (`componentProgress` pure with the zero-week rule and the odd dollar on the refund side, `cancellationQuote`, pure cancel commits, `validateInstallationCancellation`, ledger note 'cancellation refund'), actions `cancelInstallation`/`cancelAdoption` (placement invariants first, quote's own sentence on refusal), `PlacementStatus 'cancelled'` + `cancellation` receipt, `constructionRefund` ledger kind (positive; `FINANCE_CATEGORIES` 'Cancelled installation capital returned'; filed under construction beside the demolition refund; the calendar-year bucket falls out of the existing periods), `RESTORATION_BLUEPRINTS` in `FACILITY_BLUEPRINTS` only (never offered by any catalogue): sound 25,000/2 w, lighting 10,000/1 w, **`restoration-office` CANDIDATE 25,000/1 w** (5 % of the conversion, the sound ratio; an office conversion authors one component so no site share exists), all `takesTargetOffline`; law hooks: `finishTechnologyWeek` skips cancelled adoptions, `adoptionPhysicalComplete`/`adoptionChainOperational` false for them, the completion-reconciliation clause exempted by the PLACEMENT's cancellation (the leaf survives the frozen strip), S5's P09 clause and `deriveSetupProvenance` needed no change (both key on operational status / holder); conflicts resolved: cancelled placements claim no body (`occupancy.ts`), the unheld predicate keys on `holderAdoptionId`, `alreadyInstalled` ignores cancelled records, a cancelled record pays no operating cost in any week (`expectedOperatingCostFromHistory`), the construction ledger-correlation invariant admits `constructionRefund`. **Save V26:** `GameStateV26`/`SaveFileV26`/`validateSaveV26`/`convertV25ToV26`/`migrateToV26`, `LIVE_SAVE_VERSION = 26`, 12 + 1 downgrade guards, sentinel 27; the two leaves validated then stripped; the third placement status and the refund kind travel down under a `'cancellation-v26'` policy through `validateSaveV25WithPolicy`/`validateSaveV24WithPolicy` (the C1-M3a demolition-refund device) so every genuine V12–V25 file still refuses both; frozen builders' positive `projectPlacementPreV26` at `projectStateV12` (throws on a real cancellation), roster-wall harness lifts/strips the null leaf — roster-wall 77/77 and frozen builders 2/2 in the same run (the R07 class did not recur). S6 28/31 (three test-side defects), S5 45/47 and R07/S3/S4 110/120 and placement 134/147 pending the 25→26 sweep; typecheck errors only under tests/ (`evidence/p13b-s6-20260917/08-engine-*`). **Adjudicated (coordinator):** (1) restart AFTER restoration — the plan's law and S4's offline claim stand; sim-core's exception letting a restoration never refuse a new installation is reverted and the refund-once case restarts after the restoration completes; (2) the strict biconditional "asset unheld iff its adoption is cancelled" is enforced; test 7's VALID forgery detaches the asset. Both adjudications LANDED (sim-core, `placement.ts` + `installationCancellation.ts`; measured: during the restoration the restart quote is `targetEngaged`/offline, one week after it completes `ok` at the full 100,000 with a new placement while the original record stays cancelled; the biconditional is asked of the HOLDER relationship, since a cancelled adoption names its asset forever and a lawful restart may hold that same asset at $0). Test fixes + the 25→26 sweep LANDED `00e8657` (six literal classes, leaf widening, `FACILITY_BLUEPRINTS` 17 → 20, downstream `makeSave` lift points, bridge-runtime-checkpoint regex; S6 31/31, S1–S5+R07 287/287, named bridge 65/65, broad batches green, typechecks clean; evidence `07-sweep-*`, `09`). Coordinator follow-up `0935d64`/`a123562`: the live `validateTechnology` exact-key list lacked `cancelledWeek` (the V26 strip hands the frozen chain the leafless shape, so only a direct validation of a live state threw; found by `p13a-technology-membership`, 4 failures) — accepted and validated when present (a week ≥ `committedWeek`, never operational), tsc-clean narrowing; evidence `11`. **T1/T2 CLOSED.**
- [x] **S6-T3 Bridge projection 39:** test 8 RED at projection 38 (`tests/bridge-p13b-s6-cancellation.test.ts`, 11/12 RED, evidence `10-bridge-cancellation-RED.txt`): version pin, `cancellationAction`, cancel rows on the three surfaces, `cancelledWeek`, the equipment disclosure, and one finding wider than the finance line — `StudioFinanceFacility.status` is a two-value enumeration, so `snapshot()` throws for any campaign holding a cancelled placement. **Adjudicated (coordinator, plan authority):** (1) the cancellation-act row on `operationsEvents` is OUT OF S6 SCOPE — the expansion defines the cancellation's history as the retained placement record with its receipt, and the bridge bullet asks for cancel rows, restoration rows and the finance refund line; the engine emits no studio event on cancellation and none is added (the natural sibling would be a `facilityCancelled` history kind beside `facilityDemolished`, which widens the frozen history validators — OPEN, owner/companion input); the case becomes a todo naming this note; the restoration's completion already flows through `constructionCompleted` and stays pinned. (2) The three-value status is published wherever installation records appear (finance facilities included); the lot view stays two-value by construction (bodies only). (3) Per-component `kind` on the wire is derived on the bridge from the S5 label classifier (`site`/`installation`/`capture`/`post`), `null` for an authored label it does not map (the office conversion's single component) — disclosed, never defaulted; the engine receipt stays label-only (no V26 widening). (4) `equipment[]` (id, technologyId, holderAdoptionId) is published on the Laboratory page beside `adoptions[]`, as the test pins it. **Landed `25298f5` (sim-core):** `bridge/cancellation.ts` (`cancellationQuoteRow`, restoration `{blueprintId, cost, weeks}` from the blueprint the engine itself would commit, gated on the engine's `restorationRequired`), rows `cancel-adoption-<adoptionId>` (Laboratory), `cancel-<projectId>` (Office running conversion; plans page for a started plan's own placement, the existing `plan-cancel-<planId>` untouched), `cancellationAction` in all three session resolvers, `StudioFinanceFacility.status` widened (the lot-view statuses stay two-value: `PlacedFacilityView` filters bodies), `cancelledWeek` + `equipment: StudioEquipmentAsset[]` on the Laboratory page, restoration completion sentence on the existing `constructionCompleted` kind, `physicalKindOrNull` exported from `technologyAdoption.ts` (no law change); schema `sha256:bdd3e666…`, pins 38→39 in 15 files; named bridge 302/302, contract checks verified, root typecheck clean (evidence `13`). **Ratified (coordinator, adjudication 5):** the quote container on action rows is ONE closed object `StudioActionQuote` (shared required `components`/`rejections`/`refusal`; adopt-only `total`/`reusedPostFacilityId`/`reusedEquipmentAssetId`, cancel-only `refund`/`restoration`, each optional per verb, nothing defaulted) replacing projection 37's `StudioAdoptionQuote` — the C# generator refuses an object union without a shared discriminator (CF08) and adding one would alter the S5 pin at `bridge-p13b-s5-adoption.test.ts:288`; no client consumes projection ≥ 32, so the rename costs nothing today and is a backlog item. One test slip refused by sim-core and confirmed against the engine (`applyCancellation` stamps the commit week; the case's session runs at that week; line 341 added two): test-author corrected it (`state.market.tick`, the commit week) plus three unused imports — bridge test 8 **11/11 + 1 todo**, `typecheck:bridge` exit 0 (evidence `14`). **T3 CLOSED.**
- [x] **S6-T4 Matched pass, records, commit, push:** backlog S6 entry written (`dc2c810`); matched pass on `83187c2` (`20-RUN` 20:02:51Z→21:06:06Z; `20-test-core-83187c2` core 258/267 files · 2984/3009 cases + 1 todo; `21-test-bridge-83187c2` contract checks verified, bridge 61/63 · 620/632 + 1 todo). Attribution (`23-attribution-83187c2`): the inherited 22 + two S6-caused TEST-SIDE failures fixed at `58bc9df` (evidence `22`: the catalog roster gains the three restoration ids — a sweep miss on an exact-id roster; the finance-history proxy byte bound 100k→110k, the report growing by exactly the new category's zero series, trajectory byte-identical). New failures attributable to S6 law: NONE. **S6 CLOSED — LOGIC VERIFIED · UNITY NOT VERIFIED** (S6 only).
...
- No private rival research exposure: the forecast never reads rival projects, seats, spend or receipts; only the campaign-clock announcement
  and the catalogue's public window. S8 must keep this invariant when rival research exists. [SUPERSEDED by the refinement: no receipt exists; the invariant is carried by the pure read-model signature, not by a receipt validator.]
...
- NO persisted announcement record. Engine facts: `IndustryReceipt` (hollywoodTypes.ts) is keyed to a studio (`studioId` with an entered owner)
  and `validateHollywood` refuses any kind outside its exact-key table as 'unknown receipt kind' — every frozen reader V19–V26 runs it (the hollywood root exists from V19), so a
...
- Catalogue data per technology (`src/core/technologyCatalogue.ts`, data not law): `publicWindow: { from, to, announceWeek }` — lighting 884..988 announced at 884 (the S7 scope record's figure — a ±52-week window around the true commercial week; no corroborating transcription exists in this plan's R07 sections or on disk; CANDIDATE, companion wording owed), sound `{ from: 416, to: 416, announceWeek: 416 }` (its commercial week is
...
- Engine read model `src/core/technologyDisclosure.ts` (pure; inputs are the catalogue entry and the week ONLY — no `GameState` rival facts):
  `technologyForecast(entry, week) → { kind: 'window', fromWeek, toWeek } | { kind: 'exact', commercialWeek, announcedWeek }` (`kind: 'exact'` whenever `week >= announceWeek` OR the window is degenerate (`from === to`); otherwise `kind: 'window'` — sound's {416, 416, 416} is therefore exact at every week including week 0, and its Laboratory disclosure never becomes a two-bound window), `technologyAnnouncements(week) →
...
- Bridge (projection 40; text plus TWO named surface changes — the nullable `studioId` and the per-technology purchase/wait rows below): Laboratory page `forecast: StudioTechnologyForecast[]` per catalogue technology {technologyId, name,
  ...
  fixed sentence 'Public milestone facts from the catalogue; not a rival schedule.'}; `replacementLabel` on the commercial purchase row and on `adopt-*` rows for both technologies and on `adoptions[]` rows — the Laboratory page's `wait-*`/`purchase-*` rows, today emitted for sound only (`bridge/laboratory.ts` ≈322–325, a P13A-era relic), are GENERALISED per catalogue entry (`wait-<technologyId>` / `purchase-<technologyId>`; the sound ids are unchanged since `SYNCHRONIZED_SOUND.id === 'synchronized-sound'`), reflecting the engine's own per-technology `waitForTechnology`/`purchaseTechnology` law (`commercialAccessRefusal` per entry) — new bridge surface, not text-only, recorded in the backlog; industry page `announcements` group gains one derived row per announced technology (`eventId` `technology-announcement-<technologyId>`, week = announceWeek, `studioId: null` — the campaign clock owns no studio, so `StudioIndustryActivity.studioId` becomes NULLABLE on the wire (a binding change, backlog) and the row is excluded from every per-studio History filter by construction while it appears in the Industry Pulse feed; headline 'Industry announces <name>', detail naming the exact commercial date) — derived on read, never from receipts. `converted`/save pins unchanged (V26).
- Tests (RED-first, unchanged in substance; test 3 restated): 1 forecast text before 884 shows 884..988 only (never 936); at/after 884 the
  exact 936; sound exact throughout. 2 rival-independence (identical wire forecast/announcement rows across two states differing only in rival facts — necessarily PARTIAL until S8 exists: today only rival adoptions, receipts and employment can be varied; the pure signature carries the rest). 3 exactly one announcement row per technology with a non-degenerate window at/after its announce week, week = announce week,
...
**Audit (contract-auditor, read-only, 2026-09-17 ≈22:20):** two blocking findings adopted above (the derived announcement row's `studioId` → nullable on the wire, excluded from per-studio history by construction; the sound-only `wait-*`/`purchase-*` rows generalised per catalogue entry and named as new bridge surface so test 4 covers both technologies), four should-fix adopted (the degenerate-window clause in `technologyForecast`'s rule, the window figure's provenance wording, V19–V26, the superseded receipt-validator sentence), and the coverage note on test 2. Confirmed sound: no persisted announcement, Save V26 unchanged, the pure `(entry, week)` signature, the synthetic `eventId`.
...
- [x] **S7-T1 Tests 1–4 RED-first (2026-09-17 ≈22:45, `8720202`)** against `src/core/technologyDisclosure.ts`: `tests/p13b-s7-{forecast,independence,announcements,replacement}.test.ts` (4 + 5 + 5 + 5 cases), each RED at module resolution (evidence `01`–`04`). Test-author's named premises: the 884/988/884 figures and both replacement sentences pinned as the plan's CANDIDATE wording; test 2 PARTIAL until S8 (only rival adoption/cash divergence exists, exercised through `considerRivalSoundPurchase`); Save As is a bridge/campaign-library concept with no engine equivalent — test 3 shows two independently imported copies of the same V26 fixture publish identical rows, the real `saveAs` slot behaviour belongs to the bridge file (T3).
- [x] **S7-T2 Engine increment (sim-core, landed `18ab734`, 2026-09-17 ≈23:05):** catalogue `publicWindow`/`replacementLabel` data (no validator or test pinned the entry's key set), pure `src/core/technologyDisclosure.ts` (43 lines, no `GameState`); S7 19/20, S2/S5/S6/membership 174/174, typechecks clean, no save change (evidence `08-engine-*`). The one failure is test-side: the independence file's week-900 vacuity guard — MEASURED: `considerRivalSoundPurchase` admits at most one rival adoption and the base world's own tick re-mints byte-for-byte the purchase the fixture applied at 416, so the pair converges at 417 (JSON-identical at 900) while every forecast/announcement equality holds. **Adjudicated (coordinator):** the test-author re-authored case 5 with a rival-only difference the engine preserves past ticks — a no-op `RivalFinancePeriod` from the production helper `newFinancePeriod` spliced at the rival's entry week (sums unchanged, the per-period technology-adoption check avoided by date; `exportCurrentState` round-trips at 416 and 900 as inline assertions), the pair stays divergent through 900 while the player roots stay equal; forecast assertions unweakened; 5/5 (evidence `09`). **T1/T2 CLOSED, S7 engine files 20/20.** Sim-core's deliberate omission recorded: `validateTechnologyCatalogue` does not validate `publicWindow` (integers, `from <= to`, `announceWeek <= from`, `to >= commercialWeek`) — OPEN for a later sweep, not required by the plan.
- [x] **S7-T3 Bridge projection 40** (test-author dispatched ≈23:10 on `18ab734`; sim-core brief staged): test 5 RED-first (forecast rows, replacement labels on purchase/adopt/adoptions rows for both technologies, the derived announcement row with `studioId: null`, the generalised `wait-*`/`purchase-*` rows), then sim-core; pins 39 → 40; schema id recorded; backlog notes the two binding changes. **Adjudicated (coordinator, 2026-09-18 ≈00:05) — the exact-week leak named by the test-author:** the engine's `commercialAccessRefusal` sentence (`technology.ts` ≈136) and the sound-style `wait-*` detail embed the exact commercial week unconditionally, so a lighting purchase row emitted at 793 would publish 936 before the 884 announcement. Law: a technology's `wait-<id>`/`purchase-<id>` rows exist on the Laboratory page ONLY while `technologyForecast(entry, week).kind === 'exact'` — sound's rows unchanged at every week (degenerate window), lighting's absent before 884 and present from 884 (wait enabled; purchase disabled with the engine sentence until 936, which is public from 884; both absent once access is held). The engine sentence itself is untouched (no client reaches it without a row); noted as a latent text leak for any future direct client — OPEN minor. Process note: the test-author's slot watcher matched its own command line (it contained the literal `npx vitest run`), the same self-match class recorded on 2026-09-17; stopped by the coordinator. **Test 5 RED landed:** `tests/bridge-p13b-s7-disclosure.test.ts` 16 failed / 6 passed (22) at projection 39 — absent `forecast[]`, absent lighting wait/purchase rows at 884+, absent announcement row, absent `replacementLabel`, version pin (evidence `10`); the six passes are the sound-row regression pins and the pre-announcement absences. **Landed `6622062` (sim-core):** `PROJECTION_VERSION = 40`, schema `sha256:3283d0fd…` (C# `21f097ea…`; F10/F11 `52f80612…`), `StudioTechnologyForecast` + `forecast[]` on the Laboratory page (catalogue order, `campaignDate` labels, fixed basis), the `wait-<id>`/`purchase-<id>` loop per catalogue entry gated on `technologyForecast(...).kind === 'exact'`, `replacementLabel` optional on action rows / required on `StudioAdoptionRow`, the derived announcement row (`studioId: null`, nullable on the wire; exempt from the 13-week pulse window and disclosed in the pulse notice; excluded from per-studio History by the existing filter), no `session.ts` change (the laboratory resolver is already generic), pins 39→40 in 17 files incl. the S6 RED file; RED file 22/22, named bridge groups green, contract checks verified, typechecks clean (evidence `13`). CANDIDATE wording recorded (companion owed): purchase label `Purchase <technologyId> access` (sound's pinned form), wait label `Wait for commercial <name>`, announcement detail `Commercial access opens <date>. Public milestone facts from the catalogue; not a rival schedule.` **Correction (coordinator):** sim-core had made the access-held gate uniform (rows hidden once access is held), which changes sound's prior wire (published-and-disabled with `This studio already has synchronized-sound access.`); the plan mandates no such change and the "absent once held" clause was the brief's, not the plan's — withdrawn. Law: rows exist while the forecast is exact; `enabled`/`disabledReason` from the engine dry run. Gate removed `c08d368` (evidence `14`: disclosure 21/22 pending the amendment, laboratory/labs/adoption 28/28, contract unchanged); test amended to published-disabled with `This studio already has lighting-control-01 access.` — **22/22 (evidence `15`). T3 CLOSED.**
- [x] **S7-T4 Matched pass, attribution, label, records, commit, push:** backlog S7 entry (`12a9420`); matched pass on `bf794bd` (`20-RUN` 23:04:43Z→00:10:51Z; core 265/272 files · 3028/3051 + 1 todo; bridge contract checks verified, 62/64 · 641/654 + 1 todo). Attribution (`22-attribution-bf794bd`): the inherited 22/12 only; S7-caused failures NONE; no source changed after the pass started. **S7 CLOSED — LOGIC VERIFIED · UNITY NOT VERIFIED** (S7 only).
...
sound production choice; rival ledger kinds are the TEN `RivalMoneyKind`s `capacity | signing | payroll | overhead | facilityOpex | development | production | marketing | studioRevenue | technologyAdoption` (the first five already reconcile per period in `hollywoodValidation.ts` — pre-existing surfaces, not S8's); rivals have no
...
**Refinement (coordinator, 2026-09-18 ≈00:45, before S8 begins; engine facts verified; supersedes the provisional bullets where they differ; VERIFY against the companion when re-supplied).**
- Engine facts. The technology root is SHARED and already studio-keyed: `ResearchProject.studioId`, `TechnologyAccess.studioId`, adoptions per studio; but the LAW around it is player-wired today (audit B1/B3/B4): `validateTechnologyRoot` refuses any project whose `studioId` is not the player's and resolves Laboratories/employment from the player's roots; `applyTechnologyAction`/`changeProject` derive `own` internally and read `state.operations`/`state.contracts`; `advanceResearchWeek` aggregates EVERY project's spend into the player's `cash`/`ledger` (tick.ts ≈528); `deriveSetupProvenance` resolves the player's adoption only (B5); `rivalCapacityOpex` THROWS on a Laboratory facility (B2) — each is a named S8 source change below; `finishTechnologyWeek` treats a rival adoption as a clock (`committedWeek + deploymentWeeks`). Rivals are `RivalBusiness {account: RivalAccount {cash, periods: RivalFinancePeriod[]}, operations: StudioOperations, policy {…, reserveWeeks}, nextDecisionWeek}` driven by `advanceHollywoodWeek` (`staff()` hires `RIVAL_TEAM_ROLES` through the shared employment law with receipts; `decide()` commissions films). `RivalFinancePeriod.movements` is validated with `exact(p.movements, moneyKinds)` and a per-period `technologyAdoption === −access − adoption` reconciliation; `moveRivalMoney` opens a new period per calendar year. The placement root (`StudioPlacement`) has NO studio key — the lot is the player's; rivals own no placements. The physical-plan root IS per studio (`PhysicalPlan.studioId`), admission player-only today. `rivalWeeklyOperatingCost` already includes `rivalCapacityOpex(business)`; whether it is BOOKED weekly to the account as `facilityOpex` is verified at T2 (if it is, the interval-Opex obligation reduces to counting Lab/instrument opex in `rivalCapacityOpex`; if not, S8 books it per interval as the plan says — no back-charges before the migration week). Precedent for plant without a lot: S5's rival adoption is a PAPER commitment (aggregated component rows, one held asset, no placement).
- Save **V27** (allocated here; T0 fixtures are the V26 set minted at `8796fce`): `RivalMoneyKind` gains `researchSpend`, `researchCapacity`, `technologyRestoration`, `technologyRefund` (the movement record widens, every period carries the four keys at 0 on lift); `IndustryReceipt` gains `laboratoryCommitted {planId, facilityId}`, `laboratoryOperational {facilityId}`, `instrumentOperational {facilityId, technologyId}` (rival module presence is derived from receipts — no new field on `StudioFacility`), `researchSeatAssigned {projectId, talentId}`, `researchCompleted {projectId}` (rival-only emitters; the player's own facts stay in the player roots and history); rivals' Laboratories/instruments are `StudioOperations` facilities appended on plan completion (abstract plant, the S5 precedent), each traceable to an admitted plan + receipt; downgrade to V26 is refused when any rival research fact exists (a NEW positive projection `projectHollywoodPreV27` in save.ts — the first hollywood-root narrowing — modelled on the `projectPlacementPreV26` PATTERN: throws on any rival research fact, strips the four zero movement keys otherwise) and lossless otherwise (four zero movement keys stripped, no rival receipts of the new kinds).
- Rival Lab capacity: `physicalPlans` admission generalised to any studio through a RIVAL ADMISSION step in `advanceHollywoodWeek` (the rival queues a `research-laboratory` plan, then instrument plans per technology of interest, only when `account.cash ≥ quote + rivalWeeklyOperatingCost × reserveWeeks`; ≤ 2 Labs × 4 seats); capital booked as `researchCapacity` at admission; the facility appended at the plan's completion week with `laboratoryCommitted`/`laboratoryOperational` receipts; no placement record.
- Rival research: the rival policy is DATA (`RIVAL_RESEARCH_POLICY` in the catalogue module; CANDIDATE, OPEN with the companion): per technology an `interestFromWeek` (= `researchableWeek`), `seats` (≤ 4), `budgetPerWeek` (≤ the S2 usable ceiling), and the rule "a rival researches a technology only while it holds no access and no player prototype exists"; Scientists hired through `staff()` with a scientist slot per seat (the shared employment law, `employment` receipts); projects created/advanced through the SAME `technology.ts` law (a `StudioContext` resolver — player: `state.operations`/`state.contracts`/`state.studio.cash`; rival: `business.operations`/hollywood employment/`business.account` — threaded through the research internals; the player intent boundary derives `own` once and delegates; see the audit block); `researchSpend` movements equal the project's `expenditure` deltas per period (validator reconciliation, the `technologyAdoption` pattern).
- Rival adoption per technology (S5) unchanged in shape: paper commitment; the first-prototype entitlement and inventor route apply when the rival's own project completes (`researchProjectId` on its access); `technologyRestoration`/`technologyRefund` movements exist for the S6 kinds but no rival cancellation POLICY is authored (OPEN) — the kinds are validated at 0 unless a receipt exists.
- S7 invariant: `technologyForecast`/`technologyAnnouncements` are unchanged (pure); the industry page publishes rival research NEVER (no research rows, no seat counts, no spend) — only `technologyAdopted` receipts (existing) and the campaign-clock announcement; test 5 upgrades S7's partial rival-independence to a genuine one (two campaigns differing in rival RESEARCH state publish identical forecast/announcement rows).
- S5-R07 symmetry (test 6): a rival production on a rival stage with an operational rival lighting adoption takes the lighting route through `deriveSetupProvenance` with rival provenance — through the SAME functions; no shortcut; measured units 4 → 2.
- Validator: the player invariants (access identity, component sums, asset holders, plan admission) applied per studio; "no rival authority without a receipt" for every rival Lab, seat, adoption and admitted plan.
- Bridge (projection **41**, text only): industry page rival technology facts only from receipts (adoption operational; `laboratoryOperational` as 'expands its research capacity' WITHOUT counts); rival finance only through the existing public standings; Laboratory page unchanged; a `converted` pin move only.
- OPEN (recorded, not resolved): every policy number above (seats, budget, interest timing), the rival cancellation policy, the four movement kinds' player-facing labels, the acceptance wording — companion text owed.

**Audit (contract-auditor, read-only, 2026-09-18 ≈01:10) — six blocking findings and seven should-fix ADOPTED; the refinement's sentences above were corrected in place and the following are S8 law:**
1. **Architecture (B6, decided):** the RESEARCH law (seats, weeks, receipts, validator) is generalised IN PLACE per studio through a `StudioContext` resolver (`{studioId, facilities, employment, charge(kind, amount, week)}`; player → operations/contracts/`state.studio.cash` + ledger; rival → `business.operations`/hollywood employment/`moveRivalMoney`), because the scope requires "identical receipts for identical inputs vs a player run"; rival ADOPTION keeps S5's parallel paper function (`considerRivalSoundPurchase`, generalised per technology) because a rival has no placements. This departs from S5's parallel-function precedent deliberately, for research only.
2. **Validator (B1):** `validateTechnologyRoot` iterates studios (player + every entered rival), resolving Laboratories/employment through the context; project identity `<studioId>:research:<technologyId>` per studio.
3. **Research advance (B4/S3):** `advanceResearchWeek` returns entries partitioned by `studioId`; tick.ts applies only the player's slice to `cash`/`ledger`; the rival slices are booked per business as `researchSpend` inside the rival step of `advanceHollywoodWeek` (test 2 pins the player's cash/ledger byte-identical with and without rival research).
4. **Actions (B3):** `applyTechnologyAction`/`changeProject` derive `own` at the intent boundary and delegate to context-parameterised internals (`assignSeat`, `beginProject`, `releaseSeat`, `researchAfterEmploymentRelease`); rival research uses those internals from the policy step, never the player intent path.
5. **Setup provenance (B5):** `deriveSetupProvenance(state, recipe, stageFacilityId, week, studioId = playerStudioId(state))` — parameter added, both existing call sites unchanged; test 6 is authored against the parameterised signature (a prerequisite source change, RED at the missing parameter's effect, not at resolution).
6. **Capacity opex (B2):** `rivalCapacityOpex` gains `laboratory` (`TUNING.RESEARCH_LABORATORY_WEEKLY_OPERATING_COST`) and instrument opex per `instrumentOperational` receipt (`TUNING.ELECTRICAL_CONTROL_INSTRUMENTS_WEEKLY_OPERATING_COST`, per technology's authored module cost); the existing weekly `facilityOpex` booking (hollywoodTick.ts ≈289) and the validator reconciliation (hollywoodValidation.ts ≈249) thereby cover interval Opex — no separate mechanism; grandfathering = no back-charges (opex starts at the Lab's operational week).
7. **Rival admission (S1):** a new `admitRivalPlans(state)` in `physicalPlans.ts` reusing `planAdmissionView`/`commitPhysicalPlan` internals per rival business, called inside `advanceHollywoodWeek` AFTER `staff()` and BEFORE `decide()`; sequential-after-step-1.06 accepted (the rival's week is the player's week).
8. **Rival Scientists (S2):** `staff()` gains a business-specific extra role list (`'scientist'` × open seats demanded by the rival policy's active projects) threaded beside the fixed `RIVAL_TEAM_ROLES`; the same contract length/renewal law; a seat released by contract end through the generalised `researchAfterEmploymentRelease`.
9. **Abstract Laboratory (S4):** the appended rival facility is `{capability: 'laboratory', capacity: 4, name, id}` mirroring the player's blueprint; instruments are receipts (`instrumentOperational`), the seat law's module check resolved per studio (player: placement installation; rival: receipt).
10. **`researchCapacity` reconciliation (coverage 4):** per period equals −Σ `approvedQuote.total` of the rival's plans admitted in the period (receipt-driven, the `technologyAdoption` pattern).
11. **Rival adoption timing (coverage 3, adjudicated):** `finishTechnologyWeek`'s clock branch is UNCHANGED for rivals (no placements → no P09 body query); the scope's "P09 clause on rival bodies" is satisfied by the rival's stage facility existing and the receipt; recorded, not extended.
12. **Downgrade refusal (S6, test 7):** V27→V26 throws when any of the four new movement kinds is non-zero in any period or any of the five new receipt kinds exists; lossless otherwise.
13. **Bridge (test 8):** T3 verifies every receipt-kind switch/narrowing in `bridge/` and `ui/` accepts the five new kinds (audit could not rule out an exhaustive switch).
Coverage additions: test 1 pins that the tick and the validator do not throw once a rival Laboratory exists; test 2 pins the player's cash/ledger isolation; test 7 pins the downgrade-refusal conditions.
...
- [x] **S8-T1 Tests 1–7 RED-first (2026-09-18 ≈01:55, `1a79a37`)** against `src/core/rivalResearch.ts`: `tests/p13b-s8-{capacity,research,adoption,finance,nonleak,setup-symmetry,save-v27}.test.ts` (5 + 4 + 4 + 5 + 5 + 4 + 7 = 34 cases), each RED at module resolution (evidence `01`–`07`). **Adjudicated (coordinator):** (a) `rivalResearch.ts` exports `RIVAL_RESEARCH_POLICY`, `admitRivalPlans` (re-exported if implemented in `physicalPlans.ts`) and `advanceRivalResearchWeek(state, business, project)` — the tests' name/signature is the rival research step's export; (b) rivals own no placements, so `lightingAdoption`/`adoptionChainOperational`/`hasOperationalFacilityInstallation` resolve physical completion PER STUDIO (rival: the adoption's clock with its held asset) — an S8-T2 source change beyond law item 5; the test-author's raw-placement workaround in the setup-symmetry fixture is being replaced by a placement-less rival chain (evidence `09`); (c) policy rows `{technologyId, interestFromWeek, seats, budgetPerWeek}` bounded as the plan states.
- [x] **S8-T2 Engine increment (sim-core, landed `c609e0a`, 2026-09-18 ≈04:15):** Save **V27** (`RivalMoneyKind` 10 → 14; five rival receipt kinds; `projectHollywoodPreV27`; a `'research-v27'` policy threaded V27→V19 so the two V27 facts travel down as V26's did; `convertV18ToV19` strips the four minted zero keys; 14 downgrade guards; sentinel 28), `StudioContext` through the research internals (`assignSeat`/`beginProject` extracted; `applyTechnologyAction` derives `own` once; `validateTechnologyRoot` per studio with the receipt requirements; `advanceResearchWeek` player-only, `advanceRivalResearchWeek(state, business, project)`), `src/core/rivalResearch.ts` (policy rows at the bounds' maxima; `admitRivalPlans`; plan completion appends the abstract Laboratory with receipts; rival research and admission after `staff()` and before `decide()`; scientist slots in `staff()`; `finishHollywoodWeek` releases seats per expired rival contract), `rivalCapacityOpex(business, receipts)` pricing Laboratories and instrument receipts (no back-charges), `deriveSetupProvenance(…, studioId)` with the per-studio chain check, `considerRivalSoundPurchase` per technology and route; sweep 26→27 over 90 test files (classes A–H listed in evidence `08`); S8 24/34, S1–S7/R07/p13a/roster-wall/sweep batches all green, typecheck errors only in the S8 tests (11) and the bridge checkpoint path (2). **Ratified as CANDIDATE law (coordinator; OPEN for the companion):** one inventor per technology industry-wide (a rival researches only while no other studio holds research-route access or a live project — otherwise every rival invents ~140 weeks early and the commercial route dies, breaking `p13a-rival-adoption`); Labs admitted on cash/`reserveWeeks` alone from week 0 (the plan's stated rule; an interest-window gate is a companion call); `StudioContext` without a `charge` member (tick.ts owns the player's cash application); rival plan ids from the rival's own sequence (player plan ids never shift); one adoption per (rival, technology). **Adjudicated test-side (six premises, test-author revising; evidence `11`):** the three pre-S8 precondition pins inverted; downgrade cases assert through `migrateToV26`; the capacity forge made validator-consistent; the migration-basis case compares a lifted V26 period to its source; the identical-receipts fixture rebuilt on a rival Lab (double-employment defect); the expiry case constructs a non-renewable contract — LANDED `d2ce838`: **S8 35/35** (renewal and expiry split into two cases), root typecheck 0 errors (evidence `11`). **T1/T2 CLOSED on the engine side.** Bridge/ui live-version sweep 26→27 LANDED `742fb1e` (runtime-checkpoint, session, campaign-library, ui adapter; bridge pins in 5 files; the save-as A11 claim scoped to the player's own plans since rivals now admit Labs in the shared root; five stale ui live-writer pins 21/23/25→27; typechecks clean; checkpoint-recovery 11/11, save-as 1/1, ui pin files 45/45; evidence `12`). **T2 CLOSED.**
- [x] **S8-T3 Bridge projection 41** (test-author dispatched 2026-09-18 ≈05:20 on `742fb1e`; sim-core brief staged): test 8 RED-first LANDED `f08fbd8` (`tests/bridge-p13b-s8-rivals.test.ts`, 17 cases: 2 RED — the version pin and the missing `laboratoryOperational` capacity-expansion activity — 15 already true as regression guards: no research/seat/spend disclosure, rival finance behind the standings, the player's pages unchanged, `snapshot()` on a naturally simulated state carrying all five receipt kinds, Save As and the V26 fixtures; every fixture a NATURAL campaign, seed `p13b-s8-bridge-probe-01`, since every bridge read runs the full validator chain; evidence `10`). Test-author findings recorded: the rival `researchSpend` money kind shares its literal with the player's pre-existing `MoneyKind` (the finance page legitimately carries the string; the test pins the player's channel at $0 instead of a substring ban); `converted` remains not wire-observable for a genuine cross-version load (the R07 OPEN note). **Projection 41 LANDED `87aac7d` (sim-core):** `PROJECTION_VERSION = 41` (no DTO change; schema `sha256:16b84322…`, C# `be1f42ea…`), `bridge/industry.ts` publishes one `studios` activity per rival `laboratoryOperational` receipt (`<studio> expands its research capacity`; detail without any count) and nothing for the four other rival receipt kinds; rival finance stays behind the standings; pins 40 → 41 in 18 files; RED file 17/17, regression 59/59 + 1 todo, schema/lock 139/139, contract checks verified, root typecheck clean (evidence `13`). Two test-side leftovers handed to the test-author (evidence `14`): an unused Save As helper in the S8 bridge file (`typecheck:bridge` TS6133) and S3's plans case 8, whose forged rival plan id `…:r01:plan:1` now duplicates a GENUINE S8 rival plan minted by `admitRivalPlans` (the case's premise "no path grows a rival plan" is falsified) — both cleared: the helper now asserts the campaign-library listing after each Save As; case 8 rebuilt on the genuine rival plan in the fixture — 17/17 + 11/11, `typecheck:bridge` exit 0 (evidence `14`). **T3 CLOSED.**
- [x] **S8-T4 Matched pass, attribution, label, records, commit, push:** backlog S8 entry (`59876c0`); matched pass on `ef9ff76` (`20-RUN` 04:09:20Z→05:19:33Z; core 271/280 files · 3079/3103 + 1 todo; bridge contract checks verified, 61/65 · 657/671 + 1 todo; logs `679f400`). Attribution (`23-attribution-ef9ff76`): the inherited 21/11 + two P13A bridge test premises falsified by the ratified one-inventor law (the first non-player adoption and the first shared access/adoption rows are now a rival's research-route rows) — fixed test-side, 8/8 (evidence `22`); S8-caused law failures NONE; no source changed after the pass started. **S8 CLOSED — LOGIC VERIFIED · UNITY NOT VERIFIED** (S8 only). **With S1–S8 closed the eight P13B obligations are LOGIC VERIFIED · UNITY NOT VERIFIED** — not Owner acceptance, not Unity/native/visual verification; the companion re-supply remains owed for S6/S7/S8 acceptance wording.
