...
## CURRENT RESUME — read this first (updated 2026-09-25 22:44 CEST; supersedes every "Current …" section below)
...
- **Step now:** C.4 T0 DONE and published at `95b981eb`: genuine V34 corpus `tests/fixtures/p14/genuine-v34-c4-corpus/` (6 worlds, records 790/791); 6,240-week baseline 792 (both arms fail the youth clause from year 10; the C.2a arm empties the listing at week 2548; pre-existing F-792-1: every rival insolvent from week 260 in a passive world); R3 REPLACED by deficit + youth floor (782 §7, reviewed 782-A2 REFINE, acted on in §8); API contract 793; V35 sweep inventory 794. Throwing scaffold published at `bd27de93`. RUNNING: independent RED (test-author, isolated worktree `/Users/zacheryspector/The-Movies-c4-red-scaffold`, record 795) and the sole writer (sim-core, main tree, record 796). NEXT: copy the RED into the main tree, test-side V35 sweep per 794, source review, prediction, matched pass, the 6,240-week demonstration with the pass condition fixed in 782 §7.5.
...
- **Carried qualifications:** FU-1 `ui` reliability unreturned; FU-2 prepared-reuse timeout 4/6; 6,240-week endurance never run; no runtime cost measured; 30 deliberate C.1 failures and 7 C.2a changed-scenario failures are coverage debt, not verified behaviour.
