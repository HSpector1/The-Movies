// Installed from frozen244 after parent full read and independent243 corrections; original inert provenance retained.
...
import * as identityModule from '../src/core/productionIdentity.js'
import * as employmentModule from '../src/core/employment.js'
...
function observed(value: ReadyOwnerReplayInput) {
  ...
  const persisted = vi.spyOn(identityModule, 'persistedProductionIds')
  const allocate = vi.spyOn(identityModule, 'allocateProductionId')
  const freelancers = vi.spyOn(employmentModule, 'freelancerMarketIds')
  try {
    const result = replayReadyProductionPlans(value)
    console.log('264_READY_BUDGET_DIAGNOSTIC', JSON.stringify({
      plans: value.plans.map((plan) => plan.traceKey), preparationWork: result.preparationWork,
      omissions: result.omissions, attempts: result.attempts.map((attempt) => attempt.kind === 'cut'
        ? { kind: attempt.kind, reason: attempt.reason, through: attempt.through, provenance: attempt.provenance }
        : { kind: attempt.kind, traceKey: attempt.trace.traceKey, paths: attempt.trace.paths.length, provenance: attempt.provenance }),
      calls, ownerCalls: { header: header.mock.calls.length, staffing: staffing.mock.calls.length,
        add: add.mock.calls.length, link: link.mock.calls.length,
        persisted: persisted.mock.calls.length, allocate: allocate.mock.calls.length, freelancers: freelancers.mock.calls.length },
      sourceDimensions: { now: value.source.market.tick, originals: value.source.studio.activeProductions.length,
        facilities: value.source.operations.facilities.map((row) => ({ id: row.id, capability: row.capability, capacity: row.capacity })),
        workflows: value.source.operations.workflows.map((row) => ({ id: row.productionId, phase: row.phase,
          reservations: row.reservations.length, task: row.shootingTask !== null, boundSet: row.bindings.setId !== null })),
        talent: value.source.talent.length, contracts: value.source.contracts.length,
        concepts: value.source.concepts.length, employment: value.source.hollywood?.employment.length ?? 0,
        talentIdWidth: Math.max(0, ...value.source.talent.map((row) => row.id.length)),
        contractIdWidth: Math.max(0, ...value.source.contracts.map((row) => row.talentId.length)),
        conceptIdWidth: Math.max(0, ...value.source.concepts.map((row) => row.id.length)),
        employmentPersonWidth: Math.max(0, ...(value.source.hollywood?.employment.map((row) => row.terms.talentId.length) ?? [])),
        employmentStudioWidth: Math.max(0, ...(value.source.hollywood?.employment.map((row) => row.studioId.length) ?? [])) }
    }))
    ...
    persisted.mockRestore(); allocate.mockRestore(); freelancers.mockRestore()
