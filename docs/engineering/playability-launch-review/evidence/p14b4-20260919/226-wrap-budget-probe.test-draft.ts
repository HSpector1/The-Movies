...
function observed<P extends StartedPicture>(value: StartedOwnerReplayInput<P>) {
  ...
  try {
    ...
    if (value.source.studio.activeProductions.length === 2 && value.horizonEndWeek === 6) {
      console.log('226-REPLAY-DIAGNOSTIC ' + JSON.stringify({ sourceWeek: value.source.market.tick,
        horizon: value.horizonEndWeek, work: result.preparationWork, calls, external,
        fixedHolds: result.fixedHolds.length, omissions: result.omissions, attempts: result.attempts }))
    }
