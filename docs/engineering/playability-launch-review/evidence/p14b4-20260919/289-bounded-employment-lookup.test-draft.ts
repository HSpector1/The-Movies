// INERT / UNEXECUTED. Intended tests/p14b4-bounded-employment-lookup.test.ts.
...
describe('P14B4 rivalEmployment — ordinary owner semantics', () => {
  ...
  it('keeps inclusive starts, exclusive contractual ends, exclusive endedWeek and null-ended contracts', () => {
    const f = probe(), input = detached(f.hollywood, f.rows)
    for (const [week, expected] of [[9, null], [10, f.expiry], [61, f.expiry], [62, null]]
      satisfies Array<[number, IndustryEmployment | null]>) check(input, 'probe-boundary', week, expected)
    for (const [week, expected] of [[9, null], [10, f.earlyEnd], [29, f.earlyEnd], [30, null], [114, null]]
      satisfies Array<[number, IndustryEmployment | null]>) check(input, 'probe-ended', week, expected)
  })
})
