...
const producer = dir + '944-c3-t0-producer.ts', producerSha256 = sha(readFileSync(producer))
const recorderSha256 = sha(readFileSync(fileURLToPath(import.meta.url)))
if (producerSha256 !== '133507f0628cc8e4662b35dcfa1a890236f520346bde3f1acf692c85898b8156') throw new Error('Producer differs from frozen author handback')
...
writeFileSync(base + '.json', JSON.stringify(report, null, 2) + '\n')
console.log(JSON.stringify({ name: '945-c3-t0-mint', exitCode: child.status, fixedExistingSource, exactDeclaredOutputs, outputs: outputFiles.length, start: report.start, end: report.end }))
