// 947: diagnostic copy of failed944; same simulation and exact byte-equality oracle.
// Only bounded mismatch reporting and producer identity differ. Parent owns execution.
// Default: prepare/validate in memory and print manifest. --write: create new corpus only
// after EVERY premise, save, runtime and source-stability check has succeeded.
...
const PRODUCER = 'docs/engineering/playability-launch-review/evidence/p14b4-20260919/947-c3-t0-producer.ts'
...
if (runtimeCurrentSaveJson !== currentSaveJson) {
  const saved207 = JSON.parse(savedSaveJson) as { state: unknown }
  let firstByteOffset = 0
  while (firstByteOffset < runtimeCurrentSaveJson.length && firstByteOffset < currentSaveJson.length
    && runtimeCurrentSaveJson[firstByteOffset] === currentSaveJson[firstByteOffset]) firstByteOffset++
  console.error(JSON.stringify({
    diagnostic: '947 exact runtime-vs-continuous mismatch; original equality still fails',
    ...
    bytes: { left: Buffer.byteLength(runtimeCurrentSaveJson), right: Buffer.byteLength(currentSaveJson),
      leftSha256: sha(runtimeCurrentSaveJson), rightSha256: sha(currentSaveJson), firstDifferentStringOffset: firstByteOffset },
    parsed208: differences(JSON.parse(runtimeCurrentSaveJson), JSON.parse(currentSaveJson)),
    ...
    pre207MemoryToBridgeImport: differences(pre207InMemory, bridgePre207),
    newlyReleasedPlayerFilms: {
      continuous: state.studio.releasedFilms.filter(film => film.releaseTick === 208).map(film => film.productionId),
      bridge: session.gameState.studio.releasedFilms.filter(film => film.releaseTick === 208).map(film => film.productionId),
    },
    sourcePathDifference: 'Bridge adapter.advanceWeek uses develop:true; producer.step uses default develop:false. This is a source fact, not mismatch attribution.',
  }, null, 2))
  throw new Error('947: public advance and continuous tick differ; bounded structural diagnostics above; no artifacts written')
}
...
assert.equal(checkpoint.currentSaveJson, currentSaveJson)
assert.notEqual(savedSaveJson, currentSaveJson)
assert.equal(checkpoint.savedStateDigest, sha(savedSaveJson))
assert.equal(checkpoint.currentStateDigest, sha(currentSaveJson))
...
const manifest = {
  ...
  description: 'Public Full Custom authoring followed by three actual released films, natural acting announcement/retirement, and genuine public runtime advance/replay. No C.3 transition law exists in these outgoing bytes.',
