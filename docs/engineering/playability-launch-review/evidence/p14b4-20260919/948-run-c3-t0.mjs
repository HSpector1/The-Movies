...
const base = dir + '948-c3-t0-diagnostic'
...
const expectedOutputs = ['MANIFEST.json', 'genuine-v37-c3-created-week0.json.gz',
  ...
  'runtime52-current208-saved207.json.gz'].map(name => output + name).sort()
...
const producer = dir + '947-c3-t0-producer.ts', producerSha256 = sha(readFileSync(producer))
const recorderSha256 = sha(readFileSync(fileURLToPath(import.meta.url)))
if (producerSha256 !== '938615b2c2a74cca8f025d21f586cae6ef107f8fc87c6fcc4ae9c2200a2f5a5f') throw new Error('Producer differs from frozen author handback')
...
writeFileSync(base + '.json', JSON.stringify(report, null, 2) + '\n')
console.log(JSON.stringify({ name: '948-c3-t0-diagnostic', exitCode: child.status, fixedExistingSource, exactDeclaredOutputs, outputs: outputFiles.length, start: report.start, end: report.end }))
