...
| `contract.actions.renewAvailable` / `renewReason` | `contractActionDecisions` (`bridge/contract.ts`) over `renewalWindowOpen` (`employment.ts:206`) | D | PUBLIC | no | the exact reason when closed: "renewal window is not open yet — it opens N weeks from now (Week X), 12 weeks before the contract ends in Week Y" | REQ-018 (R1, projection 19) |
| `contract.actions.renewalTerms[]` (`termWeeks`, `termLabel`, `annualSalary`, `weeklySalary`, `signingBonus`, `endWeekExclusive`) | `contractOfferOptions` → `contractOffer` (`:257`) — the engine's own offers THIS week, one per `CONTRACT_TERM_OPTIONS` | D | PUBLIC | no | "Proposed terms — nothing is renewed by viewing"; the client picks a published term, never prices | REQ-018 (R1, projection 19) |
| `contract.actions.releaseAvailable` / `releaseReason` | `contractActionDecisions` over `activeScriptWriterAssignments` (the D-11.9 screenplay-task guard) | D | PUBLIC | no | "<name> is Drafting · <title> and must finish that screenplay task first" | REQ-018 (R1, projection 19) |
| **contract quote** (`quoteContract` → `StudioContractQuoteSnapshot`: `ok`, `refusal`, `signingBonus`/`terminationCost`, `cost`, `cashBefore/After`, `consequence`) | `contractDraftToEngine` + `contractQuoteSnapshot` (`bridge/contract.ts`) over `contractOffer`, `terminationCost` (`:183`), `guaranteedComp` (`:177`), `canAfford` (D-12) | D | PUBLIC (a quote mutates nothing; only a legal quote mints the ONE digest-bound commit intent) | no | the consequence sentence verbatim; a refused preview carries the engine's reason + remedy; no morale/loyalty/reputation effect is implied | REQ-018 (R1, projection 19) |
