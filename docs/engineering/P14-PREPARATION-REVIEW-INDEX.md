...
3. Read the **companion §2.1** for the redefined P14A (the symmetric premium tier, the rival trigger, case-aware admission, the terminal settlement step and the thin Core surface are Revision-2 additions), then **§3** for the firing note (the charge table, the E1 exploit the cap opens and the floor that closes it, the versioned rule for old saves, the E6 term lean that is surfaced and not retuned), **§4** for the promise catalogue and the feasibility rule, **§5** for the relationship model (evidence → translation → modernization), **§6** for the lifecycle model.
...
5. Read the **companion §9** for what the ten reviews found (122 findings), what changed, what was not applied and why, and the disclosed limit of the adversarial verification; then **§10** for the provisional ceiling and reserve.
...
**New in this package (recommendations, not law).** A symmetric, bounded premium tier on proposals (the accepted actions carry no salary field); the rival proposal trigger; case-aware admission by interval algebra with the P10 commit-from-draft primitive and two bounded `staff()` edits; the salary-expectation guard derived from the accepted ledger by the successor-row test; the busy-set release refusal; the versioned termination rule discriminated by the recording boundary; incumbent renewal as a settled proposal for a person under a case, with its refusal and confirmation branch; the disclosure table (existence, issuer, term public; money UNKNOWN; reasons ordering-only); the settlement as the terminal step of the weekly advance; a thin Core surface on the profile; the promise catalogue's qualifying event at the first completed take, with a new durable receipt; the feasibility classification rule and its worked examples under the real eight-week countdown; the nine-tier ladder with evidence labels, decay-to-baseline on read and a compaction rule; chemistry as a bounded read-only projection; aging materialized into the accepted leaf from provenance; the `retirement_announced` state, the term cap and the one-extension orchestration; the transition catalogue's eligibility inputs from `careerIdentity`; the lifecycle eligibility predicate the markets consult; cohorts and all minting through one exported primitive; the four-class register; the provisional ceiling.
...
**Provisional P14A execution ceiling.** 76–88 productive lead hours with a 35–40% protected reserve inside that total; 102–118 elapsed; refreshed after P13 acceptance (companion §10).
