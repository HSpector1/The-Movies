...
The remaining implementation work is additive and bounded. P03A intentionally ended at `Development work is complete — continue at Casting.` and implemented no P04 behavior. P03A.1/P03A.2 added the Development Rail, exact-ID Locate, writer joins, and conditional memo ownership. Owner-accepted P03A.3 now adds the reusable camera-pose origin trail, Back affordance, and world-layer Escape grammar that P04 must preserve rather than duplicate. It still does not add retained UI Toolkit routing, Casting context/focus/scroll restoration, controller navigation, a contained scroll owner, or Casting ownership. P04A remains the first major UI Toolkit retained workspace over the preserved GameObject/URP/Cinemachine lot and sealed IMGUI surfaces.
...
| P03A.3 Owner-accepted TypeScript seal | `0d9fc65765e5c55ed221688238293babfcaea93e` |
| P03A.3 Owner-accepted Unity seal | `062a881afe9d38352b86b260afbe18ee070a3deb` |
...
### P03A.3 Owner-accepted changed-path delta

TypeScript P03A.2 → P03A.3 (`d4ed07d` → `0d9fc65`) changes only `docs/campaigns/LIVING-LOT.md`. Core, bridge, projection, schema, quote, generated DTO, save, browser, and gameplay production code remain byte-unchanged. Protocol v4, projection v9, and schema `sha256:80f2f0fcd14d1b25e713c2624286a6c05a98c53ea5cfcb2b47612f8c030f5e47` remain exact.

Unity P03A.2 → P03A.3 (`bae3c4e` → `062a881`) changes exactly 22 paths:

- camera/navigation: `StudioCameraDirector.cs`, `StudioCameraInput.cs`, `StudioSelectionManager.cs`, `TycoonCameraController.cs`, `StudioCameraProofRunner.cs`, and `StudioCameraPresentationTests.cs`;
- new camera proof: `StudioCameraRecoveryTests.cs` plus `.meta`;
- legacy UI scaling: new `StudioLegacyUiMetrics.cs` plus `.meta`, and scaled `StudioBridgeClient.cs`, `StudioDevelopmentCardHud.cs`, `StudioFoundingBeaconHud.cs`, `StudioFoundingCardHud.cs`, `StudioHud.cs`, `StudioLivingTimeHud.cs`, and `StudioProductionRailHud.cs`;
- hierarchy/proof: `StudioDevelopmentJourneyProofRunner.cs`, new `StudioLegacyUiMetricsTests.cs` plus `.meta`, `StudioLivingTimeTests.cs`, and `Tools/p03a-run-development-proof.sh`.

No Input System action asset, UI Toolkit/UXML/USS/PanelSettings, bridge DTO, scene, camera rig, renderer, ProjectSettings, portrait/capture, Addressables, or TypeScript production path changed. Commits are `f9a13de` (camera recovery/wheel normalization), `bd8d261` plus meta `09f7915` (distance-scaled traversal/tests), `21b7ac7` (legacy scaling), and `062a881` (Development hierarchy only).

#### Navigation origin: REUSE / EXTEND / DO NOT REBUILD

| Classification | Exact P03A.3 seam | P04A instruction |
|---|---|---|
| **REUSE** | `StudioNavigationOrigin` in `TycoonCameraController.cs` | Camera TARGET pose token: `Pivot`, `Distance`, `Yaw`, `Pitch`. It intentionally stores no selection or workspace state. |
| **REUSE** | internal `StudioNavigationOriginTrail` | LIFO, bounded `Capacity = 8`; ninth push drops the oldest; empty pop fails; `Clear` invalidates all. Do not create another camera-pose stack. |
| **REUSE** | `TycoonCameraController.NavigationOriginAvailable`, `PushNavigationOrigin`, `TryRestoreNavigationOrigin`, `ClearNavigationOrigins` | Push immediately before explicit Focus; restore targets through normal smoothing. Home/SnapHome and deliberate manual camera intent invalidate stale origins. |
| **REUSE** | `StudioSelectionManager.Select` + `StudioLocateAction.Locate` | Exact player Focus that cannot enter an inspection pushes an origin, then calls `FocusOn`; selection-only pushes nothing. Inspection moves a separate Cinemachine camera and needs no management-pose origin. |
| **REUSE** | `StudioSelectionManager.NextCancelStep`; `StudioCancelStep`; `StudioCameraDirector.LastInspectionExitFrame` | Current world order is one layer per Esc: `ExitInspection` → `Deselect` → `RestoreOrigin` → `None`. Director execution order `-120` plus the frame marker prevents one Esc also deselecting. |
| **REUSE** | `StudioCameraDirector.InspectionControlVisible` and Back button | `◄ BACK TO STUDIO` appears for inspection or any available origin; inspection Back calls `ExitInspection(false)`, otherwise it calls `TryRestoreNavigationOrigin`. |
| **EXTEND** | no workspace route/restoration event exists | Add UI Toolkit workspace context beside the camera trail: exact Casting project/role/candidate/compare layer, scroll and focus identity, modal state, semantic owner, and opener. Associate it with explicit Locate without copying camera pose. |
| **EXTEND** | current Back calls are embedded in `StudioCameraDirector.OnGUI` and `StudioSelectionManager.HandleCancel` | Add one shared coordination/notification seam so UI Toolkit Back/Esc can peel retained UI first and restore/suspend/reopen Casting around the existing camera pop. Preserve current world fallback order. |
| **DO NOT REBUILD** | `StudioNavigationOriginTrail`, `StudioNavigationOrigin`, `FocusOn` capture path | No P04 camera token, parallel pose stack, title/ID-derived pose, Home-as-Back, or direct camera teleport. |

Manual invalidation is exact. `TycoonCameraController.Update` clears the trail for Home, keyboard movement, zoom, orbit, and edge pan; `SnapHome` also clears it. `FocusOn` itself does not push: the accepted player path is `StudioLocateAction` → `StudioSelectionManager.Select(..., true)` → `PushNavigationOrigin` → `FocusOn`. Founding's authored direct `FocusOn` remains intentionally outside the trail. P04 must use the player path for Locate and must never push from authority refresh, polling, or automatic route opening.

The remaining P04 navigation requirement is presentation context, not camera geometry. A Casting context token may remember exact UI identities and a relationship to the current navigation depth/action, but camera pose remains exclusively in `TycoonCameraController`. Since no restore event or generic Back dispatcher exists, Fable should make the smallest additive coordination seam rather than introspect the internal trail or duplicate it. Opening Casting does not move the camera; explicit Locate may suspend or retain the workspace, calls the accepted Locate path, and exact Back restores the camera then the retained Casting context.

#### Input and accepted camera behavior

P03A.3 fixes one calibration bug, not the input architecture. `StudioCameraInput.WheelNotchZoomUnits = 120f` normalizes `Mouse.current.scroll.ReadValue().y` to the zoom units already used by both management and inspection cameras. `StudioCameraInput` still directly polls `Keyboard.current`, `Mouse.current`, and `Touchscreen.current`; `Assets/InputSystem_Actions.inputactions` is unchanged and still unused by these runtime paths. No gamepad path, project-owned Global/World/Camera/UI maps, UI Toolkit navigation, centralized context arbitration, or retained-UI capture signal landed. Wave 0 still owns those gaps, with `StudioCameraInput.Current` retained as the compatible one-frame facade while its source moves behind actions.

Accepted camera preservation boundary:

- `TycoonCameraController.PanSpeedFor`: `distance * 0.78`, clamped `14..120`; close precision and faster management traversal are both intentional;
- `StudioCameraInput.WheelNotchZoomUnits = 120f`; reverify platform wheel units before a Windows build, but do not remove the accepted macOS normalization;
- `TycoonCameraController` and `StudioCameraDirector` use `Time.unscaledDeltaTime`, so pause/1×/2×/4× never alter navigation feel;
- `PushNavigationOrigin` captures the target, not the mid-smoothing pose; `TryRestoreNavigationOrigin` restores through smoothing, giving stable repeated cycles;
- manual camera input invalidates stale origins, while Locate/Focus leaves normal management input enabled;
- `StudioCameraDirector.InspectionControlVisible` and its accepted `190×50` `◄ BACK TO STUDIO` affordance stand for inspection or an available origin and scale through the legacy law;
- Home remains canonical overview and clears the trail; it is not Back.

#### Legacy UI scaling: preserve, do not import into UI Toolkit

`StudioLegacyUiMetrics` is the accepted single scaling law for surviving IMGUI: reference `1720×1045`, `ScaleFor = clamp(min(width/1720,height/1045),1,2.6)`, true-size `ScaledFont`, and base/screen `Scale`/`Unscale`. It is byte-identity at `1720×1045`, `1440×900`, and `1280×800`; Owner fullscreen `3456×2234` yields approximately `2.0093`. `WorkflowPanelInset = 18` and `WorkflowPanelMaximumWidth = 400` are now shared by the memo and `StudioHud`.

Preserve this law in the changed legacy surfaces: memo (`StudioBridgeClient`), selection receipt (`StudioHud`), Living Time, Back, Production Rail, founding beacon/card, and Development card. Preserve the P03A.3 Development hierarchy—title leads; phase/decision supports; writer/room/assessment are quieter; capacity is meta—without treating its IMGUI renderer as a Casting component.

Do **not** import `StudioLegacyUiMetrics`, its reference viewport, `GUIStyle`, base-space rect conversion, or its 2.6 cap into UI Toolkit. P04's retained host owns responsive classes, panel scaling, USS typography/spacing/hit-area tokens, focus/navigation, safe areas, and contained scroll. The legacy law is a compatibility boundary for legacy IMGUI only, not Project: Studio's universal UI architecture.
...
- “P03 may provide an origin/Locate stack” → P03A.3 now provides the accepted camera-pose trail and world Back/Esc primitive, but still no retained-workspace route/context stack;
...
- “no exact Locate API/origin landed” → `StudioLocateAction.Locate(stableId, focus)` now flows through `StudioSelectionManager` into the accepted `StudioNavigationOriginTrail`; it captures camera pose for Focus but no Casting UI context;
...
| Exact explicit Locate | `StudioLocateAction.Locate(string stableId, bool focus)`; `StudioSelectionManager.Select`; `LastLocatedStableId` proof | Use as the exact stable-ID dispatch. `focus=false` selects only; `focus=true` makes the camera promise and now pushes the accepted camera origin when management Focus moves. | It is not a retained workspace route, Casting context token, focus/scroll restore, availability join, or authority-refresh hook. Never call it from snapshot apply/`Update`. |
...
4. one retained-workspace context token recording prior route, selected stable ID, exact project/role/candidate/comparison/modal state, scroll/focus identity, and opener; camera pose stays in P03A.3's `StudioNavigationOriginTrail`;
...
6. one additive Back coordination seam that composes the retained-workspace token with existing `StudioLocateAction`, `StudioSelectionManager`, `StudioCameraDirector`, and `TycoonCameraController`; it reuses the P03A.3 camera trail, adds no pose stack, keeps Locate explicit, never moves on authority refresh, and never substitutes Home for Back.

Likely new paths—names may follow repository convention, but ownership must stay this narrow—are `Assets/Studio/Runtime/Presentation/UI/StudioManagementWorkspaceHost.cs`, `Assets/Studio/Runtime/Presentation/Input/StudioPresentationInputContext.cs`, one workspace-context/Back coordinator (not a camera-origin implementation), one `PanelSettings` asset, one shared USS token sheet, and Casting UXML/USS/controller files. Existing `StudioCameraInput.Current` may remain the one-frame camera facade while its device source moves behind actions. No Casting component may poll `Keyboard.current`, `Mouse.current`, `Touchscreen.current`, or `Gamepad.current` directly, and no Casting-specific screen-rectangle check may be appended to `StudioCameraInput`.
...
| ESC behavior | `StudioSelectionManager.NextCancelStep`; `StudioCameraDirector.LastInspectionExitFrame` | REUSE + EXTEND | Preserve one-layer world order (inspection, selection, camera origin). UI Toolkit must consume nested Casting/modal Back first, then fall through without clearing the draft. |
...
| Explicit stable-ID Locate | `StudioLocateAction.Locate(stableId, focus)` | REUSE AS-IS | It already routes player Focus through selection and P03A.3 camera-origin capture. Wrap only to suspend/restore retained Casting context. |
| Camera origin/Back | `StudioNavigationOrigin`, `StudioNavigationOriginTrail`, `TycoonCameraController.PushNavigationOrigin/TryRestoreNavigationOrigin`, `StudioCameraDirector` Back | REUSE AS-IS | Accepted camera-pose primitive; do not duplicate, serialize, or make UI Toolkit own its pose. |
| Retained workspace context/Back coordination | none landed | ADD SHARED PRESENTATION SEAM | P03A.3 restores camera only; P04 still needs route/layer/project/role/candidate/compare/scroll/focus/modal restoration and one coordinator with the existing trail. |
...
| Legacy fullscreen scale | `StudioLegacyUiMetrics` | PRESERVE LEGACY ONLY | Keep surviving IMGUI readable and byte-pinned at reference viewports; do not use it as UI Toolkit responsive/token architecture. |
...
| Back/context/scroll | P03A.3 camera trail + P03 card + Package 02 laws | retained UI context composed with existing camera Back + contained Casting scroll | `StudioCameraRecoveryTests`; Development recovery proof | nested UI Back restores project/role/candidate/compare/scroll/focus; camera pop remains P03A.3 exact; controller parity. |
| Explicit Locate | `StudioLocateAction.Locate`; `StudioSelectionManager.Select`; `StudioNavigationOriginTrail` | reuse player dispatch; workspace coordinator stores UI context only | Rail Locate test + `PersonFocus_CapturesAnOriginAndNeverTakesTheCameraInput` | refresh never moves; failed join honest; zoom after Locate; repeated/chained Back; no second pose stack. |
...
| Retained UI/input | accepted audit `8110820d`; Input System 1.19; direct-polling camera facade with corrected wheel units | bounded UI Toolkit host + action contexts + central capture/focus | camera recovery proves normalization only; no retained-host proof | host rollback, mouse/keyboard/controller, modal/world suppression, responsive contained scroll, no direct Casting polling. |
...
Remaining true blockers: **none**. The reconnaissance is **READY FOR FABLE**; the UI technology decision is closed. The first implementation prerequisites are: (1) land the bounded UI Toolkit host and action contexts, plus retained-workspace context coordination around the already-accepted P03A.3 camera trail; (2) repair or qualify/omit facility-Opex-dependent burn/runway before presenting it as complete; (3) add expiry subject identity if recoverable queued-Greenlight UX ships in the same slice; and (4) provide one contained scroll owner. These are planned implementation seams, not blockers or reasons to redesign Package 04.
...
| Package 03 final Owner-accepted inputs | TS `0d9fc65765e5c55ed221688238293babfcaea93e` (ledger-only; production remains `2ddf080`); Unity `062a881afe9d38352b86b260afbe18ee070a3deb` | P03A.3 narrow changed-path delta inspected directly; Development closed. |
...
| Select/open Casting | `FirstFilmJourneyNext.site = 'casting'`; Ready `development.board.projects[].projectId`; browser uses deep Casting routes and retained lot workspaces | Unity selection/camera owns place. P03A.3 supplies exact Locate plus an accepted camera-pose origin trail/Back; retained workspace context is absent. | Open the UI Toolkit Casting route by exact project ID; compose its retained context with existing Locate/camera restoration for optional owner/person Locate. | Do not add another selection/camera/pose stack, mutate the Development Rail into Casting UI, or claim camera restoration also restores UI context. |
...
| `StudioSelectionManager.cs` | Semantic person-first picking, single selection, double-activation focus/inspection, pre-Focus origin push, and one-layer `NextCancelStep`. | REUSE + MINIMAL COORDINATION ONLY. Do not add workspace-specific raycasts or replace its world-layer order. |
...
| `StudioInspectionTarget.cs`, `StudioCameraDirector.cs`, `TycoonCameraController.cs` | Management vs inspection cameras, input ownership, visible Back, workflow-panel restoration, bounded camera-origin trail, manual invalidation, distance-scaled traversal, unscaled smoothing. | REUSE/PRESERVE. Opening Casting must not automatically inspect/move; P04 stores no parallel camera pose. |
...
No Unity Casting screen, candidate card, role assignment workspace, comparison surface, Fit explanation, Greenlight preview, or exact Casting building inspector exists at the P03A.3 seal. The current `StudioBridgeClient` memo can list auto-generated Greenlight intent buttons and a generic “choose cast” heading, while `startAuditions` and `acknowledgeAuditions` remain ordinary memo verbs. The Development Production Rail and camera recovery add navigation only; neither surface contains the data or interaction model approved for P04A.

The sealed P03 surface is IMGUI/scene-authoring and has no P04 `.prefab`, `.uxml`, or `.uss`. `StudioLegacyUiMetrics` now scales those surviving IMGUI surfaces only. `StudioCameraInput.IsPointerOverUi` remains the rectangle/EventSystem pointer seam, and direct device polling remains. The accepted audit separately requires P04's UI Toolkit host and centralized action/context layer. There is no generic building beacon: current Casting spectacle is ambient visibility plus selected-building status.

### Back and context restoration

P03A.3 now makes camera Back exact for both existing cases: `StudioCameraDirector.ExitInspection(false)` returns from inspection without moving the management camera, while `TycoonCameraController.TryRestoreNavigationOrigin` pops the most recent pre-Focus target pose. `StudioSelectionManager.NextCancelStep` peels exactly one world layer per Esc: inspection, selection, camera origin. CP10A `StudioFoundingCardHud.BackToCompact` and sealed P03 `StudioDevelopmentCardHud.BackToDepartment` still reverse their own presentation layers. P04A must add retained UI context and coordinate it with these primitives, not replace them:

```text
workspace context = selected building ID + project ID + role/candidate/compare/modal layer + scroll/focus state
camera origin = existing StudioNavigationOriginTrail only
Back = retained UI layer first, then accepted inspection/selection/camera fallback as context requires
Locate = explicit StudioLocateAction; camera trail captures pose, workspace keeps only presentation context
authority refresh = retain origin if all exact IDs still join; otherwise fail neutral to Casting root
```

Do not wire Back to `SnapHome`, copy a camera pose into the workspace token, or refocus the camera unless the player explicitly requests Locate/Focus. Selection changes remain governed by the accepted one-layer grammar.

`TycoonCameraController` now exposes the complete camera primitive P04 needs: `PushNavigationOrigin`, `TryRestoreNavigationOrigin`, `ClearNavigationOrigins`, and `NavigationOriginAvailable`; `StudioSelectionManager.Select` already invokes capture before player Focus. What remains absent is a selection/restoration event or generic Back dispatcher that can reopen a retained workspace, plus the workspace route/layer/scroll/focus token itself. Extend that coordination boundary minimally; do not add Casting-local camera or pose state.
...
- `SelectableEntity` and `StudioSelectionManager`: reuse the exact person/selection/Focus path for physically present people; touch the manager only if Wave 0 needs one minimal Back-consumed/restored coordination seam.
...
P04A already has exact live-object lookup/select/focus through `StudioLocateAction.Locate`, and P03A.3 now captures/restores the management-camera origin on that player Focus path. Reuse both behind the retained workspace-context coordinator. If Casting needs a read-only “is this exact body present?” join, expose that narrowly from the existing `StudioBridgePresentation` identity binder; do not create a candidate body dictionary or camera pose in the workspace.
...
| `StudioCameraPresentationTests.cs` — `Director_ChangesOnlyCinemachinePriorityAndManagementInputMode`, `InspectionControlRect_IsCompactAndAnchoredInsideTheSafeArea`, receipt/hint geometry | PRESERVE updated 190×50 Back, inspection-aware receipt, Cinemachine/input laws; EXTEND only for retained-workspace avoidance/coordination. |
| `StudioCameraRecoveryTests.cs` — `NavigationOriginTrail_PushesPopsLifoCapsDepthAndClears`, `CancelStep_PeelsExactlyOneLayerTopmostFirst`, `Controller_RoundTripsOriginsAndHomeStalesThem`, `PersonFocus_CapturesAnOriginAndNeverTakesTheCameraInput`, `PanSpeed_ScalesWithViewDistanceKeepingClosePrecision`, `CameraSource_KeepsUnscaledTimeAndNormalizesWheelNotches` | PRESERVE ALL. ADD P04 composition tests proving UI layers precede/fall through to this exact world/camera grammar without a second pose stack. |
| `StudioDevelopmentJourneyProofRunner.RunCameraRecoveryChapter` — `cameraBackAvailableAfterLocate`, `cameraBackControlVisibleAfterLocate`, `cameraInputUnchangedByLocate`, `cameraBackRestoresExactPose`, `cameraRepeatedCyclesStable`, `cameraChainedBackUnwindsInOrder` | PRESERVE accepted end-to-end Locate→Back, five repeated cycles, chained writer→building LIFO, input-stays-enabled, and exact-pose proof. Add a real zoom-after-Locate P04 regression because P03A.3 records that runtime wheel proof is still source/arithmetic-level. |
| `StudioLegacyUiMetricsTests.cs` — `ScaleLaw_IsIdentityAtAndBelowTheReferenceViewports`, `ScaleLaw_RestoresPhysicalParityAtTheOwnersFullscreen`, `RectHelpers_RoundTripAndAreIdentityAtScaleOne`, `WorkflowEnvelope_HasExactlyOneSourceOfTruth`, `ScaledSurfaces_AreIdentityAtScaleOneAndGrowProportionally` | PRESERVE for legacy IMGUI. Add independent UI Toolkit token/responsive proofs; never make Casting assertions depend on this metrics class. |
| `Assets/InputSystem_Actions.inputactions`; `StudioCameraInput.cs` | P03A.3 changed wheel normalization only. ADD focused action-map/context tests for mouse, keyboard, gamepad, focus restore, modal suppression, UI capture, and no direct device polling in Casting components. |
...
ADD NEW UNITY TESTS for the runtime UI Toolkit host/PanelSettings lifetime, retained role-first workspace, one contained `ListView`/`ScrollView` owner, responsive candidate dossier/comparison, workspace-context composition with the existing origin trail, controller/keyboard/mouse Back parity, zoom after Locate, exact draft preservation/revalidation, memo-owner fallback, armed Greenlight confirmation, queued-vs-formed receipt, and multi-project isolation. No current Unity test proves those retained-UI behaviors.
...
### Wave 0 — Minimum retained UI/input/workspace-context foundation

- **Purpose:** establish only the accepted infrastructure needed to host Casting, reusing P03A.3 camera recovery and changing no simulation or existing IMGUI product surface.
- **Exact sequence:** (0.1) freeze P03A.3 camera/legacy regression tests; (0.2) add one runtime UI Toolkit `UIDocument` host, `PanelSettings`, token USS, and empty Casting route; (0.3) define Global/World/Camera/UI actions and one input-context service while keeping `StudioCameraInput.Current` as the compatibility facade; (0.4) add one retained-workspace context token for route/layer/exact IDs/scroll/focus/modal state; (0.5) add the smallest Back/restore notification or dispatcher that composes that token with existing `StudioSelectionManager.NextCancelStep`, `StudioCameraDirector`, and `TycoonCameraController.TryRestoreNavigationOrigin`; (0.6) prove mouse/keyboard/controller, explicit Locate, repeated Back, rollback, and legacy coexistence.
- **Likely files/symbols:** `Assets/InputSystem_Actions.inputactions`; additive action adapter around `StudioCameraInput`; minimal coordination touch to `StudioSelectionManager`/`StudioCameraDirector` only if needed to expose a generic consumed/restored event; reuse `TycoonCameraController.NavigationOriginAvailable/PushNavigationOrigin/TryRestoreNavigationOrigin/ClearNavigationOrigins` and `StudioLocateAction` unchanged; `StudioBridgeBootstrap` only for host installation; new runtime `UIDocument` host, one `PanelSettings`, shared token USS, `StudioPresentationInputContext`-equivalent service, and workspace-context/Back coordinator. No new camera-origin token/trail file.
- **Ownership:** Unity presentation infrastructure; Fable owns host/input/workspace-context integration. P03A.3 owns camera pose/trail behavior. No TypeScript/bridge authority changes.
- **Dependencies:** Unity `062a881`, architecture audit `8110820d`; preserve `StudioCameraRecoveryTests`, `StudioLegacyUiMetricsTests`, `StudioDevelopmentCardHud`, `StudioProductionRailHud`, `StudioLivingTimeHud`, memo, Founding, Back control, and current Cinemachine rigs.
- **Tests:** all six camera-recovery tests unchanged; host enable/disable rollback; wide/mid/narrow + safe-area/DPI; mouse/keyboard/gamepad navigation and focus restore; UI capture suppresses world/camera; modal/UI Back peels before world; explicit Locate/no refresh movement; zoom after Locate; repeated Locate→Back; workspace context returns after the accepted camera pop; no new direct device polling.
- **Risk:** HIGH—a second camera stack, altered Escape order, or general-platform refactor before feature value.
- **Stop condition:** one empty Casting-shaped retained route opens/closes over the live lot, owns focus/pointer correctly, composes with the accepted camera trail, and restores retained context; manual camera intent still invalidates stale origin; disabling the host leaves every existing IMGUI workflow and P03A.3 camera proof operational.
- **Cheapest capable worker:** Sonnet/equivalent for bounded host/action implementation after Fable fixes interfaces; Haiku/equivalent for asset/test fixture wiring; Fable alone integrates retained UI with the accepted camera/selection semantics.
...
- **Likely files/symbols:** Casting UXML/USS/controller/view-model files on the Wave 0 host; exact project route from `development.board.projects[].projectId`; reuse behavior, not IMGUI inheritance or `StudioLegacyUiMetrics`, from `StudioDevelopmentCardContracts`/`StudioDevelopmentCardHud`; call `StudioLocateAction` through the Wave 0 workspace-context coordinator while reusing P03A.3's camera trail; extend `StudioBridgeBootstrap` only to register the host; extract/extend `StudioBridgePresentation.ResolveZoneId`/`ResolvePlacedBuildingId`; generalize memo ownership around `StudioBridgeClient.PlayerIsDevelopmentOption`, `PlayerIsTimeOption`, and `SetWorldTimeOwnerPresent` so Casting owns `startAuditions`/`acknowledgeAuditions`/`greenlightPicture` only while usable.
...
- **Tests:** UI Toolkit layout/focus tests; one contained `ListView`/`ScrollView` owner; nested Back restores project/role/candidate/compare/scroll/focus while the accepted trail restores camera pose; central pointer capture; controller parity; no automatic Focus/selection on open; current `casting` and placed-facility routing fixtures; memo owner present/absent/fail-closed fallback; Living Time ownership unchanged.
...
| HIGH | Duplicate/broken camera recovery | P04 adds a second pose stack, bypasses `StudioSelectionManager.Select`, changes Esc order, or treats Home as Back. | Reuse `StudioNavigationOriginTrail` and public controller seams; workspace token holds UI state only; preserve all P03A.3 recovery tests/proofs. |
| MEDIUM | Camera hijack / broken Back | Opening Casting or accepting Greenlight focuses, snaps Home, or loses prior Casting/camera context. | No automatic focus; explicit `StudioLocateAction`; existing trail restores camera, retained token restores UI; never call `ExitInspection(true)` for ordinary Back. |
| HIGH | Input/UI foundation sprawls | P04 rewrites legacy IMGUI/camera/platform systems or Casting polls devices/owns rectangle suppression independently. | Wave 0 is one UI Toolkit host, one action/context service, and one retained-context coordinator around the existing camera trail; all existing surfaces remain rollback-safe; no direct polling in Casting. |
...
| MEDIUM | Legacy scale leaks into retained UI | Casting imports `StudioLegacyUiMetrics`/base rects and becomes tied to 1720×1045 IMGUI assumptions. | Preserve metrics only in surviving IMGUI; use PanelSettings/USS responsive tokens and UI Toolkit focus/hit-area laws for P04. |
...
- one shared presentation input-context service and one retained workspace-context/Back coordinator; adapt existing `StudioCameraInput.Current` rather than adding Casting device polling, and reuse the P03A.3 camera trail rather than adding a pose token.
...
- `StudioLocateAction.cs` as the exact stable-ID dispatch; `StudioCameraInput.cs` for action-backed sampling; `StudioSelectionManager.cs`/`StudioCameraDirector.cs` only if the smallest consumed/restored notification or generic Back dispatch is required; `TycoonCameraController.cs` should normally be reused without behavior change
...
P03A.3 regression-sensitive files Fable should avoid editing unless the bounded integration demonstrably requires it: `TycoonCameraController.cs`, `StudioCameraDirector.cs`, `StudioSelectionManager.cs`, `StudioCameraInput.cs`, `StudioLegacyUiMetrics.cs`, `StudioHud.cs`, `StudioLivingTimeHud.cs`, `StudioProductionRailHud.cs`, `StudioFoundingBeaconHud.cs`, `StudioFoundingCardHud.cs`, and `StudioDevelopmentCardHud.cs`. Never import `StudioLegacyUiMetrics` into new Casting UXML/USS/controller code. If a camera/selection file must change for one coordination event, preserve all existing public behavior and run the complete P03A.3 recovery, camera, legacy metrics, Living Time, Rail, Founding, and Development proof suites.
...
- never reimplement or shadow `StudioNavigationOrigin`, `StudioNavigationOriginTrail`, `NavigationOriginAvailable`, `PushNavigationOrigin`, `TryRestoreNavigationOrigin`, `ClearNavigationOrigins`, `StudioCancelStep`, `NextCancelStep`, `LastInspectionExitFrame`, `InspectionControlVisible`, the accepted 190×50 Back control, `PanSpeedFor`, or `WheelNotchZoomUnits`;
- never serialize camera origins, store camera pose in a Casting draft/workspace token, call `FocusOn` directly for a player Locate, use `SnapHome` as Back, or retain an origin after deliberate manual camera input;
- never import `StudioLegacyUiMetrics`, `ScaleFor`, `Scale`, `Unscale`, `ScaledFont`, its reference viewport/cap, or IMGUI `GUIStyle` geometry into the UI Toolkit Casting architecture;
...
## 20. P03A.3 Owner-accepted final delta-refresh resolution
...
| Locate | `StudioLocateAction.Locate(stableId, focus)` flows through selection; management Focus now pushes a camera origin | Reuse unchanged; never trigger from refresh, call `FocusOn` directly, or duplicate stable-ID/pose logic. |
| Camera origin/Back | `StudioNavigationOriginTrail` depth-8 LIFO; target-pose capture/restore; visible Back; manual-input/Home invalidation | Reuse as the only camera-pose stack. Preserve exact P03A.3 behavior and proofs. |
| Esc | `NextCancelStep`: inspection → selection → origin, one layer per press | Extend central arbitration so nested retained UI/modal layers consume first, then fall through unchanged. |
| Retained workspace restoration | no route/project/role/candidate/compare/scroll/focus token or camera-restore notification landed | Add presentation-only context and minimal coordination around the existing trail; store no camera pose. |
| Input | wheel notches normalized in direct-polling `StudioCameraInput`; action asset/context/controller still absent | Preserve normalization; complete the accepted action/context/UI navigation foundation. |
| Camera traversal | `PanSpeedFor`, unscaled motion, manual invalidation, Home semantics accepted | Regression boundary; no P04 tuning. |
...
| Responsive behavior | `StudioLegacyUiMetrics` scales surviving IMGUI and preserves reference pins; Development hierarchy improved; no inner scroll | Preserve legacy behavior; do not import metrics or unsafe row-dropping into P04 UI Toolkit. |
| Proof provenance | P03 artifacts do not stamp Git SHA | P04 proof must stamp both SHAs and contract versions. |
| UI technology | no runtime UI Toolkit landed in P03A.3; audit `8110820d` is accepted | P04 establishes bounded UI Toolkit host/action/workspace-context foundations; P03A.3 already owns camera origin; preserve all existing IMGUI. |

P03 files that P04 may extend but should not rewrite casually: `bridge/development.ts` as a converter/quote pattern; quote unions in `bridge/schema/bridge-schema.ts`; `BridgeSession.pendingQuotes`, `quote`, `quotedIntentFor`, `command`, and `load`; `StudioBridgeClient.RequestCommissionQuote/PostQuote` and its presentation-owner registration; `StudioDevelopmentCardContracts`/`StudioDevelopmentCardHud` safety laws as behavior only; `StudioDevelopmentContracts` TypeScript-copy pass-through; `StudioCameraInput.Current` as the compatibility facade; `StudioSelectionManager`/`StudioCameraDirector` only for minimal retained-Back coordination if necessary; `StudioLocateAction` and `TycoonCameraController` navigation APIs as accepted reusable dispatch/pose authority; `StudioBridgePresentation.ResolveZoneId`/`ResolvePlacedBuildingId` facility-world join.

P03 sealed symbols P04 must leave behaviorally intact: commission draft/range conversion in `draftToEngine`; `CASTING_BOUNDARY_LINE`; Development board copy/hierarchy; `StudioProductionRailContracts`/`StudioProductionRailHud`; `StudioLegacyUiMetrics`; `StudioLocateAction.Locate`; `StudioNavigationOriginTrail` and controller navigation APIs; `StudioCancelStep`/`NextCancelStep`/`LastInspectionExitFrame`; `PanSpeedFor`; `WheelNotchZoomUnits`; named writer `engagement === 'script'` routing and labels; Development/Living Time memo demotion; accepted-command/load quote invalidation; non-journaled quote read; 0.7-second arm; no automatic camera/selection change.

## 21. Fable handoff summary

## START HERE

Wave 0: freeze P03A.3 recovery/scaling proofs; land the bounded UI Toolkit host and centralized Input System contexts; add retained Casting context and compose its Back with the existing camera trail—no second pose stack. In parallel, Wave 1 publishes the closed TypeScript Casting/package view. Then extend sealed `POST /quote` and its single ephemeral `pendingQuotes` registry; `/command` remains the sole commit path.

## REUSE THESE

`castingSessions.ts` lifecycle/law/RNG, `castingReadModel.ts`, employment/talent/film-package selectors, `commitmentPreview`, atomic Greenlight/queue, P03 `/quote` + opaque intent, bridge journal/refusals/pending-post, card safety laws, `StudioLocateAction`, `StudioNavigationOriginTrail` + controller APIs, one-layer Esc grammar, accepted camera speed/zoom/unscaled-time behavior, exact writer/presence joins, and existing Unity production presentation.

## EXTEND THESE

Casting/package/candidate/expiry projections; the existing quote union and `BridgeSession.pendingQuotes/quotedIntentFor`; generated DTO/store; semantic Casting owner resolver; one UI Toolkit host and input-context service; a retained workspace-context/Back coordinator around the existing Locate/camera trail; one semantic memo-owner registry; contained scroll; typed receipt; identity-bearing queue expiry.

## LIKELY NEW SEAMS

Transient exact-ID player draft independent of quote revision; `bridge/casting.ts`; Package 11 consequence envelope; persistent expiry `subjectId`; runtime `UIDocument`/PanelSettings/token USS; centralized Global/World/Camera/UI contexts; retained route/project/role/candidate/compare/modal/scroll/focus token; minimal notification/dispatcher to coordinate it with P03A.3 camera Back; building-level Screen Test fallback activity.

## DO NOT REBUILD

Do not rebuild Camera Test law/RNG/time, Fit/cost/availability/finance, queue/Greenlight/save/replay, camera origin/Back/Esc/speed/zoom, Development Rail, Locate, legacy scale, or quote/command paths. Do not import `StudioLegacyUiMetrics` into UI Toolkit or migrate existing IMGUI, Addressables, renderer, DOTS, or capture/platform infrastructure. Never cross hidden `Talent.actual`, ceilings/persona/temperament decomposition, `teamDirection*`, hidden RNG/formulas, or parse payloads from display text.

## TEST THESE FIRST

Preserve all six `StudioCameraRecoveryTests`, five `StudioLegacyUiMetricsTests`, and the Development recovery proof; add UI-host/action-context/controller rollback, modal/UI-first Esc, zoom after Locate, and workspace context restoration around repeated/chained camera Back. Then test Casting memo ownership, quote purity, exact six reads, draft/quote continuity, hostile presence, finance basis, Greenlight parity/expiry, duplicate replay, and hidden-data exclusion.

## WATCH THESE RISKS

Second camera-pose stack, altered Esc/Home/manual-invalidation law, legacy scale leaking into UI Toolkit, input foundation sprawl, stale quote replay/silent draft clearing, hidden-data leak, memo owner removing the only verb, incomplete Opex runway, expiry without project ID, literal `casting`, and no contained scroll.

## P03A DELTA TO RECHECK AFTER SEAL

Closed at Owner-accepted TS `0d9fc65765e5c55ed221688238293babfcaea93e`, Unity `062a881afe9d38352b86b260afbe18ee070a3deb`, and architecture audit `8110820d96ddf2089df582bc0a0a92d3d4cf17d9`. Protocol/schema/quote remain unchanged; Rail remains Development-only; Locate now has accepted camera-origin/Back recovery; retained UI context and centralized action/controller navigation remain P04 seams; legacy scaling remains IMGUI-only; Casting's three memo verbs still need an accepted conditional owner. Development is closed. Reopen only if an accepted authority moves.
