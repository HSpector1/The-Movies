...
| **Exact Post facility identity** | A | **DELIVERED core-side; Unity mapping is chartered W3 work.** Core reservations carry exact facility ids; the browser and Unity currently know only the founding `post` body. `placeFacility`/`demolishFacility` exist, so N Post facilities are reachable. P06A scope ruling (final): the founding `post` body is the world anchor; every row labels its exact core facility id/name separately; placed facilities without authored bodies are the `Locate unavailable — no placed body` case, never first-match. **CORRECTION (found during W7 fixture authoring, 2026-09-02):** the founding Post facility capacity is **2**, not 1 — `TUNING.FOUNDING_POST_CAPACITY = 2`, pinned by `tests/c2a-m0-engine-union.test.ts`. Every place this recon or design implied capacity 1 for the founding `post` facility is wrong: genuine Wrapped-Waiting contention requires BOTH founding slots occupied before a third picture wraps in. The six-scene oracle generator handles this by deep-founding and placing one extra stage+scenery (scenes 2/6); no simulation law changed. Disclosed to the hostile reviewer. |
