...
- Radio: 126 audited units, 108 decorative-eligible, 18 functional-template units withheld, three typed fixtures, three 660-second programmes, three 30-minute simulations, three presenters, and zero current positive streaming/VOD authorization records. Radio Streamer Safe must silence all current voice/PA/sting/bed audio while retaining functional/important visual text.
- Accessibility: eight isolated bus contributions and six final-sum preset renders.
- Audio Oracle: eighteen required scenarios plus two supplemental authority-compatibility scenarios; exactly two current Unity Editor offline output-processor marker renders, explicitly not runtime mix captures. State records the earlier content phase as 124 generated/113 derived files, the two Oracle markers separately, AAC audition derivatives separately, and the whole bounded total dynamically. ERR-0013 additionally records one preservation-only failed/unpublished attempt with twenty trace-level records and zero new audio/captures; those records are not a current/archived suite or suite-level PASS.
...
- Streamer Safe authorization failure: remember that `PROTOTYPE_ONLY` and `PROTOTYPE_READY_FOR_OWNER_AUDITION` grant no streaming/VOD right. Require a contained record with exact record SHA-256, asset ID/audio SHA-256, and explicit `STREAMING_VOD` scope before audible eligibility. The current schema has no positive-record loader, so all current radio audio remains silent and fabricated/uncontained positives are rejected; functional and important visual text persists.
- Stale Oracle SHA: finish and push source changes, rebuild Unity evidence, then re-verify.
- Failed/unpublished Oracle trace reachability: do not delete traces or fabricate a suite. Publish and self-test the exact allowlist register, atomically reopen state with `REOPEN_AFTER_PREPACKAGE_FAILURE`, preflight archive integrity before any new trace write, bind the register in the next current-run pointer, rebuild D/U-bound proof and all eight reviews, and only then resolve ERR-0013 through `READY_FOR_PACKAGING`.
...
4. Re-run `tools/audio_systems_pilot_01/run_unity_lab_validation.zsh` from exact **D/U**. It process-gates and writes the exact compile/EditMode/PlayMode/build/Oracle/archive-register/validation-summary evidence paths, exports **D**, and never launches the app. Exit 75 means wait for the unrelated Unity process; never terminate it.
   A successful run is snapshotted eagerly under `09_unity-lab/CompletedRuns/`. A retry first verifies and reuses a matching immutable snapshot even after live D-bound artifacts have advanced, then archives only those exact pointer-named bytes through a staged, rehashed, atomic promotion. Do not remove the completed-run snapshot or rewrite an older archive. `MANAGEMENT-METADATA-HISTORY.v1.json` must exactly register all preserved D-bound management catalogues; an intermediate registered catalogue need not have been consumed by Unity, while every Unity-referenced catalogue must be registered. The one bounded `ArchiveSupplements` record is a hash-authenticated metadata reconstruction and is reproduced using its own historical Git-bound repair tool; the original historical archive stays intact and its `FAIL` outcome remains authoritative for that old attempt.
5. Regenerate the clean-commit identity/status/Oracle/audition chain, then build a provisional complete register and record its SHA-256. Confirm the radio runtime index and system register both report zero current positive streaming/VOD authorization records and that scheduler/Unity evidence covers actual decorative, functional, PA/help, milestone, and bed suppression plus visual-only functional/important presentation.
...
The Owner then listens to responsive music, transitions, the lot, management SFX, ordinary prototype Radio, and accessibility demonstrations; rates them; exports feedback; and decides whether revisions or a separately authorized post-P05 integration checkpoint should follow. Current Radio Streamer Safe intentionally produces no radio/PA/sting/bed audio because no positive streaming/VOD authorization exists; it demonstrates retained functional/important visual text and fail-closed rights handling instead. No music theory is required.
