...
11. Atomically update `00_state/AUDIO-SYSTEMS-PILOT-STATE.json` to exact `IN_PROGRESS` / `READY_FOR_PACKAGING`, with clean **D/U**, all live counts (including `audition_items`, `unity_editmode_passed`, `unity_playmode_passed`, and `hostile_review_lanes`), every unique/resolved recovery record (including the Streamer Safe authorization-overclaim remedy), unique decisions, a completion entry containing `clean-SHA Unity` and `Audio Oracle`, and a next action that names package creation.
