...
The chain eagerly snapshots every current-run pointer byte only after the Unity summary is `PASS`. On the next run, a matching immutable completed-run snapshot is fully verified and reused before consulting live D-bound artifacts; archival reads only that snapshot, validates every pointer size/hash, stages the archive on the same filesystem, rehashes it, and atomically promotes it. Pointer path sets are versioned by the documentation commit: historical pointers retain their exact legacy set, while every pointer from the commit introducing the failed-attempt register must also bind that register. Content-addressed management-metadata history protects the SHA-rebinding boundary; `MANAGEMENT-METADATA-HISTORY.v1.json` explicitly registers every immutable D-bound catalogue, including valid intermediate bindings, while every Unity-referenced hash must be a member of that exact registered set. A separately labelled supplement for the one pre-remedy archive inconsistency preserves the original archive unchanged, restores only its metadata pointer projection, and does not rehabilitate that historical run: its preserved Unity outcome remains `FAIL`. Later invocations reproduce that immutable supplement from its own reachable, hash-authenticated historical repair-tool binding.

ERR-0013 records a different boundary. Unity commit `199aa643c53dce124a58c813767f15277c54457e` wrote twenty immutable trace files before archive-register validation, then failed with `ORACLE_ARCHIVE_REGISTER_PRIOR_INTEGRITY_FAILED:ARCHIVED_ORACLE_SCENARIO_ID_INVALID:early_era_normal`; no suite/current pointer was published. `failed_oracle_attempts.py` authenticates an exact twenty-file allowlist plus the process gate, failure log, runtime observation, build receipt, old D/U/binary/catalogue identities, and mixed-byte archive manifest. Its suite-level verdict is null. The stale earlier validation PASS within that mixed archive is explicitly non-attributable. The full chain publishes this preservation register before Unity and the Unity publication path must preflight its archive register before any trace write. Do not delete the trace bytes, add them to an archived suite, or relax reachability to arbitrary extras.
...
Every final report must contain the exact standardized documentation SHA, Unity SHA, system-register SHA-256, Audio Oracle suite SHA-256, Audio Oracle failed-attempt-register SHA-256, and provisional/final complete-register SHA-256 binding lines. Any later hostile-review report change invalidates the index; rebuild and verify it before packaging.
11. Atomically update `00_state/AUDIO-SYSTEMS-PILOT-STATE.json` to exact `IN_PROGRESS` / `READY_FOR_PACKAGING`, with clean **D/U**, all live counts (including `audition_items`, `unity_editmode_passed`, `unity_playmode_passed`, `hostile_review_lanes`, and the separately scoped failed/unpublished Oracle attempt/trace counts), every unique/resolved recovery record (including ERR-0013), unique decisions, a completion entry containing `clean-SHA Unity` and `Audio Oracle`, and a next action that names package creation. A prepackage failure before package publication first uses `REOPEN_AFTER_PREPACKAGE_FAILURE`; that atomic transition does not trust stale D/U-bound green evidence, records ERR-0013 unresolved, and returns the phase to `PHASE_G_HOSTILE_REVIEW_REMEDIATION`. Only the normal `READY_FOR_PACKAGING` transition may resolve it, after fresh Unity, downstream, and eight-lane proof pass.

```sh
python3 tools/audio_systems_pilot_01/update_final_state.py READY_FOR_PACKAGING
```

For the bounded ERR-0013 recovery only:

```sh
python3 tools/audio_systems_pilot_01/failed_oracle_attempts.py --publish
python3 tools/audio_systems_pilot_01/failed_oracle_attempts.py --verify-only
python3 tools/audio_systems_pilot_01/failed_oracle_attempts.py --self-test
python3 tools/audio_systems_pilot_01/update_final_state.py REOPEN_AFTER_PREPACKAGE_FAILURE
```
...
System-register v5 contains 147 explicit items. The 25 additions at the final hostile-review remedy are 18 Radio Voice files, six PA Voice files, and one milestone sting. `build_system_asset_register.py` accepts a voice only after its exact programme/role path, full schedule event, delivered-voice event, typed payload IDs and expiry where applicable, per-role metadata, clean/period identities, audio probes, exact audio hashes, spoken-text hash, caption/spoken equality, duration, eligible presenter, delivered speaker, caption context, speech owner, intended bus, and exact negative streaming/VOD authorization record agree. A composite opening remains one item. Equal-content PA clean files retain distinct path IDs while sharing an acknowledged content hash; no claim of distinct audio is made. The sting is pinned to SHA-256 `8b4e0b9a4e609737d91a3fc95fe313213c496d531c00e802628c62457f02acd6`; its one important-sound caption is taken verbatim from all three hash-bound caption tracks.

The register names all 27 manifests it consumes or directly projects, including the transition catalogue, three schedules, twelve voice metadata records, and three caption tracks. Final validation records the exact rebuilt canonical-register SHA-256, requires that exact path/hash set, recomputes item-role/ID projections, and independently re-projects each radio item’s full typed payload, delivered-voice timing/context, and negative authorization identity from its schedule. Missing manifests, cross-program path substitution (including equal-hash PA files), role or responsive-honesty drift, payload drift, delivery-time drift, fabricated authorization, and approval/clearance overclaims fail closed. This document does not predict the post-remedy register hash.

The v5 policy is closed-world: there are zero current positive streaming/VOD authorization records. Every current radio voice, PA/help voice, milestone sting, and radio-music bed is silent while Radio Streamer Safe is enabled; exact functional and PA visual receipts/captions/transcripts and the milestone important-sound caption remain available. `PROTOTYPE_ONLY` and `PROTOTYPE_READY_FOR_OWNER_AUDITION` are not streaming grants. Future positive eligibility requires a contained authority record whose exact record SHA-256, asset ID, asset audio SHA-256, and explicit `STREAMING_VOD` scope all validate. No such positive-authority loader exists in v5.
...
The Audio Lab resolves only the v5 system-register entries beneath an explicitly approved root. It canonicalizes the root and candidate, rejects absolute catalogue-relative paths, traversal, symlink escape, unsupported formats, duplicate IDs, missing files, hash mismatch, and every network URL. Radio Streamer Safe additionally validates the closed-world negative authorization identity and rejects any claimed positive authorization because no contained positive-record loader is implemented. A refusal is visible and exact. There is no silent substitution, token, cloud request, or directory discovery.
...
- Radio voices use generic local macOS voices, no cloning or real-person imitation. These outputs remain local scratch material with redistribution and streaming/VOD authorization unresolved; Radio Streamer Safe silences them.
...
Pure TypeScript tests cover deterministic decision and radio scheduling, including current voice/PA/sting/bed suppression and fabricated-positive rejection under Radio Streamer Safe. Unity EditMode covers catalogue parsing, security, selection, transport math/state, radio, visual-only rights fallback, accessibility policy, and controls. Bounded PlayMode covers runtime lifecycle/event observation, while Unity Editor offline processor renders exercise final-output markers where batch audio permits. The latter are not AudioSource, mixer, built-player, or hardware-output captures. Code-sign verification binds the packaged macOS app. The complete-file register and final reconciliation rehash current/preserved media and all return-package files.
