...
Radio Streamer Safe is a rights gate, not a mix preset and not a claim that prototype audio is safe to stream. `PROTOTYPE_ONLY` and `PROTOTYPE_READY_FOR_OWNER_AUDITION` confer no streaming/VOD permission. The current catalogue contains zero positive streaming/VOD authorization records, so enabling the control silences all current radio music, radio voice, PA/help voice, and milestone-sting audio, including any already-playing source, and clears their presentation ducks. Functional and PA/help captions, typed visual receipts, and transcript entries remain available from the same resolved payload; the milestone retains its important-sound caption. A visible diagnostic explains the silence. Future audible eligibility requires a contained, hash-verified authority record tied to the exact asset ID/audio hash and explicit `STREAMING_VOD` scope; that positive-authority route is not implemented.
...
- Radio Streamer Safe retains typed functional/PA visual text and milestone important-sound captions while all current radio audio remains silent.
...
- clarity and usability of visual-only functional/important presentation while Radio Streamer Safe is enabled;
