...
| Adoption candidate | `a7e4847` |
| Production `main` | unchanged at `aadbd63d` — **not merged** |

Ten linear commits, no merges, no amends to reviewed checkpoints, no tag, no PR.
...
Geometry, texture size, massing, roof and doors are **identical** between finishes — asserted in test.
It is a finish, never an architectural change, and it must not imply damage, closure, construction,
abandonment or unavailability.
