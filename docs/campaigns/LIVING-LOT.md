...
### P04A REOPEN — Owner-launch checkpoint migration (re-sealed 2026-08-27)

**The documented Owner launch command failed on the real durable profile** (`~/Library/Application Support/Project Studio`): the engine exited pre-readiness with `checkpoint.schemaId: does not match the running TypeScript bridge schema`. The Owner did nothing wrong. A byte-preserving backup was taken first (`Project Studio-BACKUP-P04A-REOPEN-20260827`, diff-verified) and the live profile was never mutated by this remediation.

**Root cause (proven, class B + latent A + C):** the profile checkpoint (protocol 4, checkpointVersion 1, revision 21, 21 journal entries, V14 save, 623,999 bytes, written 2026-08-24 11:09) carries the **projection-v4** schema identity `sha256:f84ae77e…` — four generations older than the single carry-forward slot (which held only v8's `0285e92f…` and was never advanced through v9/v10/v11 during P04A). Two further latent failures sat behind the slot: the prior path validated embedded saves as V15 (refusing V14) and hydrated old journal bodies under the current contract (impossible across seven versions). Reproduction: old engine + byte-copy of the profile → the exact failure; fresh profile → healthy. **Self-criticism recorded:** the original seal's non-blocker #16 documented this exact fail-closed signature but mis-scoped it to LT harness fixtures rather than durable player profiles — that mis-scoping is the root of this reopen.

**Remedy (`0386d34` + boundary pins in the re-seal commit):** the single slot became a closed enumerated map of every exact historical protocol-4 identity a durable checkpoint can carry — nine, each pinned with era + introducing commit (v4-early `ba9cd199…`, v4 `f84ae77e…`, v5 `be7ed660…`, v6 `15033cf9…`, v7 `7e3af4db…`, v8 `0285e92f…`, v9-early `510f08e4…`, v9 `80f2f0fc…`, v10 `92317ec1…`). Hostile review proved this set **exhaustive**: the checkpoint file format was born (`e6fc204`) after the protocol-3 identity and before the first protocol-4 identity, so {legacy} ∪ {the nine} ∪ {current} is the complete universe. The unified prior import validates the old artifact on its ORIGINAL bytes (exact keys, canonical-JSON+LF, journal bounds/unique ids/journalDigest, save digests), **never parses journal bodies** (opaque discarded history — the exact compatibility boundary), imports both save slots through the existing canonical chain (`importSave → migrateToV15 → exportSaveJson`), keeps the founding-draft guard on the migrated state, and emits the documented cost unchanged: journal `[]`, revision 0, fresh session, atomic current-schema rewrite through the untouched lock/write laws. Protocol-3 law byte-untouched; unknown/malformed identities still fail closed on today's exact messages.

**Owner-launch proof (copied profile; the real profile migrates once on the Owner's own first launch):** old dist + copy → exact failure; new dist + copy → healthy (health 200), checkpoint migrated once — current schema, revision 0, journal 0, fresh session, save V14→V15 with a **zero-field state diff** (hostile review deep-diffed: week 3, cash $18,956,921, 60 talent, 7 contracts — everything preserved); clean stop; relaunch → healthy, checkpoint byte-identical (`a1739a4f…`), no re-migration (mtime never advanced); fresh profile on the new dist healthy. Gates: bridge suites 160/160 (+2 boundary pins → checkpoint file 35/35); typecheck clean; dist rebuilt containing all nine hashes.

**Hostile review (fresh Opus, charter focus: state loss / silent reset / broad acceptance / malformed acceptance / journal replay / atomicity / repeated migration / false claims / fresh-only proof): ACCEPT, no blockers.** It independently re-derived the enumeration, replayed the importer over the Owner's real bytes, forged adversarial variants (crossover ids, flipped hex, tampered saves with recomputed digests — all rejected; the core save construction invariant caught even digest-consistent tampering), and verified no-silent-reset (corrupt ≠ absent; only ENOENT mints fresh) and atomicity (no new fs calls; the locked link-backup/rename/fsync store). Its hardening items landed pre-seal: the map now test-pins that it NEVER contains the running identity (the exact re-migration hazard) and pins all nine literals load-bearing.
