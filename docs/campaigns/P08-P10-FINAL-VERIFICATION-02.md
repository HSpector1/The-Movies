...
(written 2026-09-07 ≈17:45Z, subject to the independent reviewer's disposition recorded in
`P08-P10-FINAL-VERIFICATION-02-REVIEW.md`; nothing here is Owner acceptance).
...
- **Not done / caveats:** the pair-1 player binary was overwritten by the pair-2 build (hashes + manifests retained);
  ...
  run-binding is reconstructed and one carries a hash caveat (§4.0 16:37 / 16:57); the walking-figure click can
  need several retries (recorded); the pair-1 roof-edge probe/oracle disagreement is a NOTE for the reviewer.
...
- **Correction (`hid-guard.mjs`, launchers, drivers).** The launchers now start the witness before the drive
  ...
  stamp → **suspend exactly as before**. Without a witness log the guard stays as strict as before. The
...
- **Driver corrections found by the runs (all harness, none product):** (1) `underUiAt` in the people and
  ...
  `hid-20260907T162344Z`); (4) a people-drive click that missed a walking figure is recorded in full as a
  retry (`missedRetry`), not as a gate failure, and the landing click states how many misses preceded it —
  the gate remains the verified exact selection.
...
- **Root cause (`StudioWorkspaceHost.SetPanelInputSuppressed`, P04A.1 commit `9e75b5f`, pre-existing).**
  ...
  input routing); never caught before because the two-repairs BUILD driver only asserted the site's rect
  was *published* after the post-Load click.
...
| 17:06–17:10 | Defect F fix → EditMode → rebuild (pair 2) | `StudioSystemMenuTests` 15/15 on the fixed host (`editmode-menu-fix-20260907T170624Z.xml`); **pre-fix control**: the new regression test FAILS on the old host, 14/15 (`editmode-menu-prefix-control-20260907T1707…Z.xml`); full EditMode **900/900** (`editmode-fixF-full-20260907T170930Z.xml`); tools committed `d849a43` (harness only), fix committed **`574339da31`** (host + tests only), player rebuilt at that commit with `dirty=false` → **pair 2**: exe `f678cf539d067ab562d064458f6aef316fe61fabf0ba87cbf2c92e670ca9a9b0`, Assembly-CSharp `75217bf0…`, engine unchanged `189326b6…`, scene unchanged `16629911…` (the drawn-geometry dump stays bound). The pair-1 player binary was overwritten in `Builds/macOS/` by this build (its hashes and manifest copies live in every pair-1 evidence dir; the control `ab1fa09b…` is untouched) | Full re-validation on pair 2 queued (`Evidence/*-FinalVerification02-Rebuild/`): sweep 46 → camera auto-proof → CONTRACT → PEOPLE → BUILD |
...
| Later Unity commits after pair 1 | `d849a43` (tools: witness, guard attribution, orbit, drivers, launchers), **`574339da31` (PRODUCT: defect F fix + tests → pair 2)**, `41d94eb` (tools: pair-2 drive corrections). `git diff --stat 75de360..HEAD -- Assets` = `StudioWorkspaceHost.cs` (+24/−2, the fix) + three test files |
| Pair-1 tools/tests-only commits (no player change vs `75de360`) | `5193986` (executable bits), `74cef3b` (test adjustments, launcher bindings), `0bb8323`, `d1715fa`, `9d3e8dc`, `08b7be4`, `bcc1af9`, `33d551b`, `2a98d6b`, `738cafa`, `f0e1dab`, `1cd232f` (harness guard, launchers, oracle-launcher engine binding) — see `git log 75de360..HEAD` |
