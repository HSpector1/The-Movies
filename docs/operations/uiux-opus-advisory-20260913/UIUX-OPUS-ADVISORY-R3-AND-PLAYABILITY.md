...
1. **The first slice must be the walking skeleton of the main screen, not the strip
   alone.** The selection note (§3) already says this; the plan's 48-hour allocation still
...
2. **Run the 6-hour audit RITE-style with fresh players, not only Howard.** RITE
...
4. **Write the input model once and reuse it for controller.** Digital-first: one Tab stop
...
10. **Reserve a later, separate pass for onboarding cues** (the original's star trail /
...
XAG 103 Multiple sensory methods · XAG 106 UI navigation · XAG 107 Input · XAG 113
...
Limits: I exercised the R3 prototype and read the plan; I did not run the native build.
