...
let people = () => S.fixture === 'early' ? FIXTURE.employees.slice(0, 3).map(p => ({ ...p, work: 'No assignment', place: 'Casting', status: 'Available', x: 21, y: 41 }))
  : S.fixture === 'long' ? [...LONG_FIXTURE.employees, ...FIXTURE.employees] : FIXTURE.employees;
let projects = () => S.fixture === 'early' ? [FIXTURE.projects[2]] : S.fixture === 'long' ? [...LONG_FIXTURE.projects, ...FIXTURE.projects] : FIXTURE.projects;
const person = id => people().find(p => p.id === id) || FIXTURE.employees.find(p=>p.id===id) || FIXTURE.candidates.find(c => c.id === id);
const project = id => projects().find(p => p.id === id);
const buildingFor = place => FIXTURE.buildings.find(b => b.name === place);
const needsMe = p => !!p.action && !['success','pending'].includes(S.responses?.[p.id]);
...
function hud() {
  ...
  return `<header class="hud">
    <div class="transport"><button class="tb on" data-act="noop" aria-pressed="true" aria-label="Paused">${icon('pause')}</button><button class="tb" data-act="noop" aria-label="Play">${icon('play')}</button><button class="tb" data-act="noop" aria-label="Fast">${icon('fast')}</button>
      <span class="date"><strong>Week ${f.week}</strong><em>${f.year}</em></span><span class="paused-stamp">PAUSED</span></div>
    ...
    <div class="money"><button class="cash" data-act="finance" aria-label="Cash ${f.cash}, opens Finance">${icon('coin')}<span><strong>${f.cash}</strong><em>Studio cash</em></span></button><span class="rank" title="Studio rank (illustrative)">${icon('star')}${f.rank.value}</span><button class="menu" data-act="menu" aria-label="Menu">${icon('menu')}</button></div>
  </header>`;
}
...
function world() {
  ...
  if (a) {
    ...
    marker = `${fp}<div class="marker ${target.id.startsWith('P') ? 'person' : 'site'}" style="left:${(a.topX ?? a.x)}px;top:${(a.top ?? (a.y - 70))}px"><span class="who">${target.id.startsWith('P') ? portraitSVG(target, 28) : target.kind==='facility'?icon('build'):icon(STAGE_ICON[PHASE_INDEX[target.phase] ?? 0])}<b>${esc(target.name || target.title)}</b><em>${esc(a.name)}</em></span></div>`;
  }
