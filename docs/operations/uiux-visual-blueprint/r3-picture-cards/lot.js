...
const LOT = (() => {
  ...
  function car(u, v, color) { const [wx, wy] = P(u + 6, v + 12); return box(u, v, 22, 11, 7, color, shade(color, 1.12)) + box(u + 6, v + 1.5, 10, 8, 5, shade(color, 0.75), shade(color, 0.55)) + `<ellipse cx="${wx.toFixed(1)}" cy="${(wy - 1).toFixed(1)}" rx="2.6" ry="1.4" fill="#2a2622"/><ellipse cx="${(wx + 11).toFixed(1)}" cy="${(wy + 5).toFixed(1)}" rx="2.6" ry="1.4" fill="#2a2622"/>`; }
...
<rect width="1440" height="868" fill="url(#sun)"/><rect width="1440" height="868" filter="url(#grain)" fill="#b9b184" opacity=".85"/>`;
    ...
    s += `<g opacity=".55" filter="url(#soft)">${[[380, 80, 240, 90], [1060, 120, 280, 110], [220, 640, 300, 110], [900, 700, 340, 120], [700, 40, 160, 60], [1200, 500, 220, 90]].map(([x, y, rx, ry]) => `<ellipse cx="${x}" cy="${y}" rx="${rx}" ry="${ry}" fill="#8aa35f"/>`).join('')}</g><g opacity=".3" filter="url(#soft)">${[[520, 420, 90, 40], [980, 300, 80, 34], [300, 520, 70, 30]].map(([x, y, rx, ry]) => `<ellipse cx="${x}" cy="${y}" rx="${rx}" ry="${ry}" fill="#a4874f"/>`).join('')}</g>`;
    // roads (world grid)
