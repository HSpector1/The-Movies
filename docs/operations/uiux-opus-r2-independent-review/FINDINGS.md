...
5. **Type has no voice and the small sizes are too small** (`V-04`). One face
   (Trebuchet MS) for everything; secondary rail text measured at 10 px, status at
   11 px, cash caption at 11 px; no display face for the studio name or film titles.
...
| U-05 | Locate marker on the lot (yellow ring + pill "Located · schematic worksite") | `A02` box 2 | Medium | **REFINE** | Selection is not visually connected to the building: the ring floats above it, the building itself is not lit, there is no leader/trail, and the lot does not pan. The original uses a guiding star trail and a camera jump (manual pp.6, 9). Recommend: light the footprint, tint the building label to the selection colour, draw a short trail from the rail card, pan the lot so the target is centred. | Selecting a rail card highlights exactly one building/footprint and the label matches the card's colour state. |
