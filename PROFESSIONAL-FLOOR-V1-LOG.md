...
**Seal HEAD: `d95d6a6`** on `professional-floor-v1-fresh` (pushed). Correction, recorded
honestly: the seal was first announced at `3fb3986`, but a PM staging error had left the
markup half of the topbar compaction (and its wiring pin) uncommitted in the working
tree — every gate figure below was generated against the full tree, i.e. against exactly
the content `d95d6a6` records; the promotion-time working-tree safety check caught the
gap before canonical `main` was touched, the missing half was committed, and the decisive
gates were re-run at `d95d6a6` (typecheck clean; full vitest identical; the governed
reachability specs and the muted+reduced proof green). All figures below stand at this
exact HEAD:
