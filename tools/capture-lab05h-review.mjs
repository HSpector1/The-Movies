// Asset Lab 05H — capture the in-engine 05G-hero <-> 05H-authored-base comparison harness
// (Scene-G "05H Hero" group) + a sustained-FPS probe. Headless Chrome + SwiftShader is DIAGNOSTIC
// ONLY (software raster); real-hardware acceptance is the owner's Apple M3 pass. Run:
//   npx vite --port 4321 --strictPort   (another shell), then:  node tools/capture-lab05h-review.mjs
import puppeteer from 'puppeteer-core'
import { mkdirSync, writeFileSync } from 'node:fs'
import { fileURLToPath } from 'node:url'

const TARGET = process.env.LAB_URL ?? 'http://localhost:4321/'
const OUT = process.env.OUT_DIR ?? fileURLToPath(new URL('../proof/lab05h/runtime/', import.meta.url))
...
const VIEWS = [
  ...
  '05H/LOD', '05H/Management Distance', '05H/Human Scale',
]
const ANIM = new Set(['05H/Walk', '05H/Talk', '05H/Kneeling', '05H/Pickup', '05H/Sitting'])
const PERF = ['05H/Side-by-Side Front', '05H/Walk', '05H/LOD']
...
await set('wireframe', true); await call('setReview', '05H/Wireframe'); await sleep(1100)
await shotCanvas('27-wireframe-comparison.png'); await set('wireframe', false)
await call('setReview', '05H/Side-by-Side Front'); await set('showHud', true); await sleep(900)
await shotFull('28-review-status-panel.png')
