#!/usr/bin/env node
// Playability candidate assembly after the bounded product/correctness review.
...
if (ts !== '/Users/bruce/The Movies - Playability Interaction TS' || unity !== '/Users/bruce/The Movies - Playability Interaction Unity' || m.typescript.branch !== 'wip/playability-interaction-01-ts' || m.unity.branch !== 'wip/playability-interaction-01-client') fail('Only the authorized isolated Playability pair may be packaged')
...
if (output !== '/Users/bruce/Desktop/Playability-Candidate-20260913-01' || values.get('--output') !== output) fail('Only the explicitly new Playability candidate destination is authorized')
...
const generatedRoot = join(ts, 'artifacts/playability-interaction-01/review-library-01') + sep
const originalRoot = join(ts, 'Evidence/Playability-Interaction-01/fixtures') + sep
if (!library.path.startsWith(generatedRoot) || !provenance.path.startsWith(generatedRoot) || p.kind !== 'p13a-generated-evidence/v1' || p.source !== 'live engine generated fixture; no user campaign input' || p.scenario !== 'playability-review-library' || p.checkpoint !== library.path || p.bytes !== library.bytes.length || p.bytes > 256 * 1024 * 1024 || p.sha256 !== shaBytes(library.bytes) || p.schemaId !== v.schemaId || p.saveVersion !== 20 || !Array.isArray(p.sources) || p.sources.length !== 6) fail('Review-library provenance does not bind exact generated immutable bytes')
digestMatches(regular(join(ts, 'scripts/playability/generate-review-library.ts')), p.generatorSha256, 'Strict review generator')
const ids = new Set(), labels = new Set()
for (const source of p.sources) {
  if (typeof source.path !== 'string' || !source.path.startsWith(originalRoot) || typeof source.manifest !== 'string' || !source.manifest.startsWith(originalRoot)) fail('Review source is outside generated Playability evidence')
  const original = regular(source.path), originalManifest = json(source.manifest), op = originalManifest.value
  digestMatches(original, source.sha256, 'Original generated review source')
  digestMatches(originalManifest, source.manifestSha256, 'Original generated provenance')
  if (original.bytes.length !== source.bytes || op.kind !== 'p13a-generated-evidence/v1' || op.source !== 'live engine generated fixture; no user campaign input' || op.checkpoint !== source.path || op.sha256 !== source.sha256 || op.bytes !== source.bytes || op.schemaId !== v.schemaId || !/^[a-f0-9]{64}$/.test(source.extractedCheckpointSha256) || !Number.isSafeInteger(source.extractedCheckpointBytes) || source.extractedCheckpointBytes <= 0 || !Number.isSafeInteger(source.week) || source.week < 0 || typeof source.label !== 'string' || !source.label || typeof source.recordId !== 'string' || ids.has(source.recordId) || labels.has(source.label)) fail('Original generated review provenance or record binding differs')
  ids.add(source.recordId); labels.add(source.label)
}
if (!ids.has(p.activeCampaignId)) fail('Review active record is not among the generated sources')
...
const packaging = { kind: 'playability-candidate-package/v1', createdAt: new Date().toISOString(), status: 'assembled; not launched or accepted', typescriptCommit: m.typescript.sha, unityCommit: m.unity.sha, buildManifestSha256: shaBytes(manifest.bytes), pairedVerificationSha256: shaBytes(verification.bytes), reviewLibrarySha256: p.sha256, immutableReviewManifest: 'ReviewSetup/manifest.json', privateProfile: 'absent; first synthetic admission remains owned by the existing guarded playability-native-preview tool', launcherBasis: 'accepted P12A PLAY_PROJECT_STUDIO.command; only package error label changed', payload: [...payload].map(([path, f]) => ({ path, bytes: f.bytes.length, sha256: f.sha256, source: f.source })) }
