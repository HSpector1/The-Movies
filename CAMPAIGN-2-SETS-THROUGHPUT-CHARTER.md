...
## CAMPAIGN CHARTER (r3 — FROZEN FOR OWNER GO)
...
> 46 MAJOR / 20 MINOR); r2 adjudicated all of them and was then verified by a
> three-lens pass that confirmed every r1 BLOCKER resolved and filed 6 BLOCKER /
> 21 MAJOR / 23 MINOR residuals — including refuting two r2 claims outright. r3
> adjudicates all residuals AND incorporates the `00C`/`00D` rulings. Architect
> re-verifications are tagged "architect-verified" in place; the verify pass
> re-checked ~90 further citations (its held-lists are the audit trail).
...
- **One named union producer.** `occupiedResourceSlots(state)` lands at M0,
  ...
  (`placement.ts:806-869`) — nine call sites across six producers; the M0 gate
...
- **THREE front-door refusals become queued intents** (r3 — the cap check is
  ...
  included, §3.5), casting start (`castingSessions.ts:302-304`). A queued intent
  is a discriminated union row carrying the FULL action payload (§8.1); **at
  dequeue the payload is revalidated** — an intent no longer legal drops with a
  Tier-W `queueIntentExpired` event and a stated reason; **nothing is held while
  queued**; pre-greenlight intents name script projects (no production id exists
  before greenlight). Greenlight commitment semantics unchanged (cash + talent at
  greenlight).
- **The deleted cap's other consumers, dispositioned** (audit list generated by
  ...
  blocker kind + copy — re-based at M4 (a closed-union schema change with its
  hostile-input guards, typed in §8.1); **`adapter.ts:1286` `canGreenlightMore` +
...
for the Soundstage blueprint at every origin on the six road-served buildable
parcels (107 cells total) together with the Post/Scenery/office slate, and
publish the arithmetic. **If** the sweep shows the C2a slate does not fit, the
...
- **Commission an Original Screenplay** — a new action beside pool commission:
  ...
  concept and its blueprint are **minted at COMMISSION** (forced by the frozen V9
  `ScriptProject.conceptId` non-null shape; there is no abandon verb, so no
  cancellation hole).
...
- **Beats — genre supplies the skeleton:** each blueprint carries a 7-beat
  ...
  `requiredSetType`** — the beat structure IS the set demand (`00D`'s hypothesis,
...
- **Quality — office tier is the ceiling lever, writers are people:** blueprint
  quality potential derives from the commissioning office tier (the shipped C1
  EST-ceiling machinery — `tuning.ts:790-792` — kept as additive uplift in V1,
  a recorded deliberate divergence from the original's hard ceiling), the chosen
  direction, and a bounded purpose-keyed draw. The corpus four-tier ladder
...
- **Two traps, closed by contract (lane 14):** `correlateConceptCost`
  ...
  `candidates.ts:236` draws a concept index from the persisted sim stream — a
  test asserts the M0A corpus never founds and agent behavior is unchanged by
  pool growth. The lot commission workspace's exact-key check
...
- **The Flip does NOT seed a Theater.** The minimum starting lot is Gate +
  ...
  (the G2 self-contradiction resolves the other way). On ENDOWED lots the
...
- The release-semantic anchors become regime-aware in C2b-M3: `nextEvent`'s
  run-completed anchor and `managedWorkflowLocation`'s releaseReady → 'theater'
  fall back to the Gate on theater-less lots (an honest semantic fallback,
  law 12).
...
type BlueprintBeat = { name: string; requiredSetType: string }  // 7 per blueprint (§3.5)
...
managed-mode saves, empty for legacy; `originalScreenplays` := empty
...
- **TUNING inventory** (names + intent; values are implementation's):
  ...
  `STUDIO_EVENT_WINDOW_WEEKS`; `SCREENPLAY_BLUEPRINT_*` (quality-potential
...
| **Stale docs** | M0 repairs the laws-doc trailer, law 19's drifted pointers (or rules symbol-name resolution), the travel-greenfield note, and the TYCOON-log queue-note cross-reference; **M3 records the writer-experience contradiction and the 30-film ceiling** (both previously undocumented) | Doc-only commits, diff-verified |
...
8. **Sealed tests whose SUBJECT owner law 1 deletes — retired WITH their named
   ...
   **Plus a re-based-not-retired category** (r3): `tests/d12-p2-calibration.
   test.ts:159` and `tests/tick.test.ts:282,:327` consume the constant in live
   loop guards — re-based onto a test-local slate bound with assertions
   unchanged. Nothing else is exempt; `rev4-open-questions.md:297/:663` (the
   source `tests/tuning.test.ts` cites) is ruled together with this item —
   annotated historical, superseded by owner law 1.
...
- **M4 — Throughput.** Delete `MAX_CONCURRENT_PRODUCTIONS` (§11.8 retirements +
  ...
  panel + Call Board floor. **Gate: G10.1 (slots no longer inert on ≥4 of 5
...
- **M6 — Premiere Night V1.** Week-authority fix (the three-hop command is the
  gate; all §6-named surfaces in scope); the Gate send-off staging (marquee port
  as endowed-lot flavor; Option B floor per §18 item 8); PAUSE-class
  integration; multi-release arbitration. **Gate: premiere e2e on 5179 on BOTH
...
- **M7 — Economy remeasure + polish.** C2-ECONOMY-SNAPSHOT (19 figures,
  E-gates); weeklyBurn truth repair; remaining §10 seams (Assembly literal-2;
  F2/F3 if not landed with M2); DSF2 narrowing recorded. **Gate: snapshot
...
|---|---|---|
| OPUS-ENGINE-CORE | C2a M0–M2, M5 | `src/core` union/keys/TUNING hoists; `studioEvents`; the wrap event; V14 in `save.ts`; **`ui/src/engine/adapter.ts`** (M2 vocabulary derivation, M5 `wrap` member + `simStopFor` extraction) and **`ui/src/lot/snapshot/nextEvent.ts`** (M5 stop-ladder surfaces) |
...
| OPUS-SCREENS | C2a M2–M6 | `Dashboard.tsx`, `StudioCalendar.tsx`, `Assembly.tsx`, `LotPackageWorkspace.tsx`, and the set/stage/queue/screenplay surfaces inside `StudioLotScreen.tsx` (one-writer serialization with OPUS-TIME per milestone) |
...
Rules carried from PF1 (appendix): no force-push; `App.tsx`/`StudioLotScreen.tsx`/
`adapter.ts` are serialized one-writer surfaces per milestone; every gate figure
...
economy snapshot reproducibility. G12 quote-grammar, extended to pre-existing
sentences the cap deletion falsifies (Dashboard cap copy; Assembly second-film
line). G13 D-16 suite in the floor. G14 flake watch. G15 era-cleanliness (TUNING
...
correlateConceptCost never-re-run regression; the agent-stream guard.** Standing
...
*r3, frozen 2026-08-18 by the Fable C2 Architect after two adversarial review
cycles and four Owner ruling sets. Planning artifacts: `docs/c2-planning/00–14`
on this branch. No production code was written or modified. PF1's worktree and
branches were never touched.*
